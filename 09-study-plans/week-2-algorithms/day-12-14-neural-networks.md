...
- Direct Answer: A stack of fully connected layers.
- Why: This matters because it tells you how to reason about mlp (multi-layer perceptron).
- Pitfall: Don't answer "MLP (Multi-Layer Perceptron)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: A stack of fully connected layers.

### Universal Approximation Theorem
- Direct Answer: A network with one hidden layer can approximate any continuous function given enough neurons.
- Why: This matters because it tells you how to reason about universal approximation theorem.
- Pitfall: Don't answer "Universal Approximation Theorem" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: A network with one hidden layer can approximate any continuous function given enough neurons.

### Sigmoid/Tanh
- Direct Answer: Saturate (output approaches ±1/0 at extreme inputs), causing vanishing gradients in deep networks.
- Why: This matters because it tells you how to reason about sigmoid/tanh.
- Pitfall: Don't answer "Sigmoid/Tanh" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Saturate (output approaches ±1/0 at extreme inputs), causing vanishing gradients in deep networks.

### ReLU
- Direct Answer: $\max(0, z)$. Non-saturating, fast to compute, but can "die" (neurons stuck at 0 if bias pushes pre-activation permanently negative).
- Why: This matters because it tells you how to reason about relu.
- Pitfall: Don't answer "ReLU" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: $\max(0, z)$. Non-saturating, fast to compute, but can "die" (neurons stuck at 0 if bias pushes pre-activation permanently negative).

### Leaky ReLU / GELU
- Direct Answer: Fix ReLU's dying neuron problem.
- Why: This matters because it tells you how to reason about leaky relu / gelu.
- Pitfall: Don't answer "Leaky ReLU / GELU" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Fix ReLU's dying neuron problem.

### SGD
- Direct Answer: Simple stochastic updates. Requires careful learning rate tuning. Can escape local minima due to noise.
- Why: This matters because it tells you how to reason about sgd.
- Pitfall: Don't answer "SGD" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Simple stochastic updates. Requires careful learning rate tuning. Can escape local minima due to noise.

### Momentum SGD
- Direct Answer: Adds a velocity term to smooth out gradient noise.
- Why: This matters because it tells you how to reason about momentum sgd.
- Pitfall: Don't answer "Momentum SGD" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Adds a velocity term to smooth out gradient noise.

### Adam
- Direct Answer: Combines Momentum (don't stop) and RMSProp (adjust learning rate per parameter). Usually the default starting point.
...
- Interview line: Say: Combines Momentum (don't stop) and RMSProp (adjust learning rate per parameter). Usually the default starting point.

### L1/L2 Weight Decay
- Direct Answer: Penalizes large weights in the loss function.
- Why: This matters because it tells you how to reason about l1/l2 weight decay.
- Pitfall: Don't answer "L1/L2 Weight Decay" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Penalizes large weights in the loss function.

### Dropout
- Direct Answer: Randomly sets activations to zero during training. At test time, scales activations by the keep probability.
...
- Interview line: Say: Randomly sets activations to zero during training. At test time, scales activations by the keep probability.

### Batch Normalization
- Direct Answer: Normalizes activations within each mini-batch, stabilizing training and acting as a mild regularizer.
...
- Interview line: Say: Normalizes activations within each mini-batch, stabilizing training and acting as a mild regularizer.

### Early Stopping
- Direct Answer: Stop training when validation loss starts increasing.
- Why: This matters because it tells you how to reason about early stopping.
- Pitfall: Don't answer "Early Stopping" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Stop training when validation loss starts increasing.
