...
### Z-Score
- Direct Answer: $z = \frac{x - \mu}{\sigma}$. Typically $|z| > 3$ is an outlier.
- Why: This matters because it tells you how to reason about z-score.
- Pitfall: Don't answer "Z-Score" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: $z = \frac{x - \mu}{\sigma}$. Typically $|z| > 3$ is an outlier.

### IQR
- Direct Answer: Values outside $[Q1 - 1.5 \cdot IQR, Q3 + 1.5 \cdot IQR]$.
- Why: This matters because it tells you how to reason about iqr.
- Pitfall: Don't answer "IQR" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Values outside $[Q1 - 1.5 \cdot IQR, Q3 + 1.5 \cdot IQR]$.

### When to use
- Direct Answer: Algorithms assuming Gaussian distributions (SVM, Linear Reg, PCA).
...
- Interview line: Say: Algorithms assuming Gaussian distributions (SVM, Linear Reg, PCA).

### When to use
- Direct Answer: Neural Networks, KNN, algorithms that don't make Gaussian assumptions.
- Why: This matters because it tells you how to reason about when to use.
- Pitfall: Don't answer "When to use" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Neural Networks, KNN, algorithms that don't make Gaussian assumptions.

### Problem
- Direct Answer: "Dummy Variable Trap" (perfect multicollinearity). Always drop one column ($n-1$) in linear models.
...
- Interview line: Say: "Dummy Variable Trap" (perfect multicollinearity). Always drop one column ($n-1$) in linear models.

### Warning
- Direct Answer: High risk of Data Leakage. Always use K-fold cross-validation during encoding.
...
- Interview line: Say: High risk of Data Leakage. Always use K-fold cross-validation during encoding.
