...
### L3 / junior
- Direct Answer: 60% ML coding + theory, 30% system design foundations, 10% behavioral
- Why: This matters because it tells you how to reason about l3 / junior.
- Pitfall: Don't answer "L3 / junior" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: 60% ML coding + theory, 30% system design foundations, 10% behavioral

### L4 / mid
- Direct Answer: 40% system design, 35% theory depth, 25% behavioral
- Why: This matters because it tells you how to reason about l4 / mid.
- Pitfall: Don't answer "L4 / mid" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: 40% system design, 35% theory depth, 25% behavioral

### L5+ / senior
- Direct Answer: 50% system design + failure modes, 30% behavioral with strong ownership framing, 20% theory depth
- Why: This matters because it tells you how to reason about l5+ / senior.
- Pitfall: Don't answer "L5+ / senior" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: 50% system design + failure modes, 30% behavioral with strong ownership framing, 20% theory depth

### L1
- Direct Answer: Normalizes activations over the batch dimension per feature: $\hat{x} = (x - \mu_B)/\sigma_B$, then rescales with learnable $\gamma, \beta$.
- Why: This matters because it tells you how to reason about l1.
- Pitfall: Don't answer "L1" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Normalizes activations over the batch dimension per feature: $\hat{x} = (x - \mu_B)/\sigma_B$, then rescales with learnable $\gamma, \beta$.

### L2: Without it, each layer sees inputs whose distribution shifts as the preceding layers change during training
- Direct Answer: called internal covariate shift. The layer must simultaneously learn its task and adapt to a moving input distribution. BatchNorm fixes the input distribution, allowing a higher learning rate and faster convergence.
- Why: This matters because it tells you how to reason about l2: without it, each layer sees inputs whose distribution shifts as the preceding layers change during training.
- Pitfall: Don't answer "L2: Without it, each layer sees inputs whose distribution shifts as the preceding layers change during training" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: called internal covariate shift. The layer must simultaneously learn its task and adapt to a moving input distribution. BatchNorm fixes the input distribution, allowing a higher l…

### L3: Breaks at small batch sizes (noisy $\mu_B, \sigma_B$ estimates corrupt the normalization). Breaks for variable-length sequences (different valid positions per sample
- Direct Answer: batch statistics are meaningless). Interacts badly with dropout (dropout changes the effective batch statistics at inference if ordering matters). For these cases: LayerNorm normalizes over the feature dimension per sample — sequence-length agnostic, works at batch size 1.
- Why: This matters because it tells you how to reason about l3: breaks at small batch sizes (noisy $\mu_b, \sigma_b$ estimates corrupt the normalization). breaks for variable-length sequences (different valid positions per sample.
- Pitfall: Don't answer "L3: Breaks at small batch sizes (noisy $\mu_B, \sigma_B$ estimates corrupt the normalization). Breaks for variable-length sequences (different valid positions per sample" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: batch statistics are meaningless). Interacts badly with dropout (dropout changes the effective batch statistics at inference if ordering matters). For these cases: LayerNorm norma…

### L1
- Direct Answer: $\text{ReLU}(x) = \max(0, x)$; gradient is 1 for positive inputs, 0 for negative.
- Why: This matters because it tells you how to reason about l1.
- Pitfall: Don't answer "L1" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: $\text{ReLU}(x) = \max(0, x)$; gradient is 1 for positive inputs, 0 for negative.

### L2: Sigmoid saturates at both ends
- Direct Answer: its derivative $\sigma(x)(1-\sigma(x))$ approaches 0 for large positive or negative inputs. Backpropagating through 20 layers of sigmoid multiplies gradients by $< 0.25^{20} \approx 10^{-13}$ — effectively zero. ReLU has gradient exactly 1 for positive pre-activations, so gradients don't shrink through ReLU layers.
- Why: This matters because it tells you how to reason about l2: sigmoid saturates at both ends.
- Pitfall: Don't answer "L2: Sigmoid saturates at both ends" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: its derivative $\sigma(x)(1-\sigma(x))$ approaches 0 for large positive or negative inputs. Backpropagating through 20 layers of sigmoid multiplies gradients by $< 0.25^{20} \appr…

### L3: Dying ReLU problem
- Direct Answer: if a neuron's pre-activation is always negative (e.g., from a bad initialization or a large weight update), the gradient is always 0 and the neuron never updates. Fix: LeakyReLU ($0.01x$ for $x < 0$) or careful He initialization. For Transformers, GELU is preferred — smooth approximation of ReLU that performs better on language tasks.
- Why: This matters because it tells you how to reason about l3: dying relu problem.
- Pitfall: Don't answer "L3: Dying ReLU problem" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: if a neuron's pre-activation is always negative (e.g., from a bad initialization or a large weight update), the gradient is always 0 and the neuron never updates. Fix: LeakyReLU (…

### L1
- Direct Answer: $\lambda\sum|w_i|$; L2: $\lambda\sum w_i^2$.
- Why: This matters because it tells you how to reason about l1.
- Pitfall: Don't answer "L1" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: $\lambda\sum|w_i|$; L2: $\lambda\sum w_i^2$.

### L2: The penalty on large weights is dominated by the gradient-of-squared-term, which shrinks weights proportionally. Some weight may get small but never exactly zero
- Direct Answer: the gradient at $w = 0$ is $2\lambda w = 0$, so no further push.
- Why: This matters because it tells you how to reason about l2: the penalty on large weights is dominated by the gradient-of-squared-term, which shrinks weights proportionally. some weight may get small but never exactly zero.
- Pitfall: Don't answer "L2: The penalty on large weights is dominated by the gradient-of-squared-term, which shrinks weights proportionally. Some weight may get small but never exactly zero" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: the gradient at $w = 0$ is $2\lambda w = 0$, so no further push.

### L1: The penalty is $\lambda \cdot \text{sign}(w)$
- Direct Answer: a constant push toward zero regardless of magnitude. For small nonzero weights, this push exceeds any gradient from the data, forcing them exactly to zero. This is why L1 produces sparse solutions. Probabilistic interpretation: L2 = Gaussian prior on weights (MAP), L1 = Laplace prior (fatter tails near zero — encourages sparsity).
- Why: This matters because it tells you how to reason about l1: the penalty is $\lambda \cdot \text{sign}(w)$.
- Pitfall: Don't answer "L1: The penalty is $\lambda \cdot \text{sign}(w)$" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: a constant push toward zero regardless of magnitude. For small nonzero weights, this push exceeds any gradient from the data, forcing them exactly to zero. This is why L1 produces…

### Tradeoff
- Direct Answer: L1 for feature selection and interpretability. L2 when all features are plausibly relevant. ElasticNet ($\alpha L1 + (1-\alpha) L2$) for both sparsity and stability.
- Why: This matters because it tells you how to reason about tradeoff.
- Pitfall: Don't answer "Tradeoff" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: L1 for feature selection and interpretability. L2 when all features are plausibly relevant. ElasticNet ($\alpha L1 + (1-\alpha) L2$) for both sparsity and stability.

### L1
- Direct Answer: CE: $-\sum y_k \log \hat{p}_k$; MSE: $\sum(y - \hat{p})^2$.
- Why: This matters because it tells you how to reason about l1.
- Pitfall: Don't answer "L1" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: CE: $-\sum y_k \log \hat{p}_k$; MSE: $\sum(y - \hat{p})^2$.

### L2: Near the decision boundary, the MSE gradient with respect to the logit is small because the sigmoid's derivative is small there. You're in the middle of learning, and the gradient provides almost no update signal. Cross-entropy's gradient with respect to the logit is $\hat{p} - y$
- Direct Answer: large and informative when the prediction is wrong, regardless of where you are in probability space.
- Why: This matters because it tells you how to reason about l2: near the decision boundary, the mse gradient with respect to the logit is small because the sigmoid's derivative is small there. you're in the middle of learning, and the gradient provides almost no update signal. cross-entropy's gradient with respect to the logit is $\hat{p} - y$.
- Pitfall: Don't answer "L2: Near the decision boundary, the MSE gradient with respect to the logit is small because the sigmoid's derivative is small there. You're in the middle of learning, and the gradient provides almost no update signal. Cross-entropy's gradient with respect to the logit is $\hat{p} - y$" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: large and informative when the prediction is wrong, regardless of where you are in probability space.

### L3: MSE assumes Gaussian noise around the target, which is appropriate for regression. Classification outputs are Bernoulli (binary) or Categorical
- Direct Answer: cross-entropy is the correct log-likelihood for these distributions. Using MSE for classification is using the wrong probabilistic model.
- Why: This matters because it tells you how to reason about l3: mse assumes gaussian noise around the target, which is appropriate for regression. classification outputs are bernoulli (binary) or categorical.
- Pitfall: Don't answer "L3: MSE assumes Gaussian noise around the target, which is appropriate for regression. Classification outputs are Bernoulli (binary) or Categorical" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: cross-entropy is the correct log-likelihood for these distributions. Using MSE for classification is using the wrong probabilistic model.

### [ ] Sigmoid, softmax, numerically stable BCE
- Direct Answer: [ ] Sigmoid, softmax, numerically stable BCE
- Why: This matters because it tells you how to reason about [ ] sigmoid, softmax, numerically stable bce.
- Pitfall: Don't answer "[ ] Sigmoid, softmax, numerically stable BCE" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: [ ] Sigmoid, softmax, numerically stable BCE

### [ ] Precision, recall, F1 with correct denominators stated verbally
- Direct Answer: [ ] Precision, recall, F1 with correct denominators stated verbally
- Why: This matters because it tells you how to reason about [ ] precision, recall, f1 with correct denominators stated verbally.
- Pitfall: Don't answer "[ ] Precision, recall, F1 with correct denominators stated verbally" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: [ ] Precision, recall, F1 with correct denominators stated verbally

### [ ] Bias-variance decomposition formula + one-sentence interpretation of each term
- Direct Answer: [ ] Bias-variance decomposition formula + one-sentence interpretation of each term
- Why: This matters because it tells you how to reason about [ ] bias-variance decomposition formula + one-sentence interpretation of each term.
- Pitfall: Don't answer "[ ] Bias-variance decomposition formula + one-sentence interpretation of each term" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: [ ] Bias-variance decomposition formula + one-sentence interpretation of each term

### [ ] Bayes theorem applied to the rare-disease test example (the answer is ~9%, not 99%)
- Direct Answer: [ ] Bayes theorem applied to the rare-disease test example (the answer is ~9%, not 99%)
- Why: This matters because it tells you how to reason about [ ] bayes theorem applied to the rare-disease test example (the answer is ~9%, not 99%).
- Pitfall: Don't answer "[ ] Bayes theorem applied to the rare-disease test example (the answer is ~9%, not 99%)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: [ ] Bayes theorem applied to the rare-disease test example (the answer is ~9%, not 99%)

### [ ] Correct p-value interpretation + two wrong interpretations stated explicitly
- Direct Answer: [ ] Correct p-value interpretation + two wrong interpretations stated explicitly
- Why: This matters because it tells you how to reason about [ ] correct p-value interpretation + two wrong interpretations stated explicitly.
- Pitfall: Don't answer "[ ] Correct p-value interpretation + two wrong interpretations stated explicitly" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: [ ] Correct p-value interpretation + two wrong interpretations stated explicitly

### [ ] Two-stage retrieval + ranking sketch
- Direct Answer: [ ] Two-stage retrieval + ranking sketch
- Why: This matters because it tells you how to reason about [ ] two-stage retrieval + ranking sketch.
- Pitfall: Don't answer "[ ] Two-stage retrieval + ranking sketch" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: [ ] Two-stage retrieval + ranking sketch

### [ ] The 10-step system design framework
- Direct Answer: [ ] The 10-step system design framework
- Why: This matters because it tells you how to reason about [ ] the 10-step system design framework.
- Pitfall: Don't answer "[ ] The 10-step system design framework" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: [ ] The 10-step system design framework

### [ ] L1 vs L2 vs Dropout vs Early stopping (mechanism, not just effect)
- Direct Answer: [ ] L1 vs L2 vs Dropout vs Early stopping (mechanism, not just effect)
- Why: This matters because it tells you how to reason about [ ] l1 vs l2 vs dropout vs early stopping (mechanism, not just effect).
- Pitfall: Don't answer "[ ] L1 vs L2 vs Dropout vs Early stopping (mechanism, not just effect)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: [ ] L1 vs L2 vs Dropout vs Early stopping (mechanism, not just effect)

### [ ] BatchNorm vs LayerNorm
- Direct Answer: with the BatchNorm failure conditions
- Why: This matters because it tells you how to reason about [ ] batchnorm vs layernorm.
- Pitfall: Don't answer "[ ] BatchNorm vs LayerNorm" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: with the BatchNorm failure conditions

### [ ] Batch vs real-time serving
- Direct Answer: with the hybrid pattern
- Why: This matters because it tells you how to reason about [ ] batch vs real-time serving.
- Pitfall: Don't answer "[ ] Batch vs real-time serving" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: with the hybrid pattern

### [ ] Precision vs recall as a cost matrix problem
- Direct Answer: [ ] Precision vs recall as a cost matrix problem
- Why: This matters because it tells you how to reason about [ ] precision vs recall as a cost matrix problem.
- Pitfall: Don't answer "[ ] Precision vs recall as a cost matrix problem" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: [ ] Precision vs recall as a cost matrix problem

### [ ] XGBoost vs neural net
- Direct Answer: with specific decision criteria
- Why: This matters because it tells you how to reason about [ ] xgboost vs neural net.
- Pitfall: Don't answer "[ ] XGBoost vs neural net" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: with specific decision criteria

### [ ] Two-tower recommendation system (retrieval + ranking + cold start path)
- Direct Answer: [ ] Two-tower recommendation system (retrieval + ranking + cold start path)
- Why: This matters because it tells you how to reason about [ ] two-tower recommendation system (retrieval + ranking + cold start path).
- Pitfall: Don't answer "[ ] Two-tower recommendation system (retrieval + ranking + cold start path)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: [ ] Two-tower recommendation system (retrieval + ranking + cold start path)

### [ ] Fraud detection pipeline (Redis velocity → rule engine → ML model → cost-aware threshold)
- Direct Answer: [ ] Fraud detection pipeline (Redis velocity → rule engine → ML model → cost-aware threshold)
- Why: This matters because it tells you how to reason about [ ] fraud detection pipeline (redis velocity → rule engine → ml model → cost-aware threshold).
- Pitfall: Don't answer "[ ] Fraud detection pipeline (Redis velocity → rule engine → ML model → cost-aware threshold)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: [ ] Fraud detection pipeline (Redis velocity → rule engine → ML model → cost-aware threshold)

### [ ] LLM serving stack (load balancer → vLLM → KV cache → streaming)
- Direct Answer: [ ] LLM serving stack (load balancer → vLLM → KV cache → streaming)
- Why: This matters because it tells you how to reason about [ ] llm serving stack (load balancer → vllm → kv cache → streaming).
- Pitfall: Don't answer "[ ] LLM serving stack (load balancer → vLLM → KV cache → streaming)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: [ ] LLM serving stack (load balancer → vLLM → KV cache → streaming)

### [ ] Temporal train/val/test split
- Direct Answer: why random splits fail for time-series
- Why: This matters because it tells you how to reason about [ ] temporal train/val/test split.
- Pitfall: Don't answer "[ ] Temporal train/val/test split" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: why random splits fail for time-series
