...
### Spam filter
- Direct Answer: FP (blocking legitimate email) is costly → prioritize precision
...
- Interview line: Say: FP (blocking legitimate email) is costly → prioritize precision

### Cancer screening
- Direct Answer: FN (missing a case) is costly → prioritize recall
- Why: This matters because it tells you how to reason about cancer screening.
- Pitfall: Don't answer "Cancer screening" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: FN (missing a case) is costly → prioritize recall

### Fraud
- Direct Answer: FN usually costs more (undetected fraud) → recall-oriented
- Why: This matters because it tells you how to reason about fraud.
- Pitfall: Don't answer "Fraud" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: FN usually costs more (undetected fraud) → recall-oriented

### ROC-AUC: measures separation across all thresholds. Optimistic on imbalanced data because TN is plentiful and easy
- Direct Answer: FPR = FP/(FP+TN) looks artificially small.
- Why: This matters because it tells you how to reason about roc-auc: measures separation across all thresholds. optimistic on imbalanced data because tn is plentiful and easy.
- Pitfall: Don't answer "ROC-AUC: measures separation across all thresholds. Optimistic on imbalanced data because TN is plentiful and easy" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: FPR = FP/(FP+TN) looks artificially small.

### PR-AUC
- Direct Answer: does not involve TN. Better choice when the positive class is rare (fraud, disease). A random classifier gets PR-AUC ≈ class prevalence; ROC-AUC ≈ 0.5 regardless of prevalence.
...
- Interview line: Say: does not involve TN. Better choice when the positive class is rare (fraud, disease). A random classifier gets PR-AUC ≈ class prevalence; ROC-AUC ≈ 0.5 regardless of prevalence.

### Xavier / Glorot (sigmoid, tanh): $\sigma = \sqrt{2/(n_{in}+n_{out})}$
- Direct Answer: balances forward and backward variance
- Why: This matters because it tells you how to reason about xavier / glorot (sigmoid, tanh): $\sigma = \sqrt{2/(n_{in}+n_{out})}$.
- Pitfall: Don't answer "Xavier / Glorot (sigmoid, tanh): $\sigma = \sqrt{2/(n_{in}+n_{out})}$" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: balances forward and backward variance

### He (ReLU): $\sigma = \sqrt{2/n_{in}}$
- Direct Answer: accounts for ReLU zeroing ~half of activations
- Why: This matters because it tells you how to reason about he (relu): $\sigma = \sqrt{2/n_{in}}$.
- Pitfall: Don't answer "He (ReLU): $\sigma = \sqrt{2/n_{in}}$" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: accounts for ReLU zeroing ~half of activations

### Warmup: linear ramp from 0 for first 1–5% of steps. Reason: at random initialization, gradient directions are chaotic
- Direct Answer: a high LR causes divergence. Warmup lets the network find a good basin first.
- Why: This matters because it tells you how to reason about warmup: linear ramp from 0 for first 1–5% of steps. reason: at random initialization, gradient directions are chaotic.
- Pitfall: Don't answer "Warmup: linear ramp from 0 for first 1–5% of steps. Reason: at random initialization, gradient directions are chaotic" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: a high LR causes divergence. Warmup lets the network find a good basin first.

### Cosine decay: $\alpha_t = \alpha_{\min} + \frac{1}{2}(\alpha_{\max} - \alpha_{\min})(1 + \cos(\pi t/T))$. Smooth decay
- Direct Answer: model fine-tunes as LR decreases.
- Why: This matters because it tells you how to reason about cosine decay: $\alpha_t = \alpha_{\min} + \frac{1}{2}(\alpha_{\max} - \alpha_{\min})(1 + \cos(\pi t/t))$. smooth decay.
- Pitfall: Don't answer "Cosine decay: $\alpha_t = \alpha_{\min} + \frac{1}{2}(\alpha_{\max} - \alpha_{\min})(1 + \cos(\pi t/T))$. Smooth decay" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: model fine-tunes as LR decreases.

### Small batch ($B < 8$)
- Direct Answer: noisy mean/variance estimates corrupt normalization
- Why: This matters because it tells you how to reason about small batch ($b < 8$).
- Pitfall: Don't answer "Small batch ($B < 8$)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: noisy mean/variance estimates corrupt normalization

### Variable-length sequences
- Direct Answer: different samples have different valid positions; batch statistics are meaningless
- Why: This matters because it tells you how to reason about variable-length sequences.
- Pitfall: Don't answer "Variable-length sequences" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: different samples have different valid positions; batch statistics are meaningless

### Inference with batch size 1: uses running statistics from training
- Direct Answer: fine if distribution hasn't shifted
- Why: This matters because it tells you how to reason about inference with batch size 1: uses running statistics from training.
- Pitfall: Don't answer "Inference with batch size 1: uses running statistics from training" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: fine if distribution hasn't shifted

### t-SNE
- Direct Answer: preserves local neighborhoods; not reproducible unless seeded; no inverse transform; only for visualization
- Why: This matters because it tells you how to reason about t-sne.
- Pitfall: Don't answer "t-SNE" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: preserves local neighborhoods; not reproducible unless seeded; no inverse transform; only for visualization

### UMAP
- Direct Answer: preserves both local and global structure better; faster; reproducible; can produce embeddings for downstream use
- Why: This matters because it tells you how to reason about umap.
- Pitfall: Don't answer "UMAP" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: preserves both local and global structure better; faster; reproducible; can produce embeddings for downstream use

### Data drift ($P(X)$ changes): feature distributions shift
- Direct Answer: model applies to different inputs than it trained on. Detect with KS test or PSI per feature.
- Why: This matters because it tells you how to reason about data drift ($p(x)$ changes): feature distributions shift.
- Pitfall: Don't answer "Data drift ($P(X)$ changes): feature distributions shift" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: model applies to different inputs than it trained on. Detect with KS test or PSI per feature.

### Concept drift ($P(Y \mid X)$ changes)
- Direct Answer: the relationship between inputs and labels changes (e.g., fraud patterns evolve). Detect by monitoring accuracy on a recent labeled slice.
- Why: This matters because it tells you how to reason about concept drift ($p(y \mid x)$ changes).
- Pitfall: Don't answer "Concept drift ($P(Y \mid X)$ changes)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: the relationship between inputs and labels changes (e.g., fraud patterns evolve). Detect by monitoring accuracy on a recent labeled slice.

### Label drift ($P(Y)$ changes)
- Direct Answer: class balance shifts (seasonal effects on click rates). Detect by monitoring score distribution.
- Why: This matters because it tells you how to reason about label drift ($p(y)$ changes).
- Pitfall: Don't answer "Label drift ($P(Y)$ changes)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: class balance shifts (seasonal effects on click rates). Detect by monitoring score distribution.
