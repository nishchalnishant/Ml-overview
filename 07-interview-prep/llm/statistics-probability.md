...
- Direct Answer: why adding noise sources adds variances
- Why: This matters because it tells you how to reason about sum of independent gaussians: $x+y \sim \mathcal{n}(\mu_x+\mu_y, \sigma_x^2+\sigma_y^2)$.
- Pitfall: Don't answer "Sum of independent Gaussians: $X+Y \sim \mathcal{N}(\mu_X+\mu_Y, \sigma_X^2+\sigma_Y^2)$" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: why adding noise sources adds variances

### 68-95-99.7 rule: $P(\mu \pm \sigma) \approx 68\%$, $P(\mu \pm 2\sigma) \approx 95\%$, $P(\mu \pm 3\sigma) \approx 99.7\%$
- Direct Answer: the basis for anomaly detection thresholds
- Why: This matters because it tells you how to reason about 68-95-99.7 rule: $p(\mu \pm \sigma) \approx 68\%$, $p(\mu \pm 2\sigma) \approx 95\%$, $p(\mu \pm 3\sigma) \approx 99.7\%$.
- Pitfall: Don't answer "68-95-99.7 rule: $P(\mu \pm \sigma) \approx 68\%$, $P(\mu \pm 2\sigma) \approx 95\%$, $P(\mu \pm 3\sigma) \approx 99.7\%$" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: the basis for anomaly detection thresholds

### Not $P(H_0 \text{ is true})$
- Direct Answer: that requires a prior (Bayesian reasoning)
- Why: This matters because it tells you how to reason about not $p(h_0 \text{ is true})$.
- Pitfall: Don't answer "Not $P(H_0 \text{ is true})$" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: that requires a prior (Bayesian reasoning)

### Not "the probability your result is a fluke"
- Direct Answer: Not "the probability your result is a fluke"
- Why: This matters because it tells you how to reason about not "the probability your result is a fluke".
- Pitfall: Don't answer "Not "the probability your result is a fluke"" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Not "the probability your result is a fluke"

### Not a measure of effect size
- Direct Answer: a tiny, meaningless difference can be highly significant with enough data
- Why: This matters because it tells you how to reason about not a measure of effect size.
- Pitfall: Don't answer "Not a measure of effect size" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: a tiny, meaningless difference can be highly significant with enough data

### Not stable
- Direct Answer: two experiments with $p = 0.049$ and $p = 0.051$ are effectively identical; the 0.05 threshold is arbitrary
- Why: This matters because it tells you how to reason about not stable.
- Pitfall: Don't answer "Not stable" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: two experiments with $p = 0.049$ and $p = 0.051$ are effectively identical; the 0.05 threshold is arbitrary

### $\alpha$ = significance level = Type I error rate. You set this; typical value 0.05.
- Direct Answer: $\alpha$ = significance level = Type I error rate. You set this; typical value 0.05.
- Why: This matters because it tells you how to reason about $\alpha$ = significance level = type i error rate. you set this; typical value 0.05..
- Pitfall: Don't answer "$\alpha$ = significance level = Type I error rate. You set this; typical value 0.05." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: $\alpha$ = significance level = Type I error rate. You set this; typical value 0.05.

### Power = $1 - \beta$ = probability of detecting a true effect. Set target (commonly 0.8 or 0.9); determines required sample size.
- Direct Answer: Power = $1 - \beta$ = probability of detecting a true effect. Set target (commonly 0.8 or 0.9); determines required sample size.
- Why: This matters because it tells you how to reason about power = $1 - \beta$ = probability of detecting a true effect. set target (commonly 0.8 or 0.9); determines required sample size..
- Pitfall: Don't answer "Power = $1 - \beta$ = probability of detecting a true effect. Set target (commonly 0.8 or 0.9); determines required sample size." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Power = $1 - \beta$ = probability of detecting a true effect. Set target (commonly 0.8 or 0.9); determines required sample size.

### Correct
- Direct Answer: "If we repeated this experiment many times and computed a CI each time, 95% of those intervals would contain the true parameter."
- Why: This matters because it tells you how to reason about correct.
- Pitfall: Don't answer "Correct" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: "If we repeated this experiment many times and computed a CI each time, 95% of those intervals would contain the true parameter."

### Incorrect
- Direct Answer: "There is a 95% probability the true value is in this specific interval."
- Why: This matters because it tells you how to reason about incorrect.
- Pitfall: Don't answer "Incorrect" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: "There is a 95% probability the true value is in this specific interval."

### It justifies treating test set accuracy (an average over test examples) as approximately Gaussian
- Direct Answer: enabling confidence intervals and hypothesis tests
- Why: This matters because it tells you how to reason about it justifies treating test set accuracy (an average over test examples) as approximately gaussian.
- Pitfall: Don't answer "It justifies treating test set accuracy (an average over test examples) as approximately Gaussian" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: enabling confidence intervals and hypothesis tests

### It explains why neural network weights at initialization, if they're a sum of many random contributions, tend to be Gaussian
- Direct Answer: It explains why neural network weights at initialization, if they're a sum of many random contributions, tend to be Gaussian
- Why: This matters because it tells you how to reason about it explains why neural network weights at initialization, if they're a sum of many random contributions, tend to be gaussian.
- Pitfall: Don't answer "It explains why neural network weights at initialization, if they're a sum of many random contributions, tend to be Gaussian" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: It explains why neural network weights at initialization, if they're a sum of many random contributions, tend to be Gaussian

### It's why batch normalization works
- Direct Answer: the mean and variance of a minibatch approximate population statistics better as batch size grows
- Why: This matters because it tells you how to reason about it's why batch normalization works.
- Pitfall: Don't answer "It's why batch normalization works" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: the mean and variance of a minibatch approximate population statistics better as batch size grows

### $D_{\text{KL}} \geq 0$, equals 0 iff $p = q$ exactly
- Direct Answer: $D_{\text{KL}} \geq 0$, equals 0 iff $p = q$ exactly
- Why: This matters because it tells you how to reason about $d_{\text{kl}} \geq 0$, equals 0 iff $p = q$ exactly.
- Pitfall: Don't answer "$D_{\text{KL}} \geq 0$, equals 0 iff $p = q$ exactly" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: $D_{\text{KL}} \geq 0$, equals 0 iff $p = q$ exactly

### Not symmetric: $D_{\text{KL}}(p\|q) \neq D_{\text{KL}}(q\|p)$
- Direct Answer: the "forward" and "reverse" KL make different approximation choices
- Why: This matters because it tells you how to reason about not symmetric: $d_{\text{kl}}(p\|q) \neq d_{\text{kl}}(q\|p)$.
- Pitfall: Don't answer "Not symmetric: $D_{\text{KL}}(p\|q) \neq D_{\text{KL}}(q\|p)$" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: the "forward" and "reverse" KL make different approximation choices

### Minimizing cross-entropy loss = minimizing KL divergence to true labels + constant (since $H(p)$ is fixed)
- Direct Answer: Minimizing cross-entropy loss = minimizing KL divergence to true labels + constant (since $H(p)$ is fixed)
- Why: This matters because it tells you how to reason about minimizing cross-entropy loss = minimizing kl divergence to true labels + constant (since $h(p)$ is fixed).
- Pitfall: Don't answer "Minimizing cross-entropy loss = minimizing KL divergence to true labels + constant (since $H(p)$ is fixed)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Minimizing cross-entropy loss = minimizing KL divergence to true labels + constant (since $H(p)$ is fixed)

### VAE loss
- Direct Answer: KL between the approximate posterior and the prior regularizes the latent space
- Why: This matters because it tells you how to reason about vae loss.
- Pitfall: Don't answer "VAE loss" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: KL between the approximate posterior and the prior regularizes the latent space

### DPO
- Direct Answer: KL penalty between the policy and the reference model controls how far fine-tuning can deviate
...
- Interview line: Say: KL penalty between the policy and the reference model controls how far fine-tuning can deviate

### Knowledge distillation
- Direct Answer: KL between teacher and student output distributions
...
- Interview line: Say: KL between teacher and student output distributions
