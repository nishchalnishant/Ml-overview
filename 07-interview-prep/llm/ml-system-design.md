...
### Real-time when batch would do
- Direct Answer: you've built a 100ms serving pipeline for decisions that could be pre-computed. Latency budget wasted, complexity increased.
- Why: This matters because it tells you how to reason about real-time when batch would do.
- Pitfall: Don't answer "Real-time when batch would do" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: you've built a 100ms serving pipeline for decisions that could be pre-computed. Latency budget wasted, complexity increased.

### Batch when real-time needed
- Direct Answer: your fraud system scores transactions from yesterday. Every real fraud during the gap goes undetected.
- Why: This matters because it tells you how to reason about batch when real-time needed.
- Pitfall: Don't answer "Batch when real-time needed" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: your fraud system scores transactions from yesterday. Every real fraud during the gap goes undetected.

### Matrix Factorization (ALS): $\hat{r}_{ui} = u_i^T v_j$
- Direct Answer: fast retrieval, no cold start
- Why: This matters because it tells you how to reason about matrix factorization (als): $\hat{r}_{ui} = u_i^t v_j$.
- Pitfall: Don't answer "Matrix Factorization (ALS): $\hat{r}_{ui} = u_i^T v_j$" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: fast retrieval, no cold start

### Two-tower: separate user and item encoders, dot product at serving
- Direct Answer: scalable, handles cold start via content features
- Why: This matters because it tells you how to reason about two-tower: separate user and item encoders, dot product at serving.
- Pitfall: Don't answer "Two-tower: separate user and item encoders, dot product at serving" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: scalable, handles cold start via content features

### DCN (Deep & Cross Network): explicit feature crosses + deep network
- Direct Answer: strong ranking model
- Why: This matters because it tells you how to reason about dcn (deep & cross network): explicit feature crosses + deep network.
- Pitfall: Don't answer "DCN (Deep & Cross Network): explicit feature crosses + deep network" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: strong ranking model

### SASRec / BST: transformer over user's interaction sequence
- Direct Answer: best for session-aware recommendations
- Why: This matters because it tells you how to reason about sasrec / bst: transformer over user's interaction sequence.
- Pitfall: Don't answer "SASRec / BST: transformer over user's interaction sequence" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: best for session-aware recommendations

### Exploration
- Direct Answer: $\epsilon$-greedy (random 5% of slots), UCB, Thompson sampling
- Why: This matters because it tells you how to reason about exploration.
- Pitfall: Don't answer "Exploration" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: $\epsilon$-greedy (random 5% of slots), UCB, Thompson sampling

### Inverse propensity scoring
- Direct Answer: weight training examples by the probability that they were shown
- Why: This matters because it tells you how to reason about inverse propensity scoring.
- Pitfall: Don't answer "Inverse propensity scoring" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: weight training examples by the probability that they were shown

### Diversity constraints at re-ranking
- Direct Answer: cap same-category items
- Why: This matters because it tells you how to reason about diversity constraints at re-ranking.
- Pitfall: Don't answer "Diversity constraints at re-ranking" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: cap same-category items

### < 10ms latency required
- Direct Answer: GBT inference is microseconds; neural net is milliseconds
- Why: This matters because it tells you how to reason about < 10ms latency required.
- Pitfall: Don't answer "< 10ms latency required" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: GBT inference is microseconds; neural net is milliseconds

### Tabular velocity features
- Direct Answer: GBT handles mixed types natively, no embedding needed
- Why: This matters because it tells you how to reason about tabular velocity features.
- Pitfall: Don't answer "Tabular velocity features" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: GBT handles mixed types natively, no embedding needed

### Interpretability
- Direct Answer: regulators require explainable decisions; SHAP on GBT is fast and reliable
- Why: This matters because it tells you how to reason about interpretability.
- Pitfall: Don't answer "Interpretability" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: regulators require explainable decisions; SHAP on GBT is fast and reliable

### Adversarial
- Direct Answer: fraudsters adapt. Retraining cadence matters more than model accuracy.
- Why: This matters because it tells you how to reason about adversarial.
- Pitfall: Don't answer "Adversarial" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: fraudsters adapt. Retraining cadence matters more than model accuracy.

### Imbalanced
- Direct Answer: 0.1–1% positive rate. Default threshold 0.5 is wrong.
- Why: This matters because it tells you how to reason about imbalanced.
- Pitfall: Don't answer "Imbalanced" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: 0.1–1% positive rate. Default threshold 0.5 is wrong.

### Temporal
- Direct Answer: time-since-last-transaction and velocity features are the most predictive. Missing these = large accuracy loss.
- Why: This matters because it tells you how to reason about temporal.
- Pitfall: Don't answer "Temporal" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: time-since-last-transaction and velocity features are the most predictive. Missing these = large accuracy loss.

### Spell correction → tokenization → intent classification (navigational / informational / transactional) → entity extraction
- Direct Answer: Spell correction → tokenization → intent classification (navigational / informational / transactional) → entity extraction
- Why: This matters because it tells you how to reason about spell correction → tokenization → intent classification (navigational / informational / transactional) → entity extraction.
- Pitfall: Don't answer "Spell correction → tokenization → intent classification (navigational / informational / transactional) → entity extraction" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Spell correction → tokenization → intent classification (navigational / informational / transactional) → entity extraction

### NDCG@K: position-discounted cumulative gain
- Direct Answer: standard metric
- Why: This matters because it tells you how to reason about ndcg@k: position-discounted cumulative gain.
- Pitfall: Don't answer "NDCG@K: position-discounted cumulative gain" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: standard metric

### MRR: $\frac{1}{|Q|}\sum_q \frac{1}{\text{rank of first relevant doc}}$
- Direct Answer: good for navigational queries
- Why: This matters because it tells you how to reason about mrr: $\frac{1}{|q|}\sum_q \frac{1}{\text{rank of first relevant doc}}$.
- Pitfall: Don't answer "MRR: $\frac{1}{|Q|}\sum_q \frac{1}{\text{rank of first relevant doc}}$" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: good for navigational queries

### A/B on CTR
- Direct Answer: the ground truth
- Why: This matters because it tells you how to reason about a/b on ctr.
- Pitfall: Don't answer "A/B on CTR" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: the ground truth

### 70B model in BF16
- Direct Answer: 140GB VRAM → minimum 2× H100 80GB with tensor parallelism
- Why: This matters because it tells you how to reason about 70b model in bf16.
- Pitfall: Don't answer "70B model in BF16" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: 140GB VRAM → minimum 2× H100 80GB with tensor parallelism

### KV cache per request
- Direct Answer: $2 \times L \times H_{kv} \times d_h \times T$ bytes per token
- Why: This matters because it tells you how to reason about kv cache per request.
- Pitfall: Don't answer "KV cache per request" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: $2 \times L \times H_{kv} \times d_h \times T$ bytes per token

### LLaMA 3 70B (80 layers, 8 GQA heads, 128 head dim)
- Direct Answer: ~160MB per 1K tokens
- Why: This matters because it tells you how to reason about llama 3 70b (80 layers, 8 gqa heads, 128 head dim).
- Pitfall: Don't answer "LLaMA 3 70B (80 layers, 8 GQA heads, 128 head dim)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: ~160MB per 1K tokens

### At 100 QPS and 500 token avg response
- Direct Answer: need KV cache capacity for ~50 concurrent active requests
- Why: This matters because it tells you how to reason about at 100 qps and 500 token avg response.
- Pitfall: Don't answer "At 100 QPS and 500 token avg response" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: need KV cache capacity for ~50 concurrent active requests

### INT8
- Direct Answer: ~2% quality drop, 2× memory reduction, faster kernels
- Why: This matters because it tells you how to reason about int8.
- Pitfall: Don't answer "INT8" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: ~2% quality drop, 2× memory reduction, faster kernels

### INT4 (GPTQ/AWQ): ~5% quality drop, 4× memory reduction
- Direct Answer: enables 70B on 2× A100 40GB
- Why: This matters because it tells you how to reason about int4 (gptq/awq): ~5% quality drop, 4× memory reduction.
- Pitfall: Don't answer "INT4 (GPTQ/AWQ): ~5% quality drop, 4× memory reduction" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: enables 70B on 2× A100 40GB

### FP8
- Direct Answer: near-zero quality drop, 2× reduction, requires Hopper+ GPU
- Why: This matters because it tells you how to reason about fp8.
- Pitfall: Don't answer "FP8" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: near-zero quality drop, 2× reduction, requires Hopper+ GPU

### Scheduled
- Direct Answer: weekly regardless of drift (handles slow concept drift)
...
- Interview line: Say: weekly regardless of drift (handles slow concept drift)

### Triggered
- Direct Answer: PSI > 0.25 on critical features, or business KPI drop > threshold
...
- Interview line: Say: PSI > 0.25 on critical features, or business KPI drop > threshold

### Continuous: online learning for high-velocity signals (ads, fraud)
- Direct Answer: requires careful validation to avoid training on noise
- Why: This matters because it tells you how to reason about continuous: online learning for high-velocity signals (ads, fraud).
- Pitfall: Don't answer "Continuous: online learning for high-velocity signals (ads, fraud)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: requires careful validation to avoid training on noise
