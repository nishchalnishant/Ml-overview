...
### Black defendants were nearly twice as likely to be falsely flagged as high-risk (false positive rate
- Direct Answer: 44.9% vs 23.5%)
- Why: This matters because it tells you how to reason about black defendants were nearly twice as likely to be falsely flagged as high-risk (false positive rate.
- Pitfall: Don't answer "Black defendants were nearly twice as likely to be falsely flagged as high-risk (false positive rate" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: 44.9% vs 23.5%)

### White defendants were more likely to be falsely classified as low-risk (false negative rate
- Direct Answer: 47.7% vs 28.0%)
- Why: This matters because it tells you how to reason about white defendants were more likely to be falsely classified as low-risk (false negative rate.
- Pitfall: Don't answer "White defendants were more likely to be falsely classified as low-risk (false negative rate" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: 47.7% vs 28.0%)

### Direct
- Direct Answer: name, email, phone number, SSN, passport number
- Why: This matters because it tells you how to reason about direct.
- Pitfall: Don't answer "Direct" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: name, email, phone number, SSN, passport number

### Quasi-identifiers
- Direct Answer: ZIP code, date of birth, gender, job title, employer, medical condition, browser fingerprint
- Why: This matters because it tells you how to reason about quasi-identifiers.
- Pitfall: Don't answer "Quasi-identifiers" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: ZIP code, date of birth, gender, job title, employer, medical condition, browser fingerprint

### Lawful basis for processing
- Direct Answer: need consent or legitimate interest to train on personal data
- Why: This matters because it tells you how to reason about lawful basis for processing.
- Pitfall: Don't answer "Lawful basis for processing" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: need consent or legitimate interest to train on personal data

### Data minimization
- Direct Answer: collect only what you need (in tension with "collect everything" ML instinct)
- Why: This matters because it tells you how to reason about data minimization.
- Pitfall: Don't answer "Data minimization" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: collect only what you need (in tension with "collect everything" ML instinct)

### Purpose limitation
- Direct Answer: data collected for fraud detection cannot be repurposed to train a recommendation model
- Why: This matters because it tells you how to reason about purpose limitation.
- Pitfall: Don't answer "Purpose limitation" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: data collected for fraud detection cannot be repurposed to train a recommendation model

### Storage limitation
- Direct Answer: personal data cannot be retained indefinitely, creating challenges for models trained on historical records
- Why: This matters because it tells you how to reason about storage limitation.
- Pitfall: Don't answer "Storage limitation" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: personal data cannot be retained indefinitely, creating challenges for models trained on historical records

### Right to erasure
- Direct Answer: individuals can request deletion of their data
- Why: This matters because it tells you how to reason about right to erasure.
- Pitfall: Don't answer "Right to erasure" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: individuals can request deletion of their data

### Anonymization requires k-anonymity (each record indistinguishable from at least k-1 others), l-diversity, t-closeness, or formal differential privacy guarantees
- Direct Answer: Anonymization requires k-anonymity (each record indistinguishable from at least k-1 others), l-diversity, t-closeness, or formal differential privacy guarantees
- Why: This matters because it tells you how to reason about anonymization requires k-anonymity (each record indistinguishable from at least k-1 others), l-diversity, t-closeness, or formal differential privacy guarantees.
- Pitfall: Don't answer "Anonymization requires k-anonymity (each record indistinguishable from at least k-1 others), l-diversity, t-closeness, or formal differential privacy guarantees" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Anonymization requires k-anonymity (each record indistinguishable from at least k-1 others), l-diversity, t-closeness, or formal differential privacy guarantees

### Simply removing identifying columns is not anonymization
- Direct Answer: Simply removing identifying columns is not anonymization
- Why: This matters because it tells you how to reason about simply removing identifying columns is not anonymization.
- Pitfall: Don't answer "Simply removing identifying columns is not anonymization" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Simply removing identifying columns is not anonymization

### Every dataset released from a company should be treated as potentially re-identifiable
- Direct Answer: Every dataset released from a company should be treated as potentially re-identifiable
- Why: This matters because it tells you how to reason about every dataset released from a company should be treated as potentially re-identifiable.
- Pitfall: Don't answer "Every dataset released from a company should be treated as potentially re-identifiable" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Every dataset released from a company should be treated as potentially re-identifiable

### ε = 0
- Direct Answer: perfectly private, completely useless (output is constant)
- Why: This matters because it tells you how to reason about ε = 0.
- Pitfall: Don't answer "ε = 0" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: perfectly private, completely useless (output is constant)

### ε small (0.1–1)
- Direct Answer: strong privacy, significant accuracy loss
- Why: This matters because it tells you how to reason about ε small (0.1–1).
- Pitfall: Don't answer "ε small (0.1–1)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: strong privacy, significant accuracy loss

### ε = 1
- Direct Answer: industry academic standard for strong DP
...
- Interview line: Say: industry academic standard for strong DP

### ε = 10
- Direct Answer: Apple/Google range for telemetry; pragmatically private
...
- Interview line: Say: Apple/Google range for telemetry; pragmatically private

### ε large
- Direct Answer: weak guarantee, barely more than publishing raw data
- Why: This matters because it tells you how to reason about ε large.
- Pitfall: Don't answer "ε large" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: weak guarantee, barely more than publishing raw data

### sensitivity = 200K / 10,000 = 20
- Direct Answer: sensitivity = 200K / 10,000 = 20
- Why: This matters because it tells you how to reason about sensitivity = 200k / 10,000 = 20.
- Pitfall: Don't answer "sensitivity = 200K / 10,000 = 20" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: sensitivity = 200K / 10,000 = 20

### ε = 1 → scale = 20/1 = 20 → standard deviation ≈ 28K
- Direct Answer: ε = 1 → scale = 20/1 = 20 → standard deviation ≈ 28K
- Why: This matters because it tells you how to reason about ε = 1 → scale = 20/1 = 20 → standard deviation ≈ 28k.
- Pitfall: Don't answer "ε = 1 → scale = 20/1 = 20 → standard deviation ≈ 28K" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: ε = 1 → scale = 20/1 = 20 → standard deviation ≈ 28K

### ε = 10 → scale = 2 → standard deviation ≈ 2.8K
- Direct Answer: ε = 10 → scale = 2 → standard deviation ≈ 2.8K
- Why: This matters because it tells you how to reason about ε = 10 → scale = 2 → standard deviation ≈ 2.8k.
- Pitfall: Don't answer "ε = 10 → scale = 2 → standard deviation ≈ 2.8K" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: ε = 10 → scale = 2 → standard deviation ≈ 2.8K

### Large models (more dimensions → more total noise)
- Direct Answer: Large models (more dimensions → more total noise)
- Why: This matters because it tells you how to reason about large models (more dimensions → more total noise).
- Pitfall: Don't answer "Large models (more dimensions → more total noise)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Large models (more dimensions → more total noise)

### Small batch sizes (noise-to-signal ratio is higher)
- Direct Answer: Small batch sizes (noise-to-signal ratio is higher)
- Why: This matters because it tells you how to reason about small batch sizes (noise-to-signal ratio is higher).
- Pitfall: Don't answer "Small batch sizes (noise-to-signal ratio is higher)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Small batch sizes (noise-to-signal ratio is higher)

### Tight ε requirements (more noise needed)
- Direct Answer: Tight ε requirements (more noise needed)
- Why: This matters because it tells you how to reason about tight ε requirements (more noise needed).
- Pitfall: Don't answer "Tight ε requirements (more noise needed)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Tight ε requirements (more noise needed)

### Data is legally or contractually impossible to centralize (HIPAA, GDPR, inter-hospital data sharing)
- Direct Answer: Data is legally or contractually impossible to centralize (HIPAA, GDPR, inter-hospital data sharing)
- Why: This matters because it tells you how to reason about data is legally or contractually impossible to centralize (hipaa, gdpr, inter-hospital data sharing).
- Pitfall: Don't answer "Data is legally or contractually impossible to centralize (HIPAA, GDPR, inter-hospital data sharing)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Data is legally or contractually impossible to centralize (HIPAA, GDPR, inter-hospital data sharing)

### Data is physically too large to move (satellite imagery, medical imaging across 10,000 hospitals)
- Direct Answer: Data is physically too large to move (satellite imagery, medical imaging across 10,000 hospitals)
- Why: This matters because it tells you how to reason about data is physically too large to move (satellite imagery, medical imaging across 10,000 hospitals).
- Pitfall: Don't answer "Data is physically too large to move (satellite imagery, medical imaging across 10,000 hospitals)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Data is physically too large to move (satellite imagery, medical imaging across 10,000 hospitals)

### Users do not consent to sharing their raw data (mobile keyboard input, health sensors)
- Direct Answer: Users do not consent to sharing their raw data (mobile keyboard input, health sensors)
- Why: This matters because it tells you how to reason about users do not consent to sharing their raw data (mobile keyboard input, health sensors).
- Pitfall: Don't answer "Users do not consent to sharing their raw data (mobile keyboard input, health sensors)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Users do not consent to sharing their raw data (mobile keyboard input, health sensors)

### FedProx: add proximal term to local objective: minimize L_local(w) + (μ/2)‖w - w_t‖²
- Direct Answer: penalizes diverging from the global model
- Why: This matters because it tells you how to reason about fedprox: add proximal term to local objective: minimize l_local(w) + (μ/2)‖w - w_t‖².
- Pitfall: Don't answer "FedProx: add proximal term to local objective: minimize L_local(w) + (μ/2)‖w - w_t‖²" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: penalizes diverging from the global model

### SCAFFOLD: control variates that correct for client drift
- Direct Answer: each client tracks a local correction term
- Why: This matters because it tells you how to reason about scaffold: control variates that correct for client drift.
- Pitfall: Don't answer "SCAFFOLD: control variates that correct for client drift" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: each client tracks a local correction term

### FedNova: normalizes updates by number of local steps before aggregation
- Direct Answer: removes scale differences between clients
- Why: This matters because it tells you how to reason about fednova: normalizes updates by number of local steps before aggregation.
- Pitfall: Don't answer "FedNova: normalizes updates by number of local steps before aggregation" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: removes scale differences between clients

### Calibration (equal accuracy of predicted probabilities)
- Direct Answer: Calibration (equal accuracy of predicted probabilities)
- Why: This matters because it tells you how to reason about calibration (equal accuracy of predicted probabilities).
- Pitfall: Don't answer "Calibration (equal accuracy of predicted probabilities)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Calibration (equal accuracy of predicted probabilities)

### Equal false positive rates
- Direct Answer: Equal false positive rates
- Why: This matters because it tells you how to reason about equal false positive rates.
- Pitfall: Don't answer "Equal false positive rates" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Equal false positive rates

### Equal false negative rates
- Direct Answer: Equal false negative rates
- Why: This matters because it tells you how to reason about equal false negative rates.
- Pitfall: Don't answer "Equal false negative rates" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Equal false negative rates

### Prioritize equal FPR
- Direct Answer: false detention is the primary harm. Reduce false positives equally across groups.
- Why: This matters because it tells you how to reason about prioritize equal fpr.
- Pitfall: Don't answer "Prioritize equal FPR" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: false detention is the primary harm. Reduce false positives equally across groups.

### Prioritize equal FNR
- Direct Answer: public safety harm is the primary concern. Reduce missed re-offenders equally.
- Why: This matters because it tells you how to reason about prioritize equal fnr.
- Pitfall: Don't answer "Prioritize equal FNR" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: public safety harm is the primary concern. Reduce missed re-offenders equally.

### Prioritize calibration
- Direct Answer: if the model says 70% risk, that should be true for both groups so decision-makers get accurate information.
- Why: This matters because it tells you how to reason about prioritize calibration.
- Pitfall: Don't answer "Prioritize calibration" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: if the model says 70% risk, that should be true for both groups so decision-makers get accurate information.

### Resampling
- Direct Answer: oversample underrepresented groups; undersample overrepresented ones. Improves representation but does not fix label bias.
- Why: This matters because it tells you how to reason about resampling.
- Pitfall: Don't answer "Resampling" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: oversample underrepresented groups; undersample overrepresented ones. Improves representation but does not fix label bias.

### Reweighting
- Direct Answer: weight training examples by group membership or proxy of fairness violation. Penalizes errors on underrepresented groups more.
- Why: This matters because it tells you how to reason about reweighting.
- Pitfall: Don't answer "Reweighting" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: weight training examples by group membership or proxy of fairness violation. Penalizes errors on underrepresented groups more.

### Data augmentation
- Direct Answer: generate synthetic examples for underrepresented groups (for CV: augment dark-skinned faces in facial recognition training data).
- Why: This matters because it tells you how to reason about data augmentation.
- Pitfall: Don't answer "Data augmentation" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: generate synthetic examples for underrepresented groups (for CV: augment dark-skinned faces in facial recognition training data).

### Label cleaning
- Direct Answer: audit and correct labels that reflect annotator bias (e.g., sentiment labels applied differently to text from different cultural contexts).
- Why: This matters because it tells you how to reason about label cleaning.
- Pitfall: Don't answer "Label cleaning" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: audit and correct labels that reflect annotator bias (e.g., sentiment labels applied differently to text from different cultural contexts).

### Adversarial debiasing
- Direct Answer: add a fairness adversary during training. Predictor minimizes task loss. Adversary tries to predict the protected attribute from the predictor's representation. Predictor is penalized when adversary succeeds. Result: representations that are predictive of the target but not of the protected attribute.
- Why: This matters because it tells you how to reason about adversarial debiasing.
- Pitfall: Don't answer "Adversarial debiasing" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: add a fairness adversary during training. Predictor minimizes task loss. Adversary tries to predict the protected attribute from the predictor's representation. Predictor is penal…

### Fairness-constrained objective
- Direct Answer: Lagrangian relaxation with fairness constraint:
- Why: This matters because it tells you how to reason about fairness-constrained objective.
- Pitfall: Don't answer "Fairness-constrained objective" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Lagrangian relaxation with fairness constraint:

### Equalized odds post-processing via regularization
- Direct Answer: add a gradient-based penalty for TPR or FPR disparities across groups.
- Why: This matters because it tells you how to reason about equalized odds post-processing via regularization.
- Pitfall: Don't answer "Equalized odds post-processing via regularization" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: add a gradient-based penalty for TPR or FPR disparities across groups.

### Threshold optimization per group
- Direct Answer: find separate decision thresholds for each group that minimize the chosen fairness criterion. Simple and effective when the model's predicted probabilities are reliable.
- Why: This matters because it tells you how to reason about threshold optimization per group.
- Pitfall: Don't answer "Threshold optimization per group" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: find separate decision thresholds for each group that minimize the chosen fairness criterion. Simple and effective when the model's predicted probabilities are reliable.

### Calibrated equalized odds (Pleiss et al., 2017)
- Direct Answer: adjust output probabilities post-hoc to satisfy equalized odds.
- Why: This matters because it tells you how to reason about calibrated equalized odds (pleiss et al., 2017).
- Pitfall: Don't answer "Calibrated equalized odds (Pleiss et al., 2017)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: adjust output probabilities post-hoc to satisfy equalized odds.

### Reject-option classification
- Direct Answer: for predictions near the decision boundary, defer to a human reviewer or secondary rule. Avoids automated decisions in the high-uncertainty region where proxy bias is most harmful.
- Why: This matters because it tells you how to reason about reject-option classification.
- Pitfall: Don't answer "Reject-option classification" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: for predictions near the decision boundary, defer to a human reviewer or secondary rule. Avoids automated decisions in the high-uncertainty region where proxy bias is most harmful.

### Stereotyped completions
- Direct Answer: "The engineer walked to her..." → model completes differently based on whether name suggests male/female
- Why: This matters because it tells you how to reason about stereotyped completions.
- Pitfall: Don't answer "Stereotyped completions" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: "The engineer walked to her..." → model completes differently based on whether name suggests male/female

### Disparate quality
- Direct Answer: responses about underrepresented groups are lower quality, less factually accurate
- Why: This matters because it tells you how to reason about disparate quality.
- Pitfall: Don't answer "Disparate quality" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: responses about underrepresented groups are lower quality, less factually accurate

### Disparate representation
- Direct Answer: some groups are overrepresented as negative examples in associations (race-crime co-occurrence)
- Why: This matters because it tells you how to reason about disparate representation.
- Pitfall: Don't answer "Disparate representation" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: some groups are overrepresented as negative examples in associations (race-crime co-occurrence)

### Cultural miscalibration
- Direct Answer: model aligned to Western liberal values may fail users from other cultural contexts
- Why: This matters because it tells you how to reason about cultural miscalibration.
- Pitfall: Don't answer "Cultural miscalibration" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: model aligned to Western liberal values may fail users from other cultural contexts

### Diverse annotator pools
- Direct Answer: geographic, cultural, linguistic diversity in RLHF annotation reduces single-culture skew
- Why: This matters because it tells you how to reason about diverse annotator pools.
- Pitfall: Don't answer "Diverse annotator pools" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: geographic, cultural, linguistic diversity in RLHF annotation reduces single-culture skew

### Explicit fairness criteria in annotation guidelines
- Direct Answer: annotators are instructed on what constitutes biased output
- Why: This matters because it tells you how to reason about explicit fairness criteria in annotation guidelines.
- Pitfall: Don't answer "Explicit fairness criteria in annotation guidelines" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: annotators are instructed on what constitutes biased output

### Constitutional AI
- Direct Answer: use AI feedback grounded in explicit principles rather than relying on potentially biased human preferences alone
- Why: This matters because it tells you how to reason about constitutional ai.
- Pitfall: Don't answer "Constitutional AI" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: use AI feedback grounded in explicit principles rather than relying on potentially biased human preferences alone

### Red-teaming
- Direct Answer: systematic adversarial testing before deployment to surface differential quality across groups
- Why: This matters because it tells you how to reason about red-teaming.
- Pitfall: Don't answer "Red-teaming" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: systematic adversarial testing before deployment to surface differential quality across groups

### Output filtering
- Direct Answer: detect and filter stereotyped or biased outputs post-generation (last resort; does not fix the model)
- Why: This matters because it tells you how to reason about output filtering.
- Pitfall: Don't answer "Output filtering" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: detect and filter stereotyped or biased outputs post-generation (last resort; does not fix the model)

### Differential privacy during training: by construction limits membership inference
- Direct Answer: the output distribution is bounded to differ by at most e^ε between including and excluding any example
- Why: This matters because it tells you how to reason about differential privacy during training: by construction limits membership inference.
- Pitfall: Don't answer "Differential privacy during training: by construction limits membership inference" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: the output distribution is bounded to differ by at most e^ε between including and excluding any example

### Regularization
- Direct Answer: reduces overfitting, which reduces the training/test distribution gap
- Why: This matters because it tells you how to reason about regularization.
- Pitfall: Don't answer "Regularization" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: reduces overfitting, which reduces the training/test distribution gap

### Output calibration
- Direct Answer: well-calibrated models produce consistent confidence regardless of training membership
- Why: This matters because it tells you how to reason about output calibration.
- Pitfall: Don't answer "Output calibration" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: well-calibrated models produce consistent confidence regardless of training membership

### Limiting output precision
- Direct Answer: returning only the top class (not probabilities) reduces the signal available to attackers
- Why: This matters because it tells you how to reason about limiting output precision.
- Pitfall: Don't answer "Limiting output precision" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: returning only the top class (not probabilities) reduces the signal available to attackers

### Full names and email addresses of individuals
- Direct Answer: Full names and email addresses of individuals
- Why: This matters because it tells you how to reason about full names and email addresses of individuals.
- Pitfall: Don't answer "Full names and email addresses of individuals" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Full names and email addresses of individuals

### Copyrighted passages
- Direct Answer: Copyrighted passages
- Why: This matters because it tells you how to reason about copyrighted passages.
- Pitfall: Don't answer "Copyrighted passages" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Copyrighted passages

### Phone numbers
- Direct Answer: Phone numbers
- Why: This matters because it tells you how to reason about phone numbers.
- Pitfall: Don't answer "Phone numbers" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Phone numbers

### Performance metrics disaggregated by demographic group (accuracy, FPR, FNR per group)
- Direct Answer: Performance metrics disaggregated by demographic group (accuracy, FPR, FNR per group)
- Why: This matters because it tells you how to reason about performance metrics disaggregated by demographic group (accuracy, fpr, fnr per group).
- Pitfall: Don't answer "Performance metrics disaggregated by demographic group (accuracy, FPR, FNR per group)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Performance metrics disaggregated by demographic group (accuracy, FPR, FNR per group)

### Fairness metric measurement (demographic parity, equalized odds
- Direct Answer: chosen based on the harm model)
- Why: This matters because it tells you how to reason about fairness metric measurement (demographic parity, equalized odds.
- Pitfall: Don't answer "Fairness metric measurement (demographic parity, equalized odds" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: chosen based on the harm model)

### Membership inference attack audit
- Direct Answer: measure attack AUC on a held-out test set
- Why: This matters because it tells you how to reason about membership inference attack audit.
- Pitfall: Don't answer "Membership inference attack audit" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: measure attack AUC on a held-out test set

### Data extraction probing for LLMs
- Direct Answer: attempt extraction of known PII from training data
- Why: This matters because it tells you how to reason about data extraction probing for llms.
- Pitfall: Don't answer "Data extraction probing for LLMs" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: attempt extraction of known PII from training data

### Calibration per group
- Direct Answer: confirm predicted probabilities are equally accurate across groups
- Why: This matters because it tells you how to reason about calibration per group.
- Pitfall: Don't answer "Calibration per group" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: confirm predicted probabilities are equally accurate across groups

### Input distribution shift per group (are certain groups' inputs shifting out of training distribution?)
- Direct Answer: Input distribution shift per group (are certain groups' inputs shifting out of training distribution?)
- Why: This matters because it tells you how to reason about input distribution shift per group (are certain groups' inputs shifting out of training distribution?).
- Pitfall: Don't answer "Input distribution shift per group (are certain groups' inputs shifting out of training distribution?)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Input distribution shift per group (are certain groups' inputs shifting out of training distribution?)

### Outcome disparity tracking (are error rates changing over time for any group?)
- Direct Answer: Outcome disparity tracking (are error rates changing over time for any group?)
- Why: This matters because it tells you how to reason about outcome disparity tracking (are error rates changing over time for any group?).
- Pitfall: Don't answer "Outcome disparity tracking (are error rates changing over time for any group?)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Outcome disparity tracking (are error rates changing over time for any group?)

### Feedback loop detection (do the model's predictions affect the distribution of future inputs?)
- Direct Answer: Feedback loop detection (do the model's predictions affect the distribution of future inputs?)
- Why: This matters because it tells you how to reason about feedback loop detection (do the model's predictions affect the distribution of future inputs?).
- Pitfall: Don't answer "Feedback loop detection (do the model's predictions affect the distribution of future inputs?)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Feedback loop detection (do the model's predictions affect the distribution of future inputs?)
