...
> This covers the unlearning algorithms (exact retraining, influence functions, SISA). For how an erasure request flows through a production system end-to-end (registry lookup, influence thresholding, audit-log pseudonymization, GDPR Article 17 vs. financial retention conflicts) see [model-governance.md §5](../../06-production-ml/03-model-governance.md#5-regulatory-compliance).
