...
### "Deep learning is non-convex so convergence is not guaranteed." True but misleading
- Direct Answer: it implies optimization is unreliable. The correct framing: overparameterization makes global optima dense and easy to reach; the real concern is which local minimum you find, not whether you find one.
- Why: This matters because it tells you how to reason about "deep learning is non-convex so convergence is not guaranteed." true but misleading.
- Pitfall: Don't answer ""Deep learning is non-convex so convergence is not guaranteed." True but misleading" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: it implies optimization is unreliable. The correct framing: overparameterization makes global optima dense and easy to reach; the real concern is which local minimum you find, not…

### "Saddle points are the main obstacle to training." Empirically, training rarely stalls at saddle points in modern overparameterized networks. The more common failure mode is convergence to a sharp minimum that does not generalize. Saddle points are a theoretical concern for underparameterized problems.
- Direct Answer: "Saddle points are the main obstacle to training." Empirically, training rarely stalls at saddle points in modern overparameterized networks. The more common failure mode is convergence to a sharp minimum that does not generalize. Saddle points are a theoretical concern for underparameterized problems.
- Why: This matters because it tells you how to reason about "saddle points are the main obstacle to training." empirically, training rarely stalls at saddle points in modern overparameterized networks. the more common failure mode is convergence to a sharp minimum that does not generalize. saddle points are a theoretical concern for underparameterized problems..
- Pitfall: Don't answer ""Saddle points are the main obstacle to training." Empirically, training rarely stalls at saddle points in modern overparameterized networks. The more common failure mode is convergence to a sharp minimum that does not generalize. Saddle points are a theoretical concern for underparameterized problems." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: "Saddle points are the main obstacle to training." Empirically, training rarely stalls at saddle points in modern overparameterized networks. The more common failure mode is conve…

### Conflating non-convexity with unpredictability. In the infinite-width NTK regime, the loss is approximately convex and training dynamics are analytically predictable. Non-convexity matters for finite networks in the feature-learning regime.
- Direct Answer: Conflating non-convexity with unpredictability. In the infinite-width NTK regime, the loss is approximately convex and training dynamics are analytically predictable. Non-convexity matters for finite networks in the feature-learning regime.
- Why: This matters because it tells you how to reason about conflating non-convexity with unpredictability. in the infinite-width ntk regime, the loss is approximately convex and training dynamics are analytically predictable. non-convexity matters for finite networks in the feature-learning regime..
- Pitfall: Don't answer "Conflating non-convexity with unpredictability. In the infinite-width NTK regime, the loss is approximately convex and training dynamics are analytically predictable. Non-convexity matters for finite networks in the feature-learning regime." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Conflating non-convexity with unpredictability. In the infinite-width NTK regime, the loss is approximately convex and training dynamics are analytically predictable. Non-convexit…

### Treating variance reduction as strictly superior to SGD. For convex problems in classical ML, yes
- Direct Answer: it recovers linear convergence. For deep networks, SGD noise is a feature. Eliminating it may hurt generalization.
- Why: This matters because it tells you how to reason about treating variance reduction as strictly superior to sgd. for convex problems in classical ml, yes.
- Pitfall: Don't answer "Treating variance reduction as strictly superior to SGD. For convex problems in classical ML, yes" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: it recovers linear convergence. For deep networks, SGD noise is a feature. Eliminating it may hurt generalization.

### Forgetting that SVRG/SAGA convergence guarantees require strong convexity. In the non-convex case, the best-known rate is $O(1/T)$ to a stationary point
- Direct Answer: no better than SGD asymptotically. The linear convergence guarantee does not carry over.
- Why: This matters because it tells you how to reason about forgetting that svrg/saga convergence guarantees require strong convexity. in the non-convex case, the best-known rate is $o(1/t)$ to a stationary point.
- Pitfall: Don't answer "Forgetting that SVRG/SAGA convergence guarantees require strong convexity. In the non-convex case, the best-known rate is $O(1/T)$ to a stationary point" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: no better than SGD asymptotically. The linear convergence guarantee does not carry over.

### Ignoring the $O(nd)$ memory cost of SAGA. This single constraint rules it out for modern large-scale deep learning, regardless of its theoretical properties.
- Direct Answer: Ignoring the $O(nd)$ memory cost of SAGA. This single constraint rules it out for modern large-scale deep learning, regardless of its theoretical properties.
- Why: This matters because it tells you how to reason about ignoring the $o(nd)$ memory cost of saga. this single constraint rules it out for modern large-scale deep learning, regardless of its theoretical properties..
- Pitfall: Don't answer "Ignoring the $O(nd)$ memory cost of SAGA. This single constraint rules it out for modern large-scale deep learning, regardless of its theoretical properties." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Ignoring the $O(nd)$ memory cost of SAGA. This single constraint rules it out for modern large-scale deep learning, regardless of its theoretical properties.

### "Second-order methods always converge faster." They converge in fewer iterations, but each iteration is more expensive. The relevant comparison is wall-clock time per unit of loss reduction. In the stochastic mini-batch regime, first-order methods often win because iteration cost is low and stochastic noise prevents the Hessian approximation from being accurate anyway.
- Direct Answer: "Second-order methods always converge faster." They converge in fewer iterations, but each iteration is more expensive. The relevant comparison is wall-clock time per unit of loss reduction. In the stochastic mini-batch regime, first-order methods often win because iteration cost is low and stochastic noise prevents the Hessian approximation from being accurate anyway.
- Why: This matters because it tells you how to reason about "second-order methods always converge faster." they converge in fewer iterations, but each iteration is more expensive. the relevant comparison is wall-clock time per unit of loss reduction. in the stochastic mini-batch regime, first-order methods often win because iteration cost is low and stochastic noise prevents the hessian approximation from being accurate anyway..
- Pitfall: Don't answer ""Second-order methods always converge faster." They converge in fewer iterations, but each iteration is more expensive. The relevant comparison is wall-clock time per unit of loss reduction. In the stochastic mini-batch regime, first-order methods often win because iteration cost is low and stochastic noise prevents the Hessian approximation from being accurate anyway." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: "Second-order methods always converge faster." They converge in fewer iterations, but each iteration is more expensive. The relevant comparison is wall-clock time per unit of loss…

### Applying Newton's method to neural networks without damping. The Hessian is indefinite at saddle points
- Direct Answer: the Newton step points toward increasing loss in negative-curvature directions. Damped Newton ($H + \lambda I$) or trust-region Newton adds positive curvature to all directions, making the step safe.
- Why: This matters because it tells you how to reason about applying newton's method to neural networks without damping. the hessian is indefinite at saddle points.
- Pitfall: Don't answer "Applying Newton's method to neural networks without damping. The Hessian is indefinite at saddle points" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: the Newton step points toward increasing loss in negative-curvature directions. Damped Newton ($H + \lambda I$) or trust-region Newton adds positive curvature to all directions, m…

### Treating K-FAC's approximation as exact. K-FAC assumes input activations $a$ and gradient signals $\delta$ are independent within a layer. This is violated (especially with batch normalization), but the approximation is good enough to provide useful curvature information.
- Direct Answer: Treating K-FAC's approximation as exact. K-FAC assumes input activations $a$ and gradient signals $\delta$ are independent within a layer. This is violated (especially with batch normalization), but the approximation is good enough to provide useful curvature information.
- Why: This matters because it tells you how to reason about treating k-fac's approximation as exact. k-fac assumes input activations $a$ and gradient signals $\delta$ are independent within a layer. this is violated (especially with batch normalization), but the approximation is good enough to provide useful curvature information..
- Pitfall: Don't answer "Treating K-FAC's approximation as exact. K-FAC assumes input activations $a$ and gradient signals $\delta$ are independent within a layer. This is violated (especially with batch normalization), but the approximation is good enough to provide useful curvature information." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Treating K-FAC's approximation as exact. K-FAC assumes input activations $a$ and gradient signals $\delta$ are independent within a layer. This is violated (especially with batch…

### "Natural gradient is just Newton's method with a different matrix." Formally they are both of the form $H^{-1} g$, but the matrices differ. Newton uses the Hessian of the loss $\nabla^2 L$ (which is indefinite at saddle points). Natural gradient uses the Fisher $F = \mathbb{E}[\nabla \log p \, \nabla \log p^\top]$ (which is always positive semi-definite). Under maximum likelihood, $F = -\mathbb{E}[\nabla^2 \log p]$
- Direct Answer: they coincide at the MLE optimum but not in general. The conceptual difference: Newton operates in parameter space, natural gradient operates in distribution space.
- Why: This matters because it tells you how to reason about "natural gradient is just newton's method with a different matrix." formally they are both of the form $h^{-1} g$, but the matrices differ. newton uses the hessian of the loss $\nabla^2 l$ (which is indefinite at saddle points). natural gradient uses the fisher $f = \mathbb{e}[\nabla \log p \, \nabla \log p^\top]$ (which is always positive semi-definite). under maximum likelihood, $f = -\mathbb{e}[\nabla^2 \log p]$.
- Pitfall: Don't answer ""Natural gradient is just Newton's method with a different matrix." Formally they are both of the form $H^{-1} g$, but the matrices differ. Newton uses the Hessian of the loss $\nabla^2 L$ (which is indefinite at saddle points). Natural gradient uses the Fisher $F = \mathbb{E}[\nabla \log p \, \nabla \log p^\top]$ (which is always positive semi-definite). Under maximum likelihood, $F = -\mathbb{E}[\nabla^2 \log p]$" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: they coincide at the MLE optimum but not in general. The conceptual difference: Newton operates in parameter space, natural gradient operates in distribution space.

### Treating natural gradient as computationally feasible without approximation. Computing and inverting $F \in \mathbb{R}^{d \times d}$ costs $O(d^3)$
- Direct Answer: same issue as Newton. K-FAC makes it tractable by exploiting layer structure. Without such approximations, natural gradient is purely theoretical.
- Why: This matters because it tells you how to reason about treating natural gradient as computationally feasible without approximation. computing and inverting $f \in \mathbb{r}^{d \times d}$ costs $o(d^3)$.
- Pitfall: Don't answer "Treating natural gradient as computationally feasible without approximation. Computing and inverting $F \in \mathbb{R}^{d \times d}$ costs $O(d^3)$" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: same issue as Newton. K-FAC makes it tractable by exploiting layer structure. Without such approximations, natural gradient is purely theoretical.

### Fisher
- Direct Answer: $F(\theta) = \nabla^2 A(\theta)$ (the Hessian of the log-partition function $A$)
- Why: This matters because it tells you how to reason about fisher.
- Pitfall: Don't answer "Fisher" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: $F(\theta) = \nabla^2 A(\theta)$ (the Hessian of the log-partition function $A$)

### Natural parameters $\theta$ and expectation parameters $\mu = \nabla A(\theta) = \mathbb{E}[T(x)]$ are dual coordinate systems connected by the Legendre transform
- Direct Answer: $A^*(\mu) = \theta^\top \mu - A(\theta)$
- Why: This matters because it tells you how to reason about natural parameters $\theta$ and expectation parameters $\mu = \nabla a(\theta) = \mathbb{e}[t(x)]$ are dual coordinate systems connected by the legendre transform.
- Pitfall: Don't answer "Natural parameters $\theta$ and expectation parameters $\mu = \nabla A(\theta) = \mathbb{E}[T(x)]$ are dual coordinate systems connected by the Legendre transform" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: $A^*(\mu) = \theta^\top \mu - A(\theta)$

### The EM algorithm
- Direct Answer: E-step is a projection in expectation parameter space (compute posterior expectations); M-step is optimization in natural parameter space
- Why: This matters because it tells you how to reason about the em algorithm.
- Pitfall: Don't answer "The EM algorithm" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: E-step is a projection in expectation parameter space (compute posterior expectations); M-step is optimization in natural parameter space

### Treating information geometry as purely academic. It directly explains K-FAC (Kronecker approximation of the Fisher manifold geometry), TRPO/PPO (KL trust region = Fisher metric ball), variational inference (KL minimization = geodesic projection on the manifold), and natural language generation temperature scaling.
- Direct Answer: Treating information geometry as purely academic. It directly explains K-FAC (Kronecker approximation of the Fisher manifold geometry), TRPO/PPO (KL trust region = Fisher metric ball), variational inference (KL minimization = geodesic projection on the manifold), and natural language generation temperature scaling.
- Why: This matters because it tells you how to reason about treating information geometry as purely academic. it directly explains k-fac (kronecker approximation of the fisher manifold geometry), trpo/ppo (kl trust region = fisher metric ball), variational inference (kl minimization = geodesic projection on the manifold), and natural language generation temperature scaling..
- Pitfall: Don't answer "Treating information geometry as purely academic. It directly explains K-FAC (Kronecker approximation of the Fisher manifold geometry), TRPO/PPO (KL trust region = Fisher metric ball), variational inference (KL minimization = geodesic projection on the manifold), and natural language generation temperature scaling." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Treating information geometry as purely academic. It directly explains K-FAC (Kronecker approximation of the Fisher manifold geometry), TRPO/PPO (KL trust region = Fisher metric b…

### Confusing Fisher information with the Hessian of the loss. They are equal under maximum likelihood ($F = -\mathbb{E}[\nabla^2 \log p_\theta]$ = negative expected Hessian of log-likelihood), but the Fisher is always positive semi-definite while the loss Hessian is indefinite at saddle points. K-FAC specifically uses the Fisher because PSD is required for a valid metric.
- Direct Answer: Confusing Fisher information with the Hessian of the loss. They are equal under maximum likelihood ($F = -\mathbb{E}[\nabla^2 \log p_\theta]$ = negative expected Hessian of log-likelihood), but the Fisher is always positive semi-definite while the loss Hessian is indefinite at saddle points. K-FAC specifically uses the Fisher because PSD is required for a valid metric.
- Why: This matters because it tells you how to reason about confusing fisher information with the hessian of the loss. they are equal under maximum likelihood ($f = -\mathbb{e}[\nabla^2 \log p_\theta]$ = negative expected hessian of log-likelihood), but the fisher is always positive semi-definite while the loss hessian is indefinite at saddle points. k-fac specifically uses the fisher because psd is required for a valid metric..
- Pitfall: Don't answer "Confusing Fisher information with the Hessian of the loss. They are equal under maximum likelihood ($F = -\mathbb{E}[\nabla^2 \log p_\theta]$ = negative expected Hessian of log-likelihood), but the Fisher is always positive semi-definite while the loss Hessian is indefinite at saddle points. K-FAC specifically uses the Fisher because PSD is required for a valid metric." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Confusing Fisher information with the Hessian of the loss. They are equal under maximum likelihood ($F = -\mathbb{E}[\nabla^2 \log p_\theta]$ = negative expected Hessian of log-li…

### Ignoring coordinate system dependence. Gradient descent in natural parameters $\theta$ is not the same as gradient descent in expectation parameters $\mu$. The natural gradient is the correction that makes descent coordinate-invariant
- Direct Answer: the same distributional update regardless of how you parameterize the model.
- Why: This matters because it tells you how to reason about ignoring coordinate system dependence. gradient descent in natural parameters $\theta$ is not the same as gradient descent in expectation parameters $\mu$. the natural gradient is the correction that makes descent coordinate-invariant.
- Pitfall: Don't answer "Ignoring coordinate system dependence. Gradient descent in natural parameters $\theta$ is not the same as gradient descent in expectation parameters $\mu$. The natural gradient is the correction that makes descent coordinate-invariant" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: the same distributional update regardless of how you parameterize the model.

### "SAM finds the globally flattest minimum." SAM minimizes worst-case perturbed loss within a local $\ell_2$ ball of radius $\rho$. It finds a locally flat minimum
- Direct Answer: stable under perturbations of radius $\rho$, not necessarily the flattest minimum globally. The choice of $\rho$ matters.
- Why: This matters because it tells you how to reason about "sam finds the globally flattest minimum." sam minimizes worst-case perturbed loss within a local $\ell_2$ ball of radius $\rho$. it finds a locally flat minimum.
- Pitfall: Don't answer ""SAM finds the globally flattest minimum." SAM minimizes worst-case perturbed loss within a local $\ell_2$ ball of radius $\rho$. It finds a locally flat minimum" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: stable under perturbations of radius $\rho$, not necessarily the flattest minimum globally. The choice of $\rho$ matters.

### Forgetting sharpness is not reparameterization-invariant without ASAM. Dinh et al. proved you can always reparameterize to make sharpness appear 0. Standard SAM's $\ell_2$-ball sharpness measure is coordinate-dependent. ASAM's adaptive norm is scale-invariant
- Direct Answer: a more principled definition.
- Why: This matters because it tells you how to reason about forgetting sharpness is not reparameterization-invariant without asam. dinh et al. proved you can always reparameterize to make sharpness appear 0. standard sam's $\ell_2$-ball sharpness measure is coordinate-dependent. asam's adaptive norm is scale-invariant.
- Pitfall: Don't answer "Forgetting sharpness is not reparameterization-invariant without ASAM. Dinh et al. proved you can always reparameterize to make sharpness appear 0. Standard SAM's $\ell_2$-ball sharpness measure is coordinate-dependent. ASAM's adaptive norm is scale-invariant" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: a more principled definition.

### Ignoring the 2× compute overhead. SAM requires two forward-backward passes per update. For large models, this is significant. In practice, apply SAM to a subset of steps (every $m$-th step) or use lookahead SAM variants to reduce overhead.
- Direct Answer: Ignoring the 2× compute overhead. SAM requires two forward-backward passes per update. For large models, this is significant. In practice, apply SAM to a subset of steps (every $m$-th step) or use lookahead SAM variants to reduce overhead.
- Why: This matters because it tells you how to reason about ignoring the 2× compute overhead. sam requires two forward-backward passes per update. for large models, this is significant. in practice, apply sam to a subset of steps (every $m$-th step) or use lookahead sam variants to reduce overhead..
- Pitfall: Don't answer "Ignoring the 2× compute overhead. SAM requires two forward-backward passes per update. For large models, this is significant. In practice, apply SAM to a subset of steps (every $m$-th step) or use lookahead SAM variants to reduce overhead." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Ignoring the 2× compute overhead. SAM requires two forward-backward passes per update. For large models, this is significant. In practice, apply SAM to a subset of steps (every $m…

### Using the same absolute warmup step count regardless of total training steps. Warmup should be ~5–10% of total steps. A model trained for 1000 steps total with 1000 warmup steps never leaves the warmup phase.
- Direct Answer: Using the same absolute warmup step count regardless of total training steps. Warmup should be ~5–10% of total steps. A model trained for 1000 steps total with 1000 warmup steps never leaves the warmup phase.
- Why: This matters because it tells you how to reason about using the same absolute warmup step count regardless of total training steps. warmup should be ~5–10% of total steps. a model trained for 1000 steps total with 1000 warmup steps never leaves the warmup phase..
- Pitfall: Don't answer "Using the same absolute warmup step count regardless of total training steps. Warmup should be ~5–10% of total steps. A model trained for 1000 steps total with 1000 warmup steps never leaves the warmup phase." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Using the same absolute warmup step count regardless of total training steps. Warmup should be ~5–10% of total steps. A model trained for 1000 steps total with 1000 warmup steps n…

### Applying warmup to SGD for the same reason as Adam. Warmup for Adam is specifically about second moment stabilization. For SGD, the rationale is weaker
- Direct Answer: you might still use warmup to avoid catastrophically large early updates from random initialization, but the mechanism differs.
- Why: This matters because it tells you how to reason about applying warmup to sgd for the same reason as adam. warmup for adam is specifically about second moment stabilization. for sgd, the rationale is weaker.
- Pitfall: Don't answer "Applying warmup to SGD for the same reason as Adam. Warmup for Adam is specifically about second moment stabilization. For SGD, the rationale is weaker" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: you might still use warmup to avoid catastrophically large early updates from random initialization, but the mechanism differs.

### Treating cosine annealing as universally superior to step decay. Step decay is more interpretable and allows deliberate LR drops at known training milestones (e.g., after curriculum phase transitions). For problems with distinct training phases, step decay's explicit transitions can be better.
- Direct Answer: Treating cosine annealing as universally superior to step decay. Step decay is more interpretable and allows deliberate LR drops at known training milestones (e.g., after curriculum phase transitions). For problems with distinct training phases, step decay's explicit transitions can be better.
- Why: This matters because it tells you how to reason about treating cosine annealing as universally superior to step decay. step decay is more interpretable and allows deliberate lr drops at known training milestones (e.g., after curriculum phase transitions). for problems with distinct training phases, step decay's explicit transitions can be better..
- Pitfall: Don't answer "Treating cosine annealing as universally superior to step decay. Step decay is more interpretable and allows deliberate LR drops at known training milestones (e.g., after curriculum phase transitions). For problems with distinct training phases, step decay's explicit transitions can be better." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Treating cosine annealing as universally superior to step decay. Step decay is more interpretable and allows deliberate LR drops at known training milestones (e.g., after curricul…

### "Gradient clipping fixes exploding gradients." Clipping suppresses the symptom
- Direct Answer: it prevents large updates. It does not fix the underlying cause. For RNNs, the cause is eigenvalues of the recurrent weight matrix exceeding 1. This persists regardless of clipping. For a structural fix, use gated architectures (LSTM, GRU) or attention.
- Why: This matters because it tells you how to reason about "gradient clipping fixes exploding gradients." clipping suppresses the symptom.
- Pitfall: Don't answer ""Gradient clipping fixes exploding gradients." Clipping suppresses the symptom" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: it prevents large updates. It does not fix the underlying cause. For RNNs, the cause is eigenvalues of the recurrent weight matrix exceeding 1. This persists regardless of clippin…

### "By-value and by-norm clipping are equivalent for large enough $c$." They converge only when a single component completely dominates. During an instability event, typically many components are large simultaneously
- Direct Answer: by-value clipping changes the direction substantially, while by-norm clipping preserves it.
- Why: This matters because it tells you how to reason about "by-value and by-norm clipping are equivalent for large enough $c$." they converge only when a single component completely dominates. during an instability event, typically many components are large simultaneously.
- Pitfall: Don't answer ""By-value and by-norm clipping are equivalent for large enough $c$." They converge only when a single component completely dominates. During an instability event, typically many components are large simultaneously" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: by-value clipping changes the direction substantially, while by-norm clipping preserves it.

### Treating clipping as a solution rather than a diagnostic tool. If clipping activates on more than 10% of training steps, the learning rate is too high or the loss has conditioning issues. Investigate the root cause; don't just increase max_norm.
- Direct Answer: Treating clipping as a solution rather than a diagnostic tool. If clipping activates on more than 10% of training steps, the learning rate is too high or the loss has conditioning issues. Investigate the root cause; don't just increase max_norm.
- Why: This matters because it tells you how to reason about treating clipping as a solution rather than a diagnostic tool. if clipping activates on more than 10% of training steps, the learning rate is too high or the loss has conditioning issues. investigate the root cause; don't just increase max_norm..
- Pitfall: Don't answer "Treating clipping as a solution rather than a diagnostic tool. If clipping activates on more than 10% of training steps, the learning rate is too high or the loss has conditioning issues. Investigate the root cause; don't just increase max_norm." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Treating clipping as a solution rather than a diagnostic tool. If clipping activates on more than 10% of training steps, the learning rate is too high or the loss has conditioning…

### "NTK theory explains real network training." NTK is an infinite-width, infinitesimal-learning-rate idealization. Practical networks have finite width and train with large learning rates in the feature-learning regime where representations change and the NTK evolves. NTK is a useful analytical tool, not a description of GPT training dynamics.
- Direct Answer: "NTK theory explains real network training." NTK is an infinite-width, infinitesimal-learning-rate idealization. Practical networks have finite width and train with large learning rates in the feature-learning regime where representations change and the NTK evolves. NTK is a useful analytical tool, not a description of GPT training dynamics.
- Why: This matters because it tells you how to reason about "ntk theory explains real network training." ntk is an infinite-width, infinitesimal-learning-rate idealization. practical networks have finite width and train with large learning rates in the feature-learning regime where representations change and the ntk evolves. ntk is a useful analytical tool, not a description of gpt training dynamics..
- Pitfall: Don't answer ""NTK theory explains real network training." NTK is an infinite-width, infinitesimal-learning-rate idealization. Practical networks have finite width and train with large learning rates in the feature-learning regime where representations change and the NTK evolves. NTK is a useful analytical tool, not a description of GPT training dynamics." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: "NTK theory explains real network training." NTK is an infinite-width, infinitesimal-learning-rate idealization. Practical networks have finite width and train with large learning…

### "Sharp minima always generalize worse than flat minima." The relationship is well-established for fixed parameterizations. But Dinh et al. (2017) proved you can reparameterize to make any sharp minimum appear flat and vice versa
- Direct Answer: sharpness as measured by $\lambda_{\max}(H)$ is coordinate-dependent. When discussing flat minima, you must specify the norm or coordinate system relative to which sharpness is defined. ASAM addresses this with scale-invariant sharpness.
- Why: This matters because it tells you how to reason about "sharp minima always generalize worse than flat minima." the relationship is well-established for fixed parameterizations. but dinh et al. (2017) proved you can reparameterize to make any sharp minimum appear flat and vice versa.
- Pitfall: Don't answer ""Sharp minima always generalize worse than flat minima." The relationship is well-established for fixed parameterizations. But Dinh et al. (2017) proved you can reparameterize to make any sharp minimum appear flat and vice versa" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: sharpness as measured by $\lambda_{\max}(H)$ is coordinate-dependent. When discussing flat minima, you must specify the norm or coordinate system relative to which sharpness is de…

### "Mode connectivity means all local minima have the same loss." Mode connectivity says you can connect two independently trained networks by a path along which loss is approximately low. It does not say the networks are identical or that all paths between minima are loss-preserving. Direct interpolation without permutation alignment can still fail.
- Direct Answer: "Mode connectivity means all local minima have the same loss." Mode connectivity says you can connect two independently trained networks by a path along which loss is approximately low. It does not say the networks are identical or that all paths between minima are loss-preserving. Direct interpolation without permutation alignment can still fail.
- Why: This matters because it tells you how to reason about "mode connectivity means all local minima have the same loss." mode connectivity says you can connect two independently trained networks by a path along which loss is approximately low. it does not say the networks are identical or that all paths between minima are loss-preserving. direct interpolation without permutation alignment can still fail..
- Pitfall: Don't answer ""Mode connectivity means all local minima have the same loss." Mode connectivity says you can connect two independently trained networks by a path along which loss is approximately low. It does not say the networks are identical or that all paths between minima are loss-preserving. Direct interpolation without permutation alignment can still fail." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: "Mode connectivity means all local minima have the same loss." Mode connectivity says you can connect two independently trained networks by a path along which loss is approximatel…
