...
### Reporting training loss as model quality. The only interesting question is held-out performance. Training loss answers a different question entirely
- Direct Answer: "did the optimization converge?"
- Why: This matters because it tells you how to reason about reporting training loss as model quality. the only interesting question is held-out performance. training loss answers a different question entirely.
- Pitfall: Don't answer "Reporting training loss as model quality. The only interesting question is held-out performance. Training loss answers a different question entirely" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: "did the optimization converge?"

### Evaluating against the test set repeatedly during development. Each time you use the test set to inform a modeling decision, you are implicitly using it as a validation set. After 20 evaluation-adjust cycles, you have effectively trained on the test set through your choices. The final reported number is optimistic.
- Direct Answer: Evaluating against the test set repeatedly during development. Each time you use the test set to inform a modeling decision, you are implicitly using it as a validation set. After 20 evaluation-adjust cycles, you have effectively trained on the test set through your choices. The final reported number is optimistic.
- Why: This matters because it tells you how to reason about evaluating against the test set repeatedly during development. each time you use the test set to inform a modeling decision, you are implicitly using it as a validation set. after 20 evaluation-adjust cycles, you have effectively trained on the test set through your choices. the final reported number is optimistic..
- Pitfall: Don't answer "Evaluating against the test set repeatedly during development. Each time you use the test set to inform a modeling decision, you are implicitly using it as a validation set. After 20 evaluation-adjust cycles, you have effectively trained on the test set through your choices. The final reported number is optimistic." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Evaluating against the test set repeatedly during development. Each time you use the test set to inform a modeling decision, you are implicitly using it as a validation set. After…

### Evaluating a model only on aggregate metrics without investigating which examples it gets wrong. Understanding the failure pattern is more actionable than knowing the average failure rate. A model that fails on a specific demographic, or on a specific data type, needs targeted fixes, not more general regularization.
- Direct Answer: Evaluating a model only on aggregate metrics without investigating which examples it gets wrong. Understanding the failure pattern is more actionable than knowing the average failure rate. A model that fails on a specific demographic, or on a specific data type, needs targeted fixes, not more general regularization.
- Why: This matters because it tells you how to reason about evaluating a model only on aggregate metrics without investigating which examples it gets wrong. understanding the failure pattern is more actionable than knowing the average failure rate. a model that fails on a specific demographic, or on a specific data type, needs targeted fixes, not more general regularization..
- Pitfall: Don't answer "Evaluating a model only on aggregate metrics without investigating which examples it gets wrong. Understanding the failure pattern is more actionable than knowing the average failure rate. A model that fails on a specific demographic, or on a specific data type, needs targeted fixes, not more general regularization." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Evaluating a model only on aggregate metrics without investigating which examples it gets wrong. Understanding the failure pattern is more actionable than knowing the average fail…

### Defaulting to F1 without justifying that precision and recall have equal cost. They rarely do. Use $F_\beta$ with a $\beta$ derived from the actual cost ratio.
- Direct Answer: Defaulting to F1 without justifying that precision and recall have equal cost. They rarely do. Use $F_\beta$ with a $\beta$ derived from the actual cost ratio.
- Why: This matters because it tells you how to reason about defaulting to f1 without justifying that precision and recall have equal cost. they rarely do. use $f_\beta$ with a $\beta$ derived from the actual cost ratio..
- Pitfall: Don't answer "Defaulting to F1 without justifying that precision and recall have equal cost. They rarely do. Use $F_\beta$ with a $\beta$ derived from the actual cost ratio." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Defaulting to F1 without justifying that precision and recall have equal cost. They rarely do. Use $F_\beta$ with a $\beta$ derived from the actual cost ratio.

### Reporting precision or recall in isolation. They are trivially gameable in opposite directions. A model that predicts nothing positive achieves perfect precision (undefined or 1.0 depending on convention) and 0% recall. A model that predicts everything positive achieves 100% recall and precision equal to the base rate. Always report both together.
- Direct Answer: Reporting precision or recall in isolation. They are trivially gameable in opposite directions. A model that predicts nothing positive achieves perfect precision (undefined or 1.0 depending on convention) and 0% recall. A model that predicts everything positive achieves 100% recall and precision equal to the base rate. Always report both together.
- Why: This matters because it tells you how to reason about reporting precision or recall in isolation. they are trivially gameable in opposite directions. a model that predicts nothing positive achieves perfect precision (undefined or 1.0 depending on convention) and 0% recall. a model that predicts everything positive achieves 100% recall and precision equal to the base rate. always report both together..
- Pitfall: Don't answer "Reporting precision or recall in isolation. They are trivially gameable in opposite directions. A model that predicts nothing positive achieves perfect precision (undefined or 1.0 depending on convention) and 0% recall. A model that predicts everything positive achieves 100% recall and precision equal to the base rate. Always report both together." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Reporting precision or recall in isolation. They are trivially gameable in opposite directions. A model that predicts nothing positive achieves perfect precision (undefined or 1.0…

### Using ROC-AUC as the primary metric for fraud detection, anomaly detection, medical diagnosis, or any other setting where the positive class is rare. In these settings, the denominator of FPR (number of true negatives) is so large that even many false positives contribute negligible FPR. Use PR-AUC.
- Direct Answer: Using ROC-AUC as the primary metric for fraud detection, anomaly detection, medical diagnosis, or any other setting where the positive class is rare. In these settings, the denominator of FPR (number of true negatives) is so large that even many false positives contribute negligible FPR. Use PR-AUC.
- Why: This matters because it tells you how to reason about using roc-auc as the primary metric for fraud detection, anomaly detection, medical diagnosis, or any other setting where the positive class is rare. in these settings, the denominator of fpr (number of true negatives) is so large that even many false positives contribute negligible fpr. use pr-auc..
- Pitfall: Don't answer "Using ROC-AUC as the primary metric for fraud detection, anomaly detection, medical diagnosis, or any other setting where the positive class is rare. In these settings, the denominator of FPR (number of true negatives) is so large that even many false positives contribute negligible FPR. Use PR-AUC." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Using ROC-AUC as the primary metric for fraud detection, anomaly detection, medical diagnosis, or any other setting where the positive class is rare. In these settings, the denomi…

### Not plotting the full curve. A single AUC number hides the shape of the tradeoff. A model might have high PR-AUC because it achieves very high precision at low recall, or very high recall at low precision. These imply completely different operating strategies. The curve reveals which.
- Direct Answer: Not plotting the full curve. A single AUC number hides the shape of the tradeoff. A model might have high PR-AUC because it achieves very high precision at low recall, or very high recall at low precision. These imply completely different operating strategies. The curve reveals which.
- Why: This matters because it tells you how to reason about not plotting the full curve. a single auc number hides the shape of the tradeoff. a model might have high pr-auc because it achieves very high precision at low recall, or very high recall at low precision. these imply completely different operating strategies. the curve reveals which..
- Pitfall: Don't answer "Not plotting the full curve. A single AUC number hides the shape of the tradeoff. A model might have high PR-AUC because it achieves very high precision at low recall, or very high recall at low precision. These imply completely different operating strategies. The curve reveals which." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Not plotting the full curve. A single AUC number hides the shape of the tradeoff. A model might have high PR-AUC because it achieves very high precision at low recall, or very hig…

### Treating a well-ranked model as automatically well-calibrated. AUC and calibration are orthogonal. Many practitioners skip calibration assessment entirely because they only look at AUC.
- Direct Answer: Treating a well-ranked model as automatically well-calibrated. AUC and calibration are orthogonal. Many practitioners skip calibration assessment entirely because they only look at AUC.
- Why: This matters because it tells you how to reason about treating a well-ranked model as automatically well-calibrated. auc and calibration are orthogonal. many practitioners skip calibration assessment entirely because they only look at auc..
- Pitfall: Don't answer "Treating a well-ranked model as automatically well-calibrated. AUC and calibration are orthogonal. Many practitioners skip calibration assessment entirely because they only look at AUC." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Treating a well-ranked model as automatically well-calibrated. AUC and calibration are orthogonal. Many practitioners skip calibration assessment entirely because they only look a…

### Applying calibration on the training set. Calibration must be fit on a held-out validation set (or via cross-validation), otherwise the calibration curve is optimistic and the model is overfit to training data probabilities.
- Direct Answer: Applying calibration on the training set. Calibration must be fit on a held-out validation set (or via cross-validation), otherwise the calibration curve is optimistic and the model is overfit to training data probabilities.
- Why: This matters because it tells you how to reason about applying calibration on the training set. calibration must be fit on a held-out validation set (or via cross-validation), otherwise the calibration curve is optimistic and the model is overfit to training data probabilities..
- Pitfall: Don't answer "Applying calibration on the training set. Calibration must be fit on a held-out validation set (or via cross-validation), otherwise the calibration curve is optimistic and the model is overfit to training data probabilities." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Applying calibration on the training set. Calibration must be fit on a held-out validation set (or via cross-validation), otherwise the calibration curve is optimistic and the mod…

### Using RMSE when the target has outliers you don't want to dominate the metric. Outliers squared in RMSE can be 10,000x more influential than normal errors. If those outliers are rare, unusual events where the model's accuracy is less critical, use MAE or Huber loss (which transitions from quadratic to linear beyond a threshold).
- Direct Answer: Using RMSE when the target has outliers you don't want to dominate the metric. Outliers squared in RMSE can be 10,000x more influential than normal errors. If those outliers are rare, unusual events where the model's accuracy is less critical, use MAE or Huber loss (which transitions from quadratic to linear beyond a threshold).
- Why: This matters because it tells you how to reason about using rmse when the target has outliers you don't want to dominate the metric. outliers squared in rmse can be 10,000x more influential than normal errors. if those outliers are rare, unusual events where the model's accuracy is less critical, use mae or huber loss (which transitions from quadratic to linear beyond a threshold)..
- Pitfall: Don't answer "Using RMSE when the target has outliers you don't want to dominate the metric. Outliers squared in RMSE can be 10,000x more influential than normal errors. If those outliers are rare, unusual events where the model's accuracy is less critical, use MAE or Huber loss (which transitions from quadratic to linear beyond a threshold)." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Using RMSE when the target has outliers you don't want to dominate the metric. Outliers squared in RMSE can be 10,000x more influential than normal errors. If those outliers are r…

### Interpreting $R^2$ as a percentage of "variance explained" without checking whether predictions fall within the data distribution. A model that always predicts a value slightly outside the range of $y$ can produce negative $R^2$, which is surprising until you understand the formula.
- Direct Answer: Interpreting $R^2$ as a percentage of "variance explained" without checking whether predictions fall within the data distribution. A model that always predicts a value slightly outside the range of $y$ can produce negative $R^2$, which is surprising until you understand the formula.
- Why: This matters because it tells you how to reason about interpreting $r^2$ as a percentage of "variance explained" without checking whether predictions fall within the data distribution. a model that always predicts a value slightly outside the range of $y$ can produce negative $r^2$, which is surprising until you understand the formula..
- Pitfall: Don't answer "Interpreting $R^2$ as a percentage of "variance explained" without checking whether predictions fall within the data distribution. A model that always predicts a value slightly outside the range of $y$ can produce negative $R^2$, which is surprising until you understand the formula." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Interpreting $R^2$ as a percentage of "variance explained" without checking whether predictions fall within the data distribution. A model that always predicts a value slightly ou…

### Comparing log loss across models trained on different problems. Log loss is not scale-free. A binary classification problem with 50/50 class balance has a different log loss scale than one with 99/1 balance. Log loss comparisons are only valid within the same problem.
- Direct Answer: Comparing log loss across models trained on different problems. Log loss is not scale-free. A binary classification problem with 50/50 class balance has a different log loss scale than one with 99/1 balance. Log loss comparisons are only valid within the same problem.
- Why: This matters because it tells you how to reason about comparing log loss across models trained on different problems. log loss is not scale-free. a binary classification problem with 50/50 class balance has a different log loss scale than one with 99/1 balance. log loss comparisons are only valid within the same problem..
- Pitfall: Don't answer "Comparing log loss across models trained on different problems. Log loss is not scale-free. A binary classification problem with 50/50 class balance has a different log loss scale than one with 99/1 balance. Log loss comparisons are only valid within the same problem." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Comparing log loss across models trained on different problems. Log loss is not scale-free. A binary classification problem with 50/50 class balance has a different log loss scale…

### Not using log loss when the model outputs probabilities that feed downstream decisions. If the model just ranks examples and a human picks the top K, accuracy or AUC is sufficient. If the probabilities are consumed numerically in a pricing formula or risk calculation, log loss or Brier score are the right metrics.
- Direct Answer: Not using log loss when the model outputs probabilities that feed downstream decisions. If the model just ranks examples and a human picks the top K, accuracy or AUC is sufficient. If the probabilities are consumed numerically in a pricing formula or risk calculation, log loss or Brier score are the right metrics.
- Why: This matters because it tells you how to reason about not using log loss when the model outputs probabilities that feed downstream decisions. if the model just ranks examples and a human picks the top k, accuracy or auc is sufficient. if the probabilities are consumed numerically in a pricing formula or risk calculation, log loss or brier score are the right metrics..
- Pitfall: Don't answer "Not using log loss when the model outputs probabilities that feed downstream decisions. If the model just ranks examples and a human picks the top K, accuracy or AUC is sufficient. If the probabilities are consumed numerically in a pricing formula or risk calculation, log loss or Brier score are the right metrics." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Not using log loss when the model outputs probabilities that feed downstream decisions. If the model just ranks examples and a human picks the top K, accuracy or AUC is sufficient…

### Preprocessing outside the cross-validation loop. This is the most common CV mistake in practice. The scaler or imputer fitted on all data embeds the validation fold's statistics into the training representation.
- Direct Answer: Preprocessing outside the cross-validation loop. This is the most common CV mistake in practice. The scaler or imputer fitted on all data embeds the validation fold's statistics into the training representation.
- Why: This matters because it tells you how to reason about preprocessing outside the cross-validation loop. this is the most common cv mistake in practice. the scaler or imputer fitted on all data embeds the validation fold's statistics into the training representation..
- Pitfall: Don't answer "Preprocessing outside the cross-validation loop. This is the most common CV mistake in practice. The scaler or imputer fitted on all data embeds the validation fold's statistics into the training representation." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Preprocessing outside the cross-validation loop. This is the most common CV mistake in practice. The scaler or imputer fitted on all data embeds the validation fold's statistics i…

### Using K-fold CV on time series data. Any shuffle of temporal data allows future information to appear in training. Time-based splits are mandatory.
- Direct Answer: Using K-fold CV on time series data. Any shuffle of temporal data allows future information to appear in training. Time-based splits are mandatory.
- Why: This matters because it tells you how to reason about using k-fold cv on time series data. any shuffle of temporal data allows future information to appear in training. time-based splits are mandatory..
- Pitfall: Don't answer "Using K-fold CV on time series data. Any shuffle of temporal data allows future information to appear in training. Time-based splits are mandatory." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Using K-fold CV on time series data. Any shuffle of temporal data allows future information to appear in training. Time-based splits are mandatory.

### Using 0.5 as the default threshold without checking whether it makes sense for the class distribution and cost structure. For any imbalanced problem (class rate < 30%), 0.5 is almost certainly too high
- Direct Answer: it under-predicts the rare class relative to the actual optimal threshold.
- Why: This matters because it tells you how to reason about using 0.5 as the default threshold without checking whether it makes sense for the class distribution and cost structure. for any imbalanced problem (class rate < 30%), 0.5 is almost certainly too high.
- Pitfall: Don't answer "Using 0.5 as the default threshold without checking whether it makes sense for the class distribution and cost structure. For any imbalanced problem (class rate < 30%), 0.5 is almost certainly too high" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: it under-predicts the rare class relative to the actual optimal threshold.

### Tuning the threshold on the test set. The threshold is a hyperparameter. Selecting it based on test set performance inflates the test metric. Choose the threshold on the validation set or through cross-validation.
- Direct Answer: Tuning the threshold on the test set. The threshold is a hyperparameter. Selecting it based on test set performance inflates the test metric. Choose the threshold on the validation set or through cross-validation.
- Why: This matters because it tells you how to reason about tuning the threshold on the test set. the threshold is a hyperparameter. selecting it based on test set performance inflates the test metric. choose the threshold on the validation set or through cross-validation..
- Pitfall: Don't answer "Tuning the threshold on the test set. The threshold is a hyperparameter. Selecting it based on test set performance inflates the test metric. Choose the threshold on the validation set or through cross-validation." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Tuning the threshold on the test set. The threshold is a hyperparameter. Selecting it based on test set performance inflates the test metric. Choose the threshold on the validatio…

### Shipping based on offline evaluation alone. Even a large, clean offline improvement can hurt production metrics due to the mechanisms above. Shadow mode costs almost nothing and catches distribution anomalies before they reach users.
- Direct Answer: Shipping based on offline evaluation alone. Even a large, clean offline improvement can hurt production metrics due to the mechanisms above. Shadow mode costs almost nothing and catches distribution anomalies before they reach users.
- Why: This matters because it tells you how to reason about shipping based on offline evaluation alone. even a large, clean offline improvement can hurt production metrics due to the mechanisms above. shadow mode costs almost nothing and catches distribution anomalies before they reach users..
- Pitfall: Don't answer "Shipping based on offline evaluation alone. Even a large, clean offline improvement can hurt production metrics due to the mechanisms above. Shadow mode costs almost nothing and catches distribution anomalies before they reach users." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Shipping based on offline evaluation alone. Even a large, clean offline improvement can hurt production metrics due to the mechanisms above. Shadow mode costs almost nothing and c…

### Not defining guardrail metrics before deployment. "We will watch for negative effects" is not a plan. Before deployment, define
- Direct Answer: which metrics are guardrails (must not degrade by more than X%), what the rollback trigger is, and who has authority to roll back. This turns a vague monitoring intention into an operational procedure.
- Why: This matters because it tells you how to reason about not defining guardrail metrics before deployment. "we will watch for negative effects" is not a plan. before deployment, define.
- Pitfall: Don't answer "Not defining guardrail metrics before deployment. "We will watch for negative effects" is not a plan. Before deployment, define" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: which metrics are guardrails (must not degrade by more than X%), what the rollback trigger is, and who has authority to roll back. This turns a vague monitoring intention into an…

### Using AUC for ranking problems. AUC is a threshold-independent metric measuring the probability that a random positive ranks above a random negative. It does not account for position within the list. A model with AUC = 0.95 might still place the most relevant item at the bottom of the top-10 list.
- Direct Answer: Using AUC for ranking problems. AUC is a threshold-independent metric measuring the probability that a random positive ranks above a random negative. It does not account for position within the list. A model with AUC = 0.95 might still place the most relevant item at the bottom of the top-10 list.
- Why: This matters because it tells you how to reason about using auc for ranking problems. auc is a threshold-independent metric measuring the probability that a random positive ranks above a random negative. it does not account for position within the list. a model with auc = 0.95 might still place the most relevant item at the bottom of the top-10 list..
- Pitfall: Don't answer "Using AUC for ranking problems. AUC is a threshold-independent metric measuring the probability that a random positive ranks above a random negative. It does not account for position within the list. A model with AUC = 0.95 might still place the most relevant item at the bottom of the top-10 list." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Using AUC for ranking problems. AUC is a threshold-independent metric measuring the probability that a random positive ranks above a random negative. It does not account for posit…

### Not accounting for diversity alongside relevance. A list of 10 nearly identical items can score perfectly on any relevance metric while providing low value to users who want variety. Diversity metrics (coverage, intra-list diversity, novelty) need to be measured separately.
- Direct Answer: Not accounting for diversity alongside relevance. A list of 10 nearly identical items can score perfectly on any relevance metric while providing low value to users who want variety. Diversity metrics (coverage, intra-list diversity, novelty) need to be measured separately.
- Why: This matters because it tells you how to reason about not accounting for diversity alongside relevance. a list of 10 nearly identical items can score perfectly on any relevance metric while providing low value to users who want variety. diversity metrics (coverage, intra-list diversity, novelty) need to be measured separately..
- Pitfall: Don't answer "Not accounting for diversity alongside relevance. A list of 10 nearly identical items can score perfectly on any relevance metric while providing low value to users who want variety. Diversity metrics (coverage, intra-list diversity, novelty) need to be measured separately." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Not accounting for diversity alongside relevance. A list of 10 nearly identical items can score perfectly on any relevance metric while providing low value to users who want varie…

### Stopping the experiment early when a significant positive result appears. This is peeking. The false positive rate for a test that is evaluated continuously and stopped whenever p < 0.05 is far above 5%
- Direct Answer: it can reach 20–30%. Fix by committing to a fixed duration and sample size before the experiment starts, or by using sequential testing (alpha spending) methods.
- Why: This matters because it tells you how to reason about stopping the experiment early when a significant positive result appears. this is peeking. the false positive rate for a test that is evaluated continuously and stopped whenever p < 0.05 is far above 5%.
- Pitfall: Don't answer "Stopping the experiment early when a significant positive result appears. This is peeking. The false positive rate for a test that is evaluated continuously and stopped whenever p < 0.05 is far above 5%" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: it can reach 20–30%. Fix by committing to a fixed duration and sample size before the experiment starts, or by using sequential testing (alpha spending) methods.

### Running multiple simultaneous A/B tests without interaction analysis. Two concurrent tests with independent 2% improvements may not combine to 4% if the user populations overlap and the effects interact. Test for interaction effects or ensure tests run on independent user segments.
- Direct Answer: Running multiple simultaneous A/B tests without interaction analysis. Two concurrent tests with independent 2% improvements may not combine to 4% if the user populations overlap and the effects interact. Test for interaction effects or ensure tests run on independent user segments.
- Why: This matters because it tells you how to reason about running multiple simultaneous a/b tests without interaction analysis. two concurrent tests with independent 2% improvements may not combine to 4% if the user populations overlap and the effects interact. test for interaction effects or ensure tests run on independent user segments..
- Pitfall: Don't answer "Running multiple simultaneous A/B tests without interaction analysis. Two concurrent tests with independent 2% improvements may not combine to 4% if the user populations overlap and the effects interact. Test for interaction effects or ensure tests run on independent user segments." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Running multiple simultaneous A/B tests without interaction analysis. Two concurrent tests with independent 2% improvements may not combine to 4% if the user populations overlap a…

### Network effects: in social or marketplace systems, assigning users randomly violates the Stable Unit Treatment Value Assumption (SUTVA)
- Direct Answer: users in the control group interact with treatment users and vice versa, contaminating both groups. Use cluster randomization (randomize by social cluster or geographic area) instead.
- Why: This matters because it tells you how to reason about network effects: in social or marketplace systems, assigning users randomly violates the stable unit treatment value assumption (sutva).
- Pitfall: Don't answer "Network effects: in social or marketplace systems, assigning users randomly violates the Stable Unit Treatment Value Assumption (SUTVA)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: users in the control group interact with treatment users and vice versa, contaminating both groups. Use cluster randomization (randomize by social cluster or geographic area) inst…
