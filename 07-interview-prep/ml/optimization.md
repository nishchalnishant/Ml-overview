...
### Conflating gradient descent (the update rule) with backpropagation (the gradient computation algorithm). Backpropagation computes $\nabla_\theta L$; gradient descent uses it. A model trained with automatic differentiation (PyTorch autograd) uses backprop for gradient computation, but could use any optimizer (SGD, Adam, L-BFGS) for the update step.
- Direct Answer: Conflating gradient descent (the update rule) with backpropagation (the gradient computation algorithm). Backpropagation computes $\nabla_\theta L$; gradient descent uses it. A model trained with automatic differentiation (PyTorch autograd) uses backprop for gradient computation, but could use any optimizer (SGD, Adam, L-BFGS) for the update step.
- Why: This matters because it tells you how to reason about conflating gradient descent (the update rule) with backpropagation (the gradient computation algorithm). backpropagation computes $\nabla_\theta l$; gradient descent uses it. a model trained with automatic differentiation (pytorch autograd) uses backprop for gradient computation, but could use any optimizer (sgd, adam, l-bfgs) for the update step..
- Pitfall: Don't answer "Conflating gradient descent (the update rule) with backpropagation (the gradient computation algorithm). Backpropagation computes $\nabla_\theta L$; gradient descent uses it. A model trained with automatic differentiation (PyTorch autograd) uses backprop for gradient computation, but could use any optimizer (SGD, Adam, L-BFGS) for the update step." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Conflating gradient descent (the update rule) with backpropagation (the gradient computation algorithm). Backpropagation computes $\nabla_\theta L$; gradient descent uses it. A mo…

### Treating gradient descent as an exact optimization algorithm. The update $\theta - \eta \nabla_\theta L$ exactly minimizes the linearized loss around $\theta$. But the actual loss is not linear, so the true minimum is not at $\theta - \eta \nabla_\theta L$ except infinitesimally. Each step is an approximation.
- Direct Answer: Treating gradient descent as an exact optimization algorithm. The update $\theta - \eta \nabla_\theta L$ exactly minimizes the linearized loss around $\theta$. But the actual loss is not linear, so the true minimum is not at $\theta - \eta \nabla_\theta L$ except infinitesimally. Each step is an approximation.
- Why: This matters because it tells you how to reason about treating gradient descent as an exact optimization algorithm. the update $\theta - \eta \nabla_\theta l$ exactly minimizes the linearized loss around $\theta$. but the actual loss is not linear, so the true minimum is not at $\theta - \eta \nabla_\theta l$ except infinitesimally. each step is an approximation..
- Pitfall: Don't answer "Treating gradient descent as an exact optimization algorithm. The update $\theta - \eta \nabla_\theta L$ exactly minimizes the linearized loss around $\theta$. But the actual loss is not linear, so the true minimum is not at $\theta - \eta \nabla_\theta L$ except infinitesimally. Each step is an approximation." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Treating gradient descent as an exact optimization algorithm. The update $\theta - \eta \nabla_\theta L$ exactly minimizes the linearized loss around $\theta$. But the actual loss…

### Assuming larger batches are strictly better because they estimate the gradient more accurately. Gradient accuracy and solution quality are not the same thing. The gradient noise in small-batch training is not a bug
- Direct Answer: it is an implicit regularizer.
- Why: This matters because it tells you how to reason about assuming larger batches are strictly better because they estimate the gradient more accurately. gradient accuracy and solution quality are not the same thing. the gradient noise in small-batch training is not a bug.
- Pitfall: Don't answer "Assuming larger batches are strictly better because they estimate the gradient more accurately. Gradient accuracy and solution quality are not the same thing. The gradient noise in small-batch training is not a bug" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: it is an implicit regularizer.

### Not scaling the learning rate when increasing batch size. The standard heuristic
- Direct Answer: multiply learning rate by the factor by which you scale the batch size. (With warmup: first warm up from the original LR to the scaled LR over ~5 epochs, then apply the scheduled LR.) Without this scaling, large-batch training undershoots the optimal LR for the landscape.
- Why: This matters because it tells you how to reason about not scaling the learning rate when increasing batch size. the standard heuristic.
- Pitfall: Don't answer "Not scaling the learning rate when increasing batch size. The standard heuristic" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: multiply learning rate by the factor by which you scale the batch size. (With warmup: first warm up from the original LR to the scaled LR over ~5 epochs, then apply the scheduled…

### Setting learning rate once and leaving it constant. Learning rate scheduling
- Direct Answer: warmup + cosine decay — typically delivers more benefit than any other single hyperparameter choice. The specific LR value matters less than the schedule shape.
- Why: This matters because it tells you how to reason about setting learning rate once and leaving it constant. learning rate scheduling.
- Pitfall: Don't answer "Setting learning rate once and leaving it constant. Learning rate scheduling" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: warmup + cosine decay — typically delivers more benefit than any other single hyperparameter choice. The specific LR value matters less than the schedule shape.

### Using the same learning rate for all layers when fine-tuning. The first few layers of a pretrained network have learned generic low-level representations; they need minimal updating. Later layers need more adaptation. Using a single LR either destroys the generic early-layer representations or fails to adapt the task-specific later layers.
- Direct Answer: Using the same learning rate for all layers when fine-tuning. The first few layers of a pretrained network have learned generic low-level representations; they need minimal updating. Later layers need more adaptation. Using a single LR either destroys the generic early-layer representations or fails to adapt the task-specific later layers.
- Why: This matters because it tells you how to reason about using the same learning rate for all layers when fine-tuning. the first few layers of a pretrained network have learned generic low-level representations; they need minimal updating. later layers need more adaptation. using a single lr either destroys the generic early-layer representations or fails to adapt the task-specific later layers..
- Pitfall: Don't answer "Using the same learning rate for all layers when fine-tuning. The first few layers of a pretrained network have learned generic low-level representations; they need minimal updating. Later layers need more adaptation. Using a single LR either destroys the generic early-layer representations or fails to adapt the task-specific later layers." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Using the same learning rate for all layers when fine-tuning. The first few layers of a pretrained network have learned generic low-level representations; they need minimal updati…

### Setting $\beta$ too high (e.g., 0.99 or higher). The effective smoothing window is $1/(1-\beta)$ steps. At $\beta = 0.99$, the window is 100 steps
- Direct Answer: the velocity accumulates so much history that it overshoots minima and causes instability. Standard is $\beta = 0.9$ for SGD + momentum.
- Why: This matters because it tells you how to reason about setting $\beta$ too high (e.g., 0.99 or higher). the effective smoothing window is $1/(1-\beta)$ steps. at $\beta = 0.99$, the window is 100 steps.
- Pitfall: Don't answer "Setting $\beta$ too high (e.g., 0.99 or higher). The effective smoothing window is $1/(1-\beta)$ steps. At $\beta = 0.99$, the window is 100 steps" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: the velocity accumulates so much history that it overshoots minima and causes instability. Standard is $\beta = 0.9$ for SGD + momentum.

### Not resetting the velocity when making large changes to the learning rate or task. The accumulated velocity encodes gradient information from the old learning rate or task. After a major LR reduction or a task switch in continual learning, reset momentum to zero.
- Direct Answer: Not resetting the velocity when making large changes to the learning rate or task. The accumulated velocity encodes gradient information from the old learning rate or task. After a major LR reduction or a task switch in continual learning, reset momentum to zero.
- Why: This matters because it tells you how to reason about not resetting the velocity when making large changes to the learning rate or task. the accumulated velocity encodes gradient information from the old learning rate or task. after a major lr reduction or a task switch in continual learning, reset momentum to zero..
- Pitfall: Don't answer "Not resetting the velocity when making large changes to the learning rate or task. The accumulated velocity encodes gradient information from the old learning rate or task. After a major LR reduction or a task switch in continual learning, reset momentum to zero." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Not resetting the velocity when making large changes to the learning rate or task. The accumulated velocity encodes gradient information from the old learning rate or task. After…

### Using torch.optim.Adam with weight decay for deep learning. Standard Adam applies weight decay incorrectly
- Direct Answer: the weight decay term is adaptively scaled, so the regularization is weaker for high-gradient parameters. Use AdamW (see section 6).
- Why: This matters because it tells you how to reason about using torch.optim.adam with weight decay for deep learning. standard adam applies weight decay incorrectly.
- Pitfall: Don't answer "Using torch.optim.Adam with weight decay for deep learning. Standard Adam applies weight decay incorrectly" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: the weight decay term is adaptively scaled, so the regularization is weaker for high-gradient parameters. Use AdamW (see section 6).

### Assuming Adam always converges faster than SGD. Adam converges in fewer steps for most problems and requires less hyperparameter tuning. But for vision models where SGD is carefully tuned (cosine schedule, right LR), SGD often achieves better final accuracy because its gradient noise produces flatter, more generalizable minima. Adam's adaptivity reduces noise and can cause convergence to sharper solutions.
- Direct Answer: Assuming Adam always converges faster than SGD. Adam converges in fewer steps for most problems and requires less hyperparameter tuning. But for vision models where SGD is carefully tuned (cosine schedule, right LR), SGD often achieves better final accuracy because its gradient noise produces flatter, more generalizable minima. Adam's adaptivity reduces noise and can cause convergence to sharper solutions.
- Why: This matters because it tells you how to reason about assuming adam always converges faster than sgd. adam converges in fewer steps for most problems and requires less hyperparameter tuning. but for vision models where sgd is carefully tuned (cosine schedule, right lr), sgd often achieves better final accuracy because its gradient noise produces flatter, more generalizable minima. adam's adaptivity reduces noise and can cause convergence to sharper solutions..
- Pitfall: Don't answer "Assuming Adam always converges faster than SGD. Adam converges in fewer steps for most problems and requires less hyperparameter tuning. But for vision models where SGD is carefully tuned (cosine schedule, right LR), SGD often achieves better final accuracy because its gradient noise produces flatter, more generalizable minima. Adam's adaptivity reduces noise and can cause convergence to sharper solutions." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Assuming Adam always converges faster than SGD. Adam converges in fewer steps for most problems and requires less hyperparameter tuning. But for vision models where SGD is careful…

### Using torch.optim.Adam for any modern deep learning task. AdamW should be the default. Standard Adam with weight_decay is misleading because it does not implement L2 regularization correctly.
- Direct Answer: Using torch.optim.Adam for any modern deep learning task. AdamW should be the default. Standard Adam with weight_decay is misleading because it does not implement L2 regularization correctly.
- Why: This matters because it tells you how to reason about using torch.optim.adam for any modern deep learning task. adamw should be the default. standard adam with weight_decay is misleading because it does not implement l2 regularization correctly..
- Pitfall: Don't answer "Using torch.optim.Adam for any modern deep learning task. AdamW should be the default. Standard Adam with weight_decay is misleading because it does not implement L2 regularization correctly." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Using torch.optim.Adam for any modern deep learning task. AdamW should be the default. Standard Adam with weight_decay is misleading because it does not implement L2 regularizatio…

### Setting weight_decay > 0.1. AdamW with weight_decay=0.3 combined with dropout is excessive regularization that hurts performance. Typical effective range
- Direct Answer: 0.01–0.1 for most tasks. Start at 0.01.
- Why: This matters because it tells you how to reason about setting weight_decay > 0.1. adamw with weight_decay=0.3 combined with dropout is excessive regularization that hurts performance. typical effective range.
- Pitfall: Don't answer "Setting weight_decay > 0.1. AdamW with weight_decay=0.3 combined with dropout is excessive regularization that hurts performance. Typical effective range" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: 0.01–0.1 for most tasks. Start at 0.01.

### Applying gradient clipping to a vanishing gradient problem. Clipping restricts gradient magnitude downward
- Direct Answer: it cannot increase magnitude. A vanishing gradient needs architectural solutions: residual connections, better initialization, normalization, non-saturating activations.
- Why: This matters because it tells you how to reason about applying gradient clipping to a vanishing gradient problem. clipping restricts gradient magnitude downward.
- Pitfall: Don't answer "Applying gradient clipping to a vanishing gradient problem. Clipping restricts gradient magnitude downward" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: it cannot increase magnitude. A vanishing gradient needs architectural solutions: residual connections, better initialization, normalization, non-saturating activations.

### Clipping by value instead of by norm. Clipping $g_1 = 1000$ and $g_2 = 0.001$ to $c = 1$ produces $(1, 0.001)$, which points almost entirely in the $g_1$ direction
- Direct Answer: the gradient direction has changed from (1, 0) to approximately (1, 0). Clipping by norm instead scales both components proportionally, preserving the direction exactly.
- Why: This matters because it tells you how to reason about clipping by value instead of by norm. clipping $g_1 = 1000$ and $g_2 = 0.001$ to $c = 1$ produces $(1, 0.001)$, which points almost entirely in the $g_1$ direction.
- Pitfall: Don't answer "Clipping by value instead of by norm. Clipping $g_1 = 1000$ and $g_2 = 0.001$ to $c = 1$ produces $(1, 0.001)$, which points almost entirely in the $g_1$ direction" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: the gradient direction has changed from (1, 0) to approximately (1, 0). Clipping by norm instead scales both components proportionally, preserving the direction exactly.

### Using the same warmup step count regardless of total training steps. Warmup should be approximately 5–10% of total training steps, not a fixed number. A model trained for 100K steps needs ~5K-10K warmup steps; a model trained for 10K steps needs 500–1K steps.
- Direct Answer: Using the same warmup step count regardless of total training steps. Warmup should be approximately 5–10% of total training steps, not a fixed number. A model trained for 100K steps needs ~5K-10K warmup steps; a model trained for 10K steps needs 500–1K steps.
- Why: This matters because it tells you how to reason about using the same warmup step count regardless of total training steps. warmup should be approximately 5–10% of total training steps, not a fixed number. a model trained for 100k steps needs ~5k-10k warmup steps; a model trained for 10k steps needs 500–1k steps..
- Pitfall: Don't answer "Using the same warmup step count regardless of total training steps. Warmup should be approximately 5–10% of total training steps, not a fixed number. A model trained for 100K steps needs ~5K-10K warmup steps; a model trained for 10K steps needs 500–1K steps." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Using the same warmup step count regardless of total training steps. Warmup should be approximately 5–10% of total training steps, not a fixed number. A model trained for 100K ste…

### Applying warmup to SGD for the same reason as Adam. SGD does not have the moment stabilization problem
- Direct Answer: it has no second moment estimate. Warmup for SGD has a different motivation (gradual adaptation from random initialization) and different optimal duration.
- Why: This matters because it tells you how to reason about applying warmup to sgd for the same reason as adam. sgd does not have the moment stabilization problem.
- Pitfall: Don't answer "Applying warmup to SGD for the same reason as Adam. SGD does not have the moment stabilization problem" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: it has no second moment estimate. Warmup for SGD has a different motivation (gradual adaptation from random initialization) and different optimal duration.

### Tuning hyperparameters against the test set. Hyperparameter search is model selection. Use a separate validation set or CV. Every hyperparameter selection based on test set performance inflates the reported final metric.
- Direct Answer: Tuning hyperparameters against the test set. Hyperparameter search is model selection. Use a separate validation set or CV. Every hyperparameter selection based on test set performance inflates the reported final metric.
- Why: This matters because it tells you how to reason about tuning hyperparameters against the test set. hyperparameter search is model selection. use a separate validation set or cv. every hyperparameter selection based on test set performance inflates the reported final metric..
- Pitfall: Don't answer "Tuning hyperparameters against the test set. Hyperparameter search is model selection. Use a separate validation set or CV. Every hyperparameter selection based on test set performance inflates the reported final metric." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Tuning hyperparameters against the test set. Hyperparameter search is model selection. Use a separate validation set or CV. Every hyperparameter selection based on test set perfor…

### Tuning hyperparameters one at a time. Learning rate and batch size interact (optimal LR scales with batch size). Model capacity and regularization interact (larger models need stronger regularization). Sequential single-hyperparameter tuning misses these interactions. Tune jointly.
- Direct Answer: Tuning hyperparameters one at a time. Learning rate and batch size interact (optimal LR scales with batch size). Model capacity and regularization interact (larger models need stronger regularization). Sequential single-hyperparameter tuning misses these interactions. Tune jointly.
- Why: This matters because it tells you how to reason about tuning hyperparameters one at a time. learning rate and batch size interact (optimal lr scales with batch size). model capacity and regularization interact (larger models need stronger regularization). sequential single-hyperparameter tuning misses these interactions. tune jointly..
- Pitfall: Don't answer "Tuning hyperparameters one at a time. Learning rate and batch size interact (optimal LR scales with batch size). Model capacity and regularization interact (larger models need stronger regularization). Sequential single-hyperparameter tuning misses these interactions. Tune jointly." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Tuning hyperparameters one at a time. Learning rate and batch size interact (optimal LR scales with batch size). Model capacity and regularization interact (larger models need str…

### Using Adagrad for deep learning. Adagrad accumulates squared gradients monotonically and never forgets
- Direct Answer: the effective learning rate decays toward zero over time, making long training runs impossible. For deep networks with long training, use RMSprop (which uses an exponential moving average of squared gradients) or Adam.
- Why: This matters because it tells you how to reason about using adagrad for deep learning. adagrad accumulates squared gradients monotonically and never forgets.
- Pitfall: Don't answer "Using Adagrad for deep learning. Adagrad accumulates squared gradients monotonically and never forgets" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: the effective learning rate decays toward zero over time, making long training runs impossible. For deep networks with long training, use RMSprop (which uses an exponential moving…

### Forgetting to switch from Adam to AdamW when using weight decay. If weight_decay > 0 in torch.optim.Adam, the regularization is incorrect. Always use torch.optim.AdamW when applying weight decay.
- Direct Answer: Forgetting to switch from Adam to AdamW when using weight decay. If weight_decay > 0 in torch.optim.Adam, the regularization is incorrect. Always use torch.optim.AdamW when applying weight decay.
- Why: This matters because it tells you how to reason about forgetting to switch from adam to adamw when using weight decay. if weight_decay > 0 in torch.optim.adam, the regularization is incorrect. always use torch.optim.adamw when applying weight decay..
- Pitfall: Don't answer "Forgetting to switch from Adam to AdamW when using weight decay. If weight_decay > 0 in torch.optim.Adam, the regularization is incorrect. Always use torch.optim.AdamW when applying weight decay." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Forgetting to switch from Adam to AdamW when using weight decay. If weight_decay > 0 in torch.optim.Adam, the regularization is incorrect. Always use torch.optim.AdamW when applyi…

### Changing multiple hyperparameters simultaneously when debugging. If you change LR, batch size, and architecture at once and performance improves, you do not know which change helped. Change one thing at a time. Keep a log.
- Direct Answer: Changing multiple hyperparameters simultaneously when debugging. If you change LR, batch size, and architecture at once and performance improves, you do not know which change helped. Change one thing at a time. Keep a log.
- Why: This matters because it tells you how to reason about changing multiple hyperparameters simultaneously when debugging. if you change lr, batch size, and architecture at once and performance improves, you do not know which change helped. change one thing at a time. keep a log..
- Pitfall: Don't answer "Changing multiple hyperparameters simultaneously when debugging. If you change LR, batch size, and architecture at once and performance improves, you do not know which change helped. Change one thing at a time. Keep a log." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Changing multiple hyperparameters simultaneously when debugging. If you change LR, batch size, and architecture at once and performance improves, you do not know which change help…

### Not logging gradient norms. Gradient norm monitoring is cheap (one extra line per training step) and catches vanishing/exploding gradient issues before they become catastrophic. Add it from the start.
- Direct Answer: Not logging gradient norms. Gradient norm monitoring is cheap (one extra line per training step) and catches vanishing/exploding gradient issues before they become catastrophic. Add it from the start.
- Why: This matters because it tells you how to reason about not logging gradient norms. gradient norm monitoring is cheap (one extra line per training step) and catches vanishing/exploding gradient issues before they become catastrophic. add it from the start..
- Pitfall: Don't answer "Not logging gradient norms. Gradient norm monitoring is cheap (one extra line per training step) and catches vanishing/exploding gradient issues before they become catastrophic. Add it from the start." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Not logging gradient norms. Gradient norm monitoring is cheap (one extra line per training step) and catches vanishing/exploding gradient issues before they become catastrophic. A…

### Quantizing without benchmarking on the production task. Accuracy degradation from quantization is task- and distribution-dependent. Aggregate benchmarks may show 0.5% degradation while the specific input distribution the model will serve shows 5% degradation. Always benchmark on representative production data.
- Direct Answer: Quantizing without benchmarking on the production task. Accuracy degradation from quantization is task- and distribution-dependent. Aggregate benchmarks may show 0.5% degradation while the specific input distribution the model will serve shows 5% degradation. Always benchmark on representative production data.
- Why: This matters because it tells you how to reason about quantizing without benchmarking on the production task. accuracy degradation from quantization is task- and distribution-dependent. aggregate benchmarks may show 0.5% degradation while the specific input distribution the model will serve shows 5% degradation. always benchmark on representative production data..
- Pitfall: Don't answer "Quantizing without benchmarking on the production task. Accuracy degradation from quantization is task- and distribution-dependent. Aggregate benchmarks may show 0.5% degradation while the specific input distribution the model will serve shows 5% degradation. Always benchmark on representative production data." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Quantizing without benchmarking on the production task. Accuracy degradation from quantization is task- and distribution-dependent. Aggregate benchmarks may show 0.5% degradation…

### Assuming FP16 is always safe without gradient scaling. Naively running training in FP16 without loss scaling causes underflow: gradients with magnitude < $\sim 6 \times 10^{-8}$ (FP16 minimum positive) are rounded to zero. Framework mixed-precision implementations (torch.cuda.amp.autocast) handle this automatically via dynamic loss scaling
- Direct Answer: but do not implement manual FP16 without understanding this.
- Why: This matters because it tells you how to reason about assuming fp16 is always safe without gradient scaling. naively running training in fp16 without loss scaling causes underflow: gradients with magnitude < $\sim 6 \times 10^{-8}$ (fp16 minimum positive) are rounded to zero. framework mixed-precision implementations (torch.cuda.amp.autocast) handle this automatically via dynamic loss scaling.
- Pitfall: Don't answer "Assuming FP16 is always safe without gradient scaling. Naively running training in FP16 without loss scaling causes underflow: gradients with magnitude < $\sim 6 \times 10^{-8}$ (FP16 minimum positive) are rounded to zero. Framework mixed-precision implementations (torch.cuda.amp.autocast) handle this automatically via dynamic loss scaling" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: but do not implement manual FP16 without understanding this.
