...
- Direct Answer: fast, approximate method that finds a small candidate set. Misses some good candidates, but fast. Optimized for recall.
- Why: This matters because it tells you how to reason about retrieval (recall-oriented).
- Pitfall: Don't answer "Retrieval (recall-oriented)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: fast, approximate method that finds a small candidate set. Misses some good candidates, but fast. Optimized for recall.

### Ranking (precision-oriented)
- Direct Answer: accurate, heavy model scores only retrieved candidates. Can use expensive features. Optimized for precision on the candidate set.
- Why: This matters because it tells you how to reason about ranking (precision-oriented).
- Pitfall: Don't answer "Ranking (precision-oriented)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: accurate, heavy model scores only retrieved candidates. Can use expensive features. Optimized for precision on the candidate set.

### Vector search / approximate nearest neighbors (FAISS, ScaNN)
- Direct Answer: precomputed embeddings, fast similarity search
- Why: This matters because it tells you how to reason about vector search / approximate nearest neighbors (faiss, scann).
- Pitfall: Don't answer "Vector search / approximate nearest neighbors (FAISS, ScaNN)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: precomputed embeddings, fast similarity search

### Collaborative filtering (item-item or user-item)
- Direct Answer: based on historical co-engagement
- Why: This matters because it tells you how to reason about collaborative filtering (item-item or user-item).
- Pitfall: Don't answer "Collaborative filtering (item-item or user-item)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: based on historical co-engagement

### Inverted index
- Direct Answer: keyword or tag matching for content retrieval
- Why: This matters because it tells you how to reason about inverted index.
- Pitfall: Don't answer "Inverted index" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: keyword or tag matching for content retrieval

### Rules-based
- Direct Answer: freshness filters, content type constraints
- Why: This matters because it tells you how to reason about rules-based.
- Pitfall: Don't answer "Rules-based" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: freshness filters, content type constraints

### Gradient-boosted trees (fast, interpretable, good for tabular features)
- Direct Answer: Gradient-boosted trees (fast, interpretable, good for tabular features)
- Why: This matters because it tells you how to reason about gradient-boosted trees (fast, interpretable, good for tabular features).
- Pitfall: Don't answer "Gradient-boosted trees (fast, interpretable, good for tabular features)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Gradient-boosted trees (fast, interpretable, good for tabular features)

### Deep neural networks (can incorporate embedding features, cross-features)
- Direct Answer: Deep neural networks (can incorporate embedding features, cross-features)
- Why: This matters because it tells you how to reason about deep neural networks (can incorporate embedding features, cross-features).
- Pitfall: Don't answer "Deep neural networks (can incorporate embedding features, cross-features)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Deep neural networks (can incorporate embedding features, cross-features)

### Cross encoders (most accurate, most expensive
- Direct Answer: joint encoding of query and candidate)
- Why: This matters because it tells you how to reason about cross encoders (most accurate, most expensive.
- Pitfall: Don't answer "Cross encoders (most accurate, most expensive" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: joint encoding of query and candidate)

### DCN (Deep & Cross Network)
- Direct Answer: explicitly models feature interactions
- Why: This matters because it tells you how to reason about dcn (deep & cross network).
- Pitfall: Don't answer "DCN (Deep & Cross Network)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: explicitly models feature interactions

### Different code paths
- Direct Answer: training uses batch preprocessing script; serving uses streaming computation. They disagree on how to handle nulls, outliers, or encoding.
- Why: This matters because it tells you how to reason about different code paths.
- Pitfall: Don't answer "Different code paths" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: training uses batch preprocessing script; serving uses streaming computation. They disagree on how to handle nulls, outliers, or encoding.

### Different data freshness
- Direct Answer: training uses a daily batch feature; serving uses a version computed an hour ago (or yesterday).
- Why: This matters because it tells you how to reason about different data freshness.
- Pitfall: Don't answer "Different data freshness" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: training uses a daily batch feature; serving uses a version computed an hour ago (or yesterday).

### Leakage during training
- Direct Answer: training includes a feature that uses future information. At serving time, that information is not yet available.
- Why: This matters because it tells you how to reason about leakage during training.
- Pitfall: Don't answer "Leakage during training" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: training includes a feature that uses future information. At serving time, that information is not yet available.

### Different normalization
- Direct Answer: training computes mean/std on training set; serving uses a stale statistics snapshot.
- Why: This matters because it tells you how to reason about different normalization.
- Pitfall: Don't answer "Different normalization" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: training computes mean/std on training set; serving uses a stale statistics snapshot.

### Offline store
- Direct Answer: historical feature values for training (data warehouse, columnar storage)
- Why: This matters because it tells you how to reason about offline store.
- Pitfall: Don't answer "Offline store" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: historical feature values for training (data warehouse, columnar storage)

### Online store
- Direct Answer: low-latency key-value lookup for inference (Redis, DynamoDB)
- Why: This matters because it tells you how to reason about online store.
- Pitfall: Don't answer "Online store" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: low-latency key-value lookup for inference (Redis, DynamoDB)

### Feature registry
- Direct Answer: versioned definitions of how each feature is computed
- Why: This matters because it tells you how to reason about feature registry.
- Pitfall: Don't answer "Feature registry" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: versioned definitions of how each feature is computed

### Backfill pipeline
- Direct Answer: populates the offline store from the online computation history
- Why: This matters because it tells you how to reason about backfill pipeline.
- Pitfall: Don't answer "Backfill pipeline" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: populates the offline store from the online computation history

### Training
- Direct Answer: computed via SQL window function over historical transaction table. Gets the exact 24-hour count.
- Why: This matters because it tells you how to reason about training.
- Pitfall: Don't answer "Training" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: computed via SQL window function over historical transaction table. Gets the exact 24-hour count.

### Serving
- Direct Answer: implemented in the fraud service. Queries a Redis counter that gets incremented on each transaction. Counter resets at midnight. Produces wrong values for IPs that span the midnight boundary.
- Why: This matters because it tells you how to reason about serving.
- Pitfall: Don't answer "Serving" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: implemented in the fraud service. Queries a Redis counter that gets incremented on each transaction. Counter resets at midnight. Produces wrong values for IPs that span the midnig…

### Feature defined once
- Direct Answer: COUNT transactions WHERE ip = X AND timestamp > NOW() - 24h
- Why: This matters because it tells you how to reason about feature defined once.
- Pitfall: Don't answer "Feature defined once" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: COUNT transactions WHERE ip = X AND timestamp > NOW() - 24h

### Training
- Direct Answer: feature store backfills this value from the historical event log using the same logic
- Why: This matters because it tells you how to reason about training.
- Pitfall: Don't answer "Training" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: feature store backfills this value from the historical event log using the same logic

### Serving
- Direct Answer: feature store online layer maintains the rolling counter with the same semantics
- Why: This matters because it tells you how to reason about serving.
- Pitfall: Don't answer "Serving" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: feature store online layer maintains the rolling counter with the same semantics

### Guaranteed consistency
- Direct Answer: Guaranteed consistency
- Why: This matters because it tells you how to reason about guaranteed consistency.
- Pitfall: Don't answer "Guaranteed consistency" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Guaranteed consistency

### PSI (Population Stability Index)
- Direct Answer: compare feature bucket distributions between training and current data
- Why: This matters because it tells you how to reason about psi (population stability index).
- Pitfall: Don't answer "PSI (Population Stability Index)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: compare feature bucket distributions between training and current data

### KS test
- Direct Answer: compare empirical CDFs of continuous features
- Why: This matters because it tells you how to reason about ks test.
- Pitfall: Don't answer "KS test" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: compare empirical CDFs of continuous features

### Chi-squared test
- Direct Answer: compare categorical feature distributions
- Why: This matters because it tells you how to reason about chi-squared test.
- Pitfall: Don't answer "Chi-squared test" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: compare categorical feature distributions

### Prediction distribution shift
- Direct Answer: monitor the distribution of model scores over time
- Why: This matters because it tells you how to reason about prediction distribution shift.
- Pitfall: Don't answer "Prediction distribution shift" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: monitor the distribution of model scores over time

### Label-prediction agreement
- Direct Answer: when ground truth labels arrive (with delay), measure accuracy and flag changes
- Why: This matters because it tells you how to reason about label-prediction agreement.
- Pitfall: Don't answer "Label-prediction agreement" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: when ground truth labels arrive (with delay), measure accuracy and flag changes

### Business KPI monitoring
- Direct Answer: downstream metrics (CTR, conversion) that reflect whether the model is making useful decisions
- Why: This matters because it tells you how to reason about business kpi monitoring.
- Pitfall: Don't answer "Business KPI monitoring" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: downstream metrics (CTR, conversion) that reflect whether the model is making useful decisions

### Scheduled
- Direct Answer: weekly or monthly regardless of detected drift (baseline hygiene)
- Why: This matters because it tells you how to reason about scheduled.
- Pitfall: Don't answer "Scheduled" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: weekly or monthly regardless of detected drift (baseline hygiene)

### Triggered
- Direct Answer: PSI > 0.25 on key features, or prediction distribution shift > 2σ from baseline
- Why: This matters because it tells you how to reason about triggered.
- Pitfall: Don't answer "Triggered" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: PSI > 0.25 on key features, or prediction distribution shift > 2σ from baseline

### Continuous
- Direct Answer: online learning for high-velocity signals (ads, fraud) where the world changes daily
- Why: This matters because it tells you how to reason about continuous.
- Pitfall: Don't answer "Continuous" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: online learning for high-velocity signals (ads, fraud) where the world changes daily

### Post-training quantization (PTQ)
- Direct Answer: quantize a pretrained model without retraining. Fast to apply.
- Why: This matters because it tells you how to reason about post-training quantization (ptq).
- Pitfall: Don't answer "Post-training quantization (PTQ)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: quantize a pretrained model without retraining. Fast to apply.

### Quantization-aware training (QAT)
- Direct Answer: simulate quantization during training. Better accuracy for aggressive quantization.
- Why: This matters because it tells you how to reason about quantization-aware training (qat).
- Pitfall: Don't answer "Quantization-aware training (QAT)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: simulate quantization during training. Better accuracy for aggressive quantization.

### Accuracy cost
- Direct Answer: typically < 1% for int8; higher for int4 without calibration.
- Why: This matters because it tells you how to reason about accuracy cost.
- Pitfall: Don't answer "Accuracy cost" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: typically < 1% for int8; higher for int4 without calibration.

### Unstructured pruning
- Direct Answer: zeroes out individual weights. Hard to accelerate on current hardware.
- Why: This matters because it tells you how to reason about unstructured pruning.
- Pitfall: Don't answer "Unstructured pruning" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: zeroes out individual weights. Hard to accelerate on current hardware.

### Structured pruning
- Direct Answer: removes entire neurons, heads, or layers. Directly reduces compute.
- Why: This matters because it tells you how to reason about structured pruning.
- Pitfall: Don't answer "Structured pruning" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: removes entire neurons, heads, or layers. Directly reduces compute.

### Match soft probabilities (temperature scaling), intermediate representations, or both.
- Direct Answer: Match soft probabilities (temperature scaling), intermediate representations, or both.
- Why: This matters because it tells you how to reason about match soft probabilities (temperature scaling), intermediate representations, or both..
- Pitfall: Don't answer "Match soft probabilities (temperature scaling), intermediate representations, or both." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Match soft probabilities (temperature scaling), intermediate representations, or both.

### DistilBERT achieved 97% of BERT's performance with 40% fewer parameters and 60% speedup.
- Direct Answer: DistilBERT achieved 97% of BERT's performance with 40% fewer parameters and 60% speedup.
- Why: This matters because it tells you how to reason about distilbert achieved 97% of bert's performance with 40% fewer parameters and 60% speedup..
- Pitfall: Don't answer "DistilBERT achieved 97% of BERT's performance with 40% fewer parameters and 60% speedup." by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: DistilBERT achieved 97% of BERT's performance with 40% fewer parameters and 60% speedup.

### Requires training
- Direct Answer: higher upfront cost but no inference overhead.
- Why: This matters because it tells you how to reason about requires training.
- Pitfall: Don't answer "Requires training" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: higher upfront cost but no inference overhead.

### Embedding precomputation
- Direct Answer: compute user/item embeddings in batch; serve from lookup table.
- Why: This matters because it tells you how to reason about embedding precomputation.
- Pitfall: Don't answer "Embedding precomputation" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: compute user/item embeddings in batch; serve from lookup table.

### KV-cache for LLMs
- Direct Answer: reuse past key-value computations across tokens in autoregressive generation.
- Why: This matters because it tells you how to reason about kv-cache for llms.
- Pitfall: Don't answer "KV-cache for LLMs" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: reuse past key-value computations across tokens in autoregressive generation.

### Dynamic batching
- Direct Answer: accumulate requests until batch is full or timeout fires.
- Why: This matters because it tells you how to reason about dynamic batching.
- Pitfall: Don't answer "Dynamic batching" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: accumulate requests until batch is full or timeout fires.

### Continuous batching (for LLMs)
- Direct Answer: interleave different-length sequences to maximize GPU utilization. Avoids the fixed-batch-size bottleneck that wastes compute on padding.
- Why: This matters because it tells you how to reason about continuous batching (for llms).
- Pitfall: Don't answer "Continuous batching (for LLMs)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: interleave different-length sequences to maximize GPU utilization. Avoids the fixed-batch-size bottleneck that wastes compute on padding.

### Model artifact (weights, architecture)
- Direct Answer: Model artifact (weights, architecture)
- Why: This matters because it tells you how to reason about model artifact (weights, architecture).
- Pitfall: Don't answer "Model artifact (weights, architecture)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Model artifact (weights, architecture)

### Training data reference (data hash or snapshot ID)
- Direct Answer: Training data reference (data hash or snapshot ID)
- Why: This matters because it tells you how to reason about training data reference (data hash or snapshot id).
- Pitfall: Don't answer "Training data reference (data hash or snapshot ID)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Training data reference (data hash or snapshot ID)

### Feature definition version
- Direct Answer: Feature definition version
- Why: This matters because it tells you how to reason about feature definition version.
- Pitfall: Don't answer "Feature definition version" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Feature definition version

### Training code commit SHA
- Direct Answer: Training code commit SHA
- Why: This matters because it tells you how to reason about training code commit sha.
- Pitfall: Don't answer "Training code commit SHA" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Training code commit SHA

### Evaluation metrics (at training time, by dataset and group)
- Direct Answer: Evaluation metrics (at training time, by dataset and group)
- Why: This matters because it tells you how to reason about evaluation metrics (at training time, by dataset and group).
- Pitfall: Don't answer "Evaluation metrics (at training time, by dataset and group)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Evaluation metrics (at training time, by dataset and group)

### Serving configuration (batch size, hardware, preprocessing steps)
- Direct Answer: Serving configuration (batch size, hardware, preprocessing steps)
- Why: This matters because it tells you how to reason about serving configuration (batch size, hardware, preprocessing steps).
- Pitfall: Don't answer "Serving configuration (batch size, hardware, preprocessing steps)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Serving configuration (batch size, hardware, preprocessing steps)

### Deployment history (which versions were live when)
- Direct Answer: Deployment history (which versions were live when)
- Why: This matters because it tells you how to reason about deployment history (which versions were live when).
- Pitfall: Don't answer "Deployment history (which versions were live when)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Deployment history (which versions were live when)

### Latency at query time
- Direct Answer: effectively zero (lookup)
- Why: This matters because it tells you how to reason about latency at query time.
- Pitfall: Don't answer "Latency at query time" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: effectively zero (lookup)

### Feature space
- Direct Answer: can use rich features that take seconds to compute
- Why: This matters because it tells you how to reason about feature space.
- Pitfall: Don't answer "Feature space" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: can use rich features that take seconds to compute

### Freshness
- Direct Answer: predictions can be stale (minutes to hours, depending on batch cadence)
- Why: This matters because it tells you how to reason about freshness.
- Pitfall: Don't answer "Freshness" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: predictions can be stale (minutes to hours, depending on batch cadence)

### Cost
- Direct Answer: amortized over batch
...
- Interview line: Say: amortized over batch

### Latency
- Direct Answer: must meet SLA (typically < 100ms)
...
- Interview line: Say: must meet SLA (typically < 100ms)

### Feature space
- Direct Answer: only features computable within the latency budget
- Why: This matters because it tells you how to reason about feature space.
- Pitfall: Don't answer "Feature space" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: only features computable within the latency budget

### Freshness
- Direct Answer: always fresh
- Why: This matters because it tells you how to reason about freshness.
- Pitfall: Don't answer "Freshness" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: always fresh

### Cost
- Direct Answer: per-request model forward pass
- Why: This matters because it tells you how to reason about cost.
- Pitfall: Don't answer "Cost" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: per-request model forward pass

### For each user
- Direct Answer: compute top 20 recommended items using collaborative filtering + content model
- Why: This matters because it tells you how to reason about for each user.
- Pitfall: Don't answer "For each user" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: compute top 20 recommended items using collaborative filtering + content model

### Store
- Direct Answer: user_id → [item_1, item_2, ..., item_20]
- Why: This matters because it tells you how to reason about store.
- Pitfall: Don't answer "Store" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: user_id → [item_1, item_2, ..., item_20]

### Cost
- Direct Answer: 5M users × 20 items × model forward pass = handled in batch job
- Why: This matters because it tells you how to reason about cost.
- Pitfall: Don't answer "Cost" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: 5M users × 20 items × model forward pass = handled in batch job

### Look up precomputed recommendations for each user
- Direct Answer: Look up precomputed recommendations for each user
- Why: This matters because it tells you how to reason about look up precomputed recommendations for each user.
- Pitfall: Don't answer "Look up precomputed recommendations for each user" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Look up precomputed recommendations for each user

### Apply business rules (filter out out-of-stock items, exclude recently purchased)
- Direct Answer: Apply business rules (filter out out-of-stock items, exclude recently purchased)
- Why: This matters because it tells you how to reason about apply business rules (filter out out-of-stock items, exclude recently purchased).
- Pitfall: Don't answer "Apply business rules (filter out out-of-stock items, exclude recently purchased)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Apply business rules (filter out out-of-stock items, exclude recently purchased)

### Serve
- Direct Answer: Serve
- Why: This matters because it tells you how to reason about serve.
- Pitfall: Don't answer "Serve" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Serve

### AUC > 0.92 on holdout set
- Direct Answer: AUC > 0.92 on holdout set
- Why: This matters because it tells you how to reason about auc > 0.92 on holdout set.
- Pitfall: Don't answer "AUC > 0.92 on holdout set" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: AUC > 0.92 on holdout set

### AUC on each demographic group > 0.88 (fairness gate)
- Direct Answer: AUC on each demographic group > 0.88 (fairness gate)
- Why: This matters because it tells you how to reason about auc on each demographic group > 0.88 (fairness gate).
- Pitfall: Don't answer "AUC on each demographic group > 0.88 (fairness gate)" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: AUC on each demographic group > 0.88 (fairness gate)

### Inference latency P99 < 50ms on standard hardware
- Direct Answer: Inference latency P99 < 50ms on standard hardware
- Why: This matters because it tells you how to reason about inference latency p99 < 50ms on standard hardware.
- Pitfall: Don't answer "Inference latency P99 < 50ms on standard hardware" by naming the concept alone; state the mechanism and tradeoff.
- Interview line: Say: Inference latency P99 < 50ms on standard hardware
