...
*Related: [Speculative Decoding](07-speculative-decoding.md) | [Context Window Extension](../10-llms/07-context-window-extension.md) | [Tuning and Optimization](10-tuning-optimization.md)*
...
### Q: What is speculative decoding and when does it provide a speedup? [Hard]
...
- *You tie the speedup to acceptance rate alpha. Under what serving condition does speculative decoding go from a win to a net loss even with a decent alpha?* -> At high batch / high load. Speculative decoding trades *extra compute* (drafting + parallel verification of K tokens) for *fewer sequential steps* -- that's a great trade when you're memory-bandwidth-bound at low batch and have spare FLOPs. But once the server is compute-bound under a full batch, the draft and the K-wide verification compete for the same saturated compute, so you're adding work to a system with no slack, and even alpha=0.7 can slow aggregate throughput. It optimizes single-stream latency, often at the cost of multi-stream throughput.
- *Acceptance rate isn't a fixed model property. What makes alpha drift within a single deployment, and why does that break your capacity planning?* -> Alpha depends on the *content*: it's high on predictable text (boilerplate, code scaffolding) and low on creative, high-entropy, or out-of-domain generation -- so the same draft/target pair delivers 2.5x on one traffic mix and ~1x on another. If you provisioned assuming a measured alpha from one workload, a shift in query mix silently erases the speedup and leaves you with the draft-model overhead. You can't size the fleet on a single alpha number; it's a distribution that moves with usage.

**Trap:** citing "identical output distribution" as if speculative decoding is free correctness-wise and therefore always worth enabling. Correctness is preserved, but the *performance* is conditional -- on batch size, on alpha, and on the draft model being genuinely cheap relative to the target. It's a latency optimization with a compute cost, not a universal throughput win.
### Q: Compare continuous batching vs static batching for LLM serving — why is continuous batching essential at scale? [Medium]
...
- *Continuous batching inserts new requests every step to keep the GPU full. What fairness/latency problem does that create for a request already streaming?* -> When new requests are admitted mid-flight, they share the same compute step as in-progress decodes, so every currently-streaming request's inter-token latency (TPOT) jumps as batch size grows -- an early user's smooth stream can stutter because the scheduler kept admitting newcomers to maximize throughput. Continuous batching optimizes aggregate GPU utilization, and without an admission/priority policy that throughput comes out of the tail latency of the requests already in the batch. Throughput and per-request smoothness are in tension.
- *You say PagedAttention gives "near-zero fragmentation." What new failure mode does paging introduce that contiguous allocation didn't have?* -> Preemption and its cost. Because KV blocks are allocated on demand, a burst of long-context requests can exhaust the block pool mid-generation, forcing the scheduler to *preempt* running sequences -- either recomputing their KV later (compute cost) or swapping it to host memory (bandwidth cost). Contiguous pre-allocation would have simply rejected the request up front; paging lets you over-admit and then pay for it with preemption stalls under memory pressure. You traded fragmentation waste for admission-control complexity.

**Trap:** presenting continuous batching + PagedAttention as pure throughput upside with no downside. The throughput is real, but it's bought with tail-latency variance (mid-batch admission raising TPOT) and preemption risk under memory pressure -- which is why production schedulers layer priority, chunked prefill, and admission control on top. The mechanism alone doesn't give you an SLA.
## Flashcards
