...
### Q: What are the failure modes of RAG and how do you address each? [Hard]
...
- *Your fixes for retrieval miss are query expansion and HyDE. How can HyDE make retrieval strictly worse, and on which queries?* -> HyDE embeds a *generated* hypothetical answer, so it inherits the generator's hallucinations -- on tail or out-of-domain queries where the model doesn't know the answer, it confabulates a plausible-but-wrong document and retrieves neighbors of that fiction, pulling in confidently irrelevant context. It helps most on queries the model already half-knows and hurts most on exactly the rare queries retrieval exists to cover. You've moved the hallucination problem upstream into retrieval, where it's harder to detect than in the final answer.
- *You add "answer ONLY from provided documents" to fix faithfulness. Why can that instruction degrade the product even when it works?* -> Because hard grounding turns every retrieval miss into a refusal -- when the right chunk wasn't retrieved (the most common RAG failure), the model now says "I don't know" instead of answering from parametric knowledge it may actually have. You trade hallucination for coverage loss, and users often experience an over-refusing assistant as more broken than an occasionally-wrong one. The grounding strictness has to be tuned against retrieval recall, not maxed out.

**Trap:** treating RAG failure modes as independent bugs with independent fixes. They couple -- tighter grounding amplifies retrieval-miss pain, aggressive query expansion raises latency and dilutes precision, and reranking hard enough to fix context-length can bury the answer via lost-in-the-middle. Fixing one failure mode routinely worsens another; the real work is balancing them for your query distribution.
### Q: What is the difference between RAG and fine-tuning for injecting knowledge, and when do you use each? [Medium]
...
- *You say "fine-tune for behavior, RAG for facts." A team fine-tunes on their internal docs to "teach the model company knowledge." Why is that usually the wrong tool, and what actually happens?* -> Fine-tuning on a corpus of facts is a poor way to *store* facts: the model doesn't reliably memorize tail facts from a few exposures, it can't cite sources, and it confabulates similar-sounding facts for the ones it half-learned -- so you get an authoritative-sounding model that's subtly wrong and un-auditable. And the moment a document changes, the knowledge is stale with no way to patch it short of retraining. What fine-tuning *does* absorb from that corpus is style and domain vocabulary, not a queryable fact store -- which is RAG's job.
- *RAG "keeps knowledge up to date," but what maintenance burden does that claim quietly assume, and how does it fail silently?* -> It assumes the index actually tracks the source of truth -- but embeddings go stale when documents change and nobody re-embeds, deletions leave orphaned chunks that still retrieve, and a model/encoder upgrade requires re-embedding the *entire* corpus or query/doc spaces diverge. "Always up to date" is true only if you run the re-embedding and reconciliation pipeline; skip it and RAG serves confidently outdated chunks with no error, which is harder to notice than a model that's uniformly stale.

**Trap:** framing it as RAG-vs-fine-tuning, either/or. Production systems almost always do both -- fine-tune (or instruction-tune) for behavior, format, and domain fluency; RAG for current, attributable facts -- and the interesting question is the boundary between them, not which one to pick.
## Flashcards
