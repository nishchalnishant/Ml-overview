...
### Q: What are the assumptions behind k-means and when does it fail? [Medium]
...
- *You'd "use silhouette score to pick k." On a large high-dimensional dataset, why can silhouette quietly mislead you?* -> Silhouette is built on Euclidean distances, and in high dimensions distances concentrate -- near and far collapse toward the same value, so the silhouette signal flattens and its peak becomes noise. It also assumes convex, roughly-equal clusters (the same assumptions k-means makes), so it rewards the k that fits *k-means' own bias*, not the true structure. Picking k by silhouette can just confirm the algorithm's spherical prior rather than test it.
- *You reach for GMM to escape the spherical assumption. What does GMM cost you that k-means didn't, beyond compute?* -> Identifiability and stability: full-covariance GMM has many more parameters, so on limited data it overfits, and EM can drive a component's covariance toward a singularity (one point, zero variance, infinite likelihood) -- you have to regularize the covariance or it collapses. You also inherit the same local-minimum and k-selection problems, now with a likelihood that's easy to game. GMM buys flexibility at the price of a much twitchier fit.

**Trap:** presenting DBSCAN as the general fix for non-spherical clusters. DBSCAN trades k for two parameters (eps, min_samples) that are just as hard to set, assumes roughly uniform density, and falls apart when clusters have very different densities -- exactly one of the k-means failure modes you invoked it to solve. There is no assumption-free clusterer; you're choosing which assumption to violate.
