...
### Q: How does TensorRT optimize a neural network graph for NVIDIA GPUs (e.g., kernel fusion, precision calibration)?
A: TensorRT is an SDK that compiles a generic neural network graph (like ONNX) into a highly optimized engine specifically tuned for the exact NVIDIA GPU architecture it is compiled on. It performs several key optimizations: 
1. **Kernel Fusion:** It fuses multiple sequential operations (e.g., Conv2D + BatchNorm + ReLU) into a single CUDA kernel. This drastically reduces the memory bandwidth overhead of reading and writing intermediate activation tensors to VRAM, which is often the primary bottleneck in inference.
2. **Precision Calibration (Quantization):** It safely lowers the precision of weights and activations from FP32 to FP16 or INT8. For INT8, it uses a calibration dataset to measure the dynamic range of activations and computes a scale factor to minimize information loss (typically using KL-divergence), enabling massive throughput gains with negligible accuracy drops.
3. **Dynamic Tensor Memory & Kernel Auto-Tuning:** It pre-allocates memory for all intermediate tensors (avoiding runtime allocation overhead) and empirically profiles different CUDA kernel implementations for each layer, selecting the fastest one for the specific batch size, layer dimensions, and target hardware architecture.


> Staff-level and Research Engineer depth — distributed systems, deep debugging, and complex system design.

### Q: What is ZeRO (DeepSpeed) and how do its stages differ in memory savings?
ZeRO eliminates data parallelism memory redundancy. **Stage 1**: shards optimizer states (Adam momentum/variance, ~4x reduction). **Stage 2**: shards gradients (~8x reduction). **Stage 3**: shards parameters (gathered just-in-time, memory scales as 1/N GPUs). Higher stages trade increased communication overhead for memory savings.

### Q: What is gradient checkpointing and its tradeoff?
Standard backprop stores all intermediate activations, dominating memory for large sequences. Checkpointing stores only a subset, recomputing discarded activations on-the-fly during the backward pass. Trades ~30% more compute time for ~70% less activation memory.

### Q: Compare data, tensor, and pipeline parallelism (3D parallelism).
**Data**: splits batch across GPUs (DDP/FSDP). **Tensor**: splits individual weight matrices/layers across GPUs (requires high-bandwidth NVLink within a node). **Pipeline**: splits model layers across nodes/GPUs (requires micro-batching to minimize pipeline idle bubble). 3D parallelism combines all three for massive models.

### Q: Why is LLM decode memory-bandwidth-bound while prefill is compute-bound?
Decode (batch size 1) loads N parameters to do 2N FLOPs (low arithmetic intensity); limited by HBM speed. Prefill processes S tokens simultaneously, reusing loaded weights (high arithmetic intensity); limited by GPU FLOPs. Implication: decode throughput scales almost for free with increasing batch size up to the compute bound.

### Q: What are the primary levers for reducing ML training cost?
(1) Spot/preemptible instances (60-90% discount, requires robust fault-tolerant checkpointing). (2) Mixed precision (BF16) (1.5-2x speedup, 50% memory reduction, negligible accuracy drop). (3) Gradient checkpointing (when memory prevents efficient batch sizes).

### Q: What is CUPED and how does it reduce experiment duration?
CUPED uses pre-experiment data (e.g., a user's historical spend) as a covariate to explain away pre-existing between-user variance. `Y_adjusted = Y - theta * (X - E[X])`. Reduces metric variance, allowing detection of the same effect size with 30-70% less traffic/time. Fails for new/cold-start users with no history.

### Q: Walk through a production incident: precision dropped from 0.85 to 0.60.
1. Check training-serving skew (compare live feature distributions to training).
2. Check label distribution / base rate (did fraud prevalence shift, moving the operating point?).
3. Check concept drift (recompute PR-AUC on a fresh labeled sample).
4. Check upstream data pipelines (schema changes, nulls).
5. Check calibration (recompute Platt scaling; miscalibration shifts precision without changing ranking).

### Q: Design an automatic rollback system for model deployment.
Define guardrails: max error rate, max P99 latency, max PR-AUC regression vs champion. A canary evaluates these on a rolling window. Any failure triggers automatic rollback: repromote the specific versioned prior model artifact from the registry (Archived → Production), alert on-call, and scale down the canary.

### Q: Design an incident response process for a production ML model.
Define ML-specific severity tiers: P1 (critical harm, e.g., fraud model failing open) → immediate kill-switch/rollback. P2 (high, major metric regression) → hotfix in hours. P3 (medium, drift detected). P4 (low, anomaly logged). Requires dedicated ML-quality alerting (drift/calibration monitors) wired into paging systems.

### Q: What is Inverse Propensity Scoring (IPS) for click logs?
Click logs have position bias (position 1 gets clicked more than position 10 just by being there). IPS reweights each click by `1 / P(examined | position)`. Upweights clicks on low-visibility positions to estimate true relevance. High variance for small propensities requires clipping.

### Q: A model's P99 latency spiked from 50ms to 400ms overnight with no code deploy. Diagnose.
1. Infra metrics: QPS spike, CPU throttling, GC pauses, node autoscaling lag.
2. Noisy neighbors on shared clusters.
3. Request shape: did payloads/sequences suddenly get larger?
4. P50 vs P99 gap: if P50 is stable, it's a tail-latency issue (retries, cold cache).
5. Silent config/data change: e.g., feature store TTL expired en masse causing cache misses. Check automated retrain jobs that pushed a larger model.

### Q: Distinguish a training-serving skew bug from genuine concept drift.
Check feature distributions first. If live features diverge from training (PSI/KS-test), it's a pipeline bug/skew (e.g., null-handling change). If feature distributions are stable but predictions don't track labels, it's concept drift. Always rule out skew first before launching an expensive retraining cycle.

### Q: Why is offline accuracy misleading (Epic Sepsis Model example)?
Offline metrics evaluate on static snapshots. The Epic Sepsis Model reported 0.76 AUROC (looks good), but at the deployed threshold had 33% sensitivity and 18% clinician acceptance. AUROC hides real-world performance at the specific operating point, and ignores downstream human-factors (alert fatigue).

### Q: Serve a GBM model with a strict <50ms end-to-end latency budget.
Budget allocation: feature fetch from Redis <10ms, transformation <2ms, compiled GBM inference <2ms, network overhead <5ms. Keep model warm (LRU cache). Size Redis connection pool to avoid stalls. Use timeouts on feature fetching with fallback to default values. Inference is rarely the bottleneck; feature fetch fan-out is.

### Q: Prevent training-serving skew in a real-time streaming ML system.
Use a single source-of-truth feature definition (e.g., Feast FeatureView, Flink SQL) compiled to both batch and streaming execution paths. Run continuous online/offline parity testing. Ensure point-in-time correct joins so the streaming path doesn't leak "future" information relative to the training snapshot.

### Q: Design a fraud detection system with <10ms latency.
Write path: Kafka → Flink job (velocity features) → Redis online store. Read path: synchronous feature fetch (parallel GETs) + lightweight compiled GBM scoring in ~2ms. Heavier models (graphs) run asynchronously to precompute risk scores. Include a circuit breaker and rules-engine fallback if the ML service times out.

### Q: Real-time recommendation system at 10K RPS. What are the biggest risks?
1. ANN index partial failure (silent return of fewer results).
2. Feature store read amplification (connection pool exhaustion).
3. Thundering herd on cache expiry (stagger TTLs).
4. Cold start under load (needs fast fallback).
5. Feedback loop / exposure bias (reinforcing popularity). Peak load graceful degradation is the main constraint.

### Q: Backfill two years of a feature without introducing training data leakage.
Naive join leaks future information. Correct approach: point-in-time-correct backfill. For each historical label, recompute the feature using only data existing as of the label's timestamp. If the feature is streaming, replay the raw historical event log through the streaming job in batch mode, preserving original event timestamps.
