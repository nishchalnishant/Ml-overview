...
**Q: Walk through the full design of an A/B test for a new recommendation algorithm. What can go wrong?**
A: (1) **Define the metric**: primary metric (e.g., 7-day retention or revenue per user), guardrail metrics (latency, error rate, abuse signals); avoid moving multiple things at once. (2) **Power analysis**: determine required sample size: n = 2(z_{α/2} + z_β)²·σ²/δ² where δ is the minimum detectable effect (MDE), σ² is metric variance, α=0.05 (false positive rate), β=0.2 (80% power). For a retention metric with σ=0.4 and MDE=1%: n ≈ 12,800 per variant. (3) **Randomization unit**: randomize at user level (not session level) to avoid the same user seeing both variants. Use a hash function on user_id + experiment_id for deterministic assignment. (4) **Guardrail checks**: AA test first (assign users to two identical "variants" — any significant difference means your randomization or pipeline is broken). Run for the full intended duration — stopping early inflates false positives. (5) **Analysis**: two-sample t-test for means, z-test for proportions; check for SUTVA violations (did control users interact with treatment users?). What goes wrong: novelty effect (users engage more because it's new, effect fades), seasonal confounding (week 1 is a holiday), SRM (sample ratio mismatch — unequal group sizes indicate a logging bug), and metric sensitivity (the metric doesn't move even when the change is good because it's too noisy).

**Q: What is CUPED and when would you use it in an A/B test?**
A: CUPED (Controlled-Experiment Using Pre-Experiment Data) is a variance reduction technique that uses pre-experiment covariate data to reduce the variance of the outcome metric, increasing statistical power without collecting more data. For metric Y (post-experiment) and covariate X (pre-experiment, e.g., same metric from the week before): Y_CUPED = Y - θX, where θ = Cov(Y,X)/Var(X). The adjusted metric has the same mean (since E[X] is the same in both groups due to randomization) but lower variance: Var(Y_CUPED) = Var(Y)(1 - ρ²), where ρ = Corr(Y,X). If ρ = 0.8 (strong autocorrelation in user behavior, typical for retention/engagement): variance reduction = 1 - 0.64 = 36%, meaning you need 36% fewer users for the same power. When to use: whenever a pre-experiment measurement of the same or correlated metric is available — engagement, revenue, time-on-site all have high autocorrelation across weeks. LinkedIn, Netflix, and Airbnb all apply CUPED as standard practice, reporting it halves required experiment duration on behavioral metrics. Limitation: only helps when the covariate is strongly correlated with the outcome; doesn't help for new features where no pre-experiment analog exists.
