...
def find_chapter_start_from_page(pdf_path, chapter_number, start_search_page):
    try:
        ...
        for i in range(start_search_page, num_pages):
            ...
            if chapter_str in page_text:
                index = page_text.find(chapter_str)
                # Heuristic: Header usually at the top
                if index < 500:
                    return i, page_text
        return None, None
...
file_path = "_OceanofPDF.com_Hands-On_Large_Language_Models_-_Jay_Alammar.pdf"
# Start searching from page 460 (Ch 11 was 445)
start_page, page_content = find_chapter_start_from_page(file_path, 12, 460)

if start_page:
    print(f"Chapter 12 found at page {start_page}")
...
```
Chapter 12 found at page 486
--- CONTENT EXTRACT ---
Chapter 12. Fine-T uning
Generation Models
...
tuning pretrained text generation models to make them more ef fective tools
...
or foundation  model. Base models are a key artifact of the trainingprocess but are harder for the end user to deal with. This is why the next
step is important.
Figur e 12-1. During language modeling, the LLM aims to pr edict the next token based on an
input. This is a pr ocess without labels.
...
the base model are updated to be more in line with our tar get task, like
...
Figur e 12-2. During supervised fine-tuning, the LLM aims to pr edict the next token based on an
...
generative model.
3. Fine-tuning 2 (pr eference tuning)
...
model to our preferences, which are defined by the data th

```

Based on the content retrieved from the file, here are the comprehensive notes for Chapter 12.
