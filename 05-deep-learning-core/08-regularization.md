...
### Q: Explain the L1 vs L2 regularization trade-off — why does L1 produce sparse weights? [Medium]
...
- *"L1 isn't differentiable at zero — how does SGD handle it?"* → Plain subgradient descent oscillates around zero and rarely lands exactly on it. Real sparsity comes from proximal methods — soft-thresholding, which is exactly the shrink-then-clip-to-zero operator — or from FTRL-style updates. If you claim "L1 gives you exact zeros" with vanilla SGD, expect to be pushed on it.
- *"Give the Bayesian reading."* → L2 is a Gaussian prior on the weights, L1 a Laplace prior. The Laplace has a spike at zero, so the MAP estimate genuinely prefers exact zeros. Same conclusion as the geometry argument, from a different direction.
- *"Is weight decay the same as L2?"* → Only for plain SGD. With adaptive optimizers the L2 gradient gets divided by the per-parameter second-moment estimate, so large-gradient parameters get decayed less — which is not what you want. AdamW decouples the decay from the gradient, and that's why it's the default for transformers.

**Trap:** Reaching for L1 to do feature selection on correlated features. It picks one arbitrarily from each correlated group and zeroes the rest — the choice is unstable across reseeds, so the "selected features" aren't a reliable finding. ElasticNet exists for this.

---

### Q: Why does dropout work — what is it implicitly optimizing? [Medium]

 Dropout zeroes each activation with probability p during training and scales by 1/(1-p) at test time. Three complementary explanations: (1) Ensemble approximation — each forward pass uses a different subset of the network, so training corresponds to optimizing 2^N possible networks simultaneously; test-time prediction is approximately averaging these networks; (2) Prevents co-adaptation — neurons can't rely on the presence of specific other neurons, so each must learn to be useful independently; this forces distributed representations; (3) Implicit Bayesian inference — Gal & Ghahramani (2016) showed dropout at test time approximates Bayesian posterior inference over the network weights; keeping dropout on at test time gives a MC estimate of prediction uncertainty. Practical notes: dropout is effective for dense layers but hurts convolutional layers (spatial correlation means nearby units are redundant; SpatialDropout2D is better). DropPath (for transformers/residual nets) drops entire residual branches at path granularity, working better for deep architectures where per-activation dropout creates inconsistent feature maps.

**Cross-questions to expect:**
- *"Why is it so much rarer in modern LLMs?"* → Large models on near-single-epoch data regimes are not in the overfitting regime that dropout was designed for, and dropout costs throughput and adds gradient noise. Most large LLMs train with dropout 0.0; it reappears at fine-tuning time, when a small dataset does make overfitting the binding constraint.
- *"What happens if you put dropout before BatchNorm?"* → A variance mismatch. Dropout changes the activation variance between train and test, and BN's running statistics are estimated under the train-time variance — so they're wrong at inference (Li et al., 2019). Standard fixes: put dropout after all BN layers, or use one and not the other.
- *"Is the $2^N$ ensemble claim exact?"* → No. Weight-scaling inference is exactly the ensemble geometric mean only for a single linear layer with softmax; for deep nonlinear nets it's an approximation that happens to work well. Stating it as exact is a claim you'll be asked to justify.

**Trap:** Quoting "0.5 is the standard rate." That is the 2014 dense-layer figure. Transformers use 0.0–0.1, convolutional nets use spatial variants, and 0.5 inside a modern architecture is usually just underfitting.

---

### Q: How do you detect and handle overfitting in a deep learning model? [Easy]

 Detection: training loss much lower than validation loss and diverging, or validation metric plateaus while training metric continues improving. Diagnosis first — check if overfitting is from: (1) too few examples (get more data or augment), (2) model too large (reduce depth/width), (3) training too long (early stopping), or (4) features leaking label information. Mitigation stack in order of preference: (1) More data / data augmentation — always the first resort; augmentation creates synthetic variations the model shouldn't overfit to; (2) Early stopping — checkpoints at best validation metric, stop after K epochs without improvement; (3) Dropout — typically 0.1-0.3 in transformer FFNs, 0.5 in dense layers; (4) Weight decay (L2) — λ=0.01-0.1 for most cases; (5) Reduce model size if the gap is extreme. Anti-pattern: applying heavy regularization instead of diagnosing the root cause. If the model overfits with 1K training examples, adding dropout may hide the symptom but not address that you need more data.

**Cross-questions to expect:**
- *"Train loss below val loss — is that always overfitting?"* → No. Dropout and augmentation are active during training and off at validation, so a lower *val* loss early in training is expected and benign. The signal is the val curve turning upward while train keeps falling, not the gap itself.
- *"What if train and val are both bad?"* → That's underfitting, and every remedy above makes it worse. Diagnose direction before reaching into the regularization toolbox — this is the single most common way candidates answer this question backwards.
- *"How do you know your val set is even trustworthy?"* → Check the split. Random splits leak under temporal or grouped structure — the same user, patient, or session on both sides gives you an optimistic val score, and the model looks fine until production. A suspiciously *small* train/val gap on structured data is a leakage smell.
- *"Does double descent change the 'reduce model size' advice?"* → It complicates it. Past the interpolation threshold, more capacity can improve test error rather than hurt it, so shrinking the model is a plausible fix in the classical regime and a bad reflex at large scale. Say which regime you're in.

**Trap:** Treating early stopping as free. It couples your stopping point to the val set, so the reported val metric is optimistically biased — you selected on it. Keep a held-out test set you never stop on.
