...
Questions requiring deep reasoning about the actual implementation, edge cases,
"how would you extend/combine this," or senior/staff-level judgment calls.

### Tabular ML Pipeline (Project 01)

#### Q: How would you extend this to a real production scale?
Swap `generate_data.py`'s synthetic CSV for a real feature store/warehouse query;
replace `joblib`-based artifact storage with a model registry (e.g. MLflow Model
Registry) that versions the model against data version and code commit hash; add
canary/shadow deployment in front of `serve.py` before full rollout; add
input-drift monitoring (e.g. PSI on the `ColumnTransformer`'s input distributions)
and prediction-drift monitoring on the `/score` output distribution; close the
label loop by logging every prediction with an ID and joining back true churn
outcomes after the label window elapses; and replace the 8-trial hand-rolled random
search with Optuna (TPE + pruning) once each trial's cost justifies a smarter
sampler, per the note in `05-interview-template`'s `04_hyperparameter_search.py`.

#### Q: `model_dl.pt` reportedly doesn't ship the final fit's trained weights into the checkpoint in a production-safe way. How would you fix this, and why does it matter?
The comment in `train_dl.py` flags that the checkpoint currently captures
architecture/config from the winning HPO trial but the actual production-ready
retrain-on-full-data step isn't wired into what gets persisted — meaning if you
naively deploy `model_dl.pt` as saved, you could be shipping the last CV fold's
weights (trained on 4/5 of the data) rather than a final model refit on 100% of
the available training data before serving. The fix: after HPO selects a config and
K-fold CV confirms it, add an explicit final step that retrains on the *entire*
training set (train+val combined, test still held out) with the winning
hyperparameters, and only *that* state dict gets serialized into the checkpoint —
otherwise you're silently under-utilizing data at serving time relative to what the
reported CV score implies is achievable.

### RAG Pipeline (Project 02)

#### Q: What's a limitation of the two-stage retrieval design here?
The "reranker" is only a lexical-overlap heuristic, not a learned cross-encoder, so
it can't capture semantic relationships a real cross-encoder would (e.g.,
paraphrases with zero keyword overlap won't get reranked up). The numpy brute-force
index in `retrieval.py`'s `dense_search()` is O(n) per query over the full vector
matrix — fine for a documentation-sized corpus but wouldn't scale to millions of
chunks without a real ANN index (FAISS HNSW/IVF, or a hosted vector DB). There's
also no query rewriting/decomposition, no hybrid BM25 index (the "lexical" signal
is a hand-rolled keyword-overlap ratio, not real BM25), and no chunk-level
deduplication if two chunks (with overlap) contain near-identical sentences.

#### Q: How would you scale this RAG pipeline to a production, multi-tenant document set?
Replace the numpy `.npy` + `metadata.json` persistence with a real vector database
(pgvector, Pinecone, Weaviate, or FAISS with an HNSW/IVF index) to support ANN
search at scale and incremental upserts instead of a full rebuild via
`build_index.py`. Replace the keyword-overlap reranker with a real cross-encoder
(e.g. `ms-marco-MiniLM` or a hosted rerank API) for genuine semantic reranking.
Add per-tenant namespacing/filtering at the index layer for multi-tenant isolation.
Add caching for repeated queries and embedding batches, async I/O for the
embedding and generation calls, and observability (retrieval hit-rate, citation
compliance rate, latency percentiles per stage). Swap `generate.py`'s
`_openai_answer`/`_anthropic_answer` for a proper client with retries, streaming,
and cost/latency tracking rather than a single blocking call.

### LLM Fine-Tuning with LoRA (Project 03)

#### Q: How would you extend this to a QLoRA setup or a much larger base model?
Swap `AutoModelForCausalLM.from_pretrained(args.model)` for a quantized load via
`bitsandbytes` (`load_in_4bit=True`, `bnb_4bit_compute_dtype=torch.bfloat16`) and
wrap with `peft.prepare_model_for_kbit_training()` before `get_peft_model()` — this
is the QLoRA pattern, letting a much larger base model's frozen weights live in
4-bit while only the LoRA adapters train in higher precision. For a real dataset,
replace `prepare_data.py`'s hardcoded list with `datasets.load_dataset(...)` on an
actual instruction-tuning corpus (e.g. Alpaca, Dolly), add a validation split,
set `TrainingArguments(evaluation_strategy="steps", eval_steps=...)`, and swap
`target_modules` to the architecture's actual attention projections (e.g.
`["q_proj", "k_proj", "v_proj", "o_proj"]` for Llama/Qwen-style models) instead of
GPT-2's fused `c_attn`.

#### Q: If you needed to serve dozens of different LoRA adapters for different customers behind one endpoint, how would you architect that on top of this project's pattern?
Keep exactly one frozen base model resident in memory (the same base model
`compare.py` loads once) and load each customer's adapter as a small, swappable
`PeftModel` layer on top rather than maintaining N full model copies — this is
already the deployment shape the project sets up (`PeftModel.from_pretrained(base,
adapter_dir)`). At serving scale, use a multi-adapter serving framework (e.g. vLLM's
S-LoRA/dynamic LoRA support, or Hugging Face's `PeftModel` adapter-swapping API via
`model.set_adapter(name)`/`add_adapter()`) so a single batch of requests routed to
different customers can be served by dynamically selecting the right adapter's
weights per request without reloading the base model, batching requests across
adapters where the serving framework supports heterogeneous-adapter batching, and
keeping a registry mapping customer ID → adapter path/version so adapters can be
updated independently without touching the shared base model or other customers'
adapters.

### Multi-Agent A2A/MCP System (Project 04)

#### Q: Trace one concrete request through all three agents and both protocol layers.
Given a goal like "Why did churn increase in Q3?": (1) `run_pipeline.py` A2A-POSTs
an initial `Task` to the Research Agent at port 8001. (2) The Research Agent
extracts keywords from the goal, uses its MCP client to call `search_notes(keyword)`
on `notes-server` (which searches the raw text corpus in `data/notes.txt`) for each
keyword, collects matching notes, then itself becomes an A2A *client* and
`send_task()`s a new `Task` carrying those findings to the Analysis Agent at port
8002. (3) The Analysis Agent reads the findings, decides which metrics are
relevant, calls its MCP client's `compute_trend` tool on `stats-server` (real
arithmetic over `data/metrics.json`: quarter-over-quarter % change, trend
direction, next-quarter projection) per metric, then A2A-sends a `Task` with
findings + analysis to the Report Agent at port 8003. (4) The Report Agent
assembles a Markdown report and calls its MCP client's `write_report` tool on
`filesystem-server` to persist `output/quarterly_report.md`, then returns the
completed `Task` back up the A2A chain to the Analysis Agent, to the Research
Agent, to the original driver, which prints the final text.

#### Q: What's a limitation of this system as built?
It's a strictly linear pipeline (Research → Analysis → Report) with no branching,
retries, or parallel fan-out — a real multi-agent system might need the Report
Agent to request clarification back from Analysis, or fan work out to multiple
Research Agents in parallel. There's no authentication/authorization on the A2A
HTTP endpoints (anyone who can reach `127.0.0.1:8001-8003` can POST tasks), no
persistence of task state (a crashed agent loses in-flight tasks), and the "trend"
computation in `stats-server` is simple arithmetic over a static
`data/metrics.json`, not a connection to a real analytics warehouse. It's also all
local processes on one machine — there's no service discovery beyond hardcoded
ports, which wouldn't survive a real multi-host deployment.

#### Q: How would you productionize this multi-agent system?
Replace hardcoded `127.0.0.1:800N` URLs with real service discovery (DNS,
a service mesh, or a registry the driver queries instead of assuming fixed ports).
Add authentication on both the MCP transport (currently trusted local stdio
subprocesses) and the A2A HTTP layer (mutual TLS or signed tokens between agent
services, especially once agents might be owned by different teams or companies —
the scenario A2A is designed for). Add task persistence and retries so a crashed
Analysis Agent doesn't silently lose the Research Agent's findings — durable queues
or a task store keyed by `task.id`. Add structured logging/tracing across the A2A
hops and MCP tool calls for debugging a failed pipeline run. Move `data/notes.txt`
and `data/metrics.json` to real backing services (a search index, a metrics
warehouse) behind the same MCP tool interfaces so the agent code doesn't change,
only the MCP servers' internals.

#### Q: If the Report Agent crashes mid-task after the Analysis Agent has already sent it work, what actually happens today, and how would you make this failure mode recoverable?
Today, nothing recovers it: `send_task()` is a single blocking HTTP POST with no
retry, timeout-and-retry, or persistence layer, so if the Report Agent process dies
after accepting the task (status `"working"`) but before finishing, the Analysis
Agent's `send_task()` call simply fails or hangs, the exception propagates back up
the A2A chain, and `run_pipeline.py`'s `finally` block tears down all three
subprocesses — the in-flight task and the Research/Analysis agents' completed work
are lost with no record. To make this recoverable: persist each `Task` (keyed by
`task.id`) to durable storage the moment it's created, not just held in memory;
have `send_task()` retry with backoff and idempotency keys so a retried POST after
a timeout doesn't double-execute a side effect like `write_report`; add a
supervisor/health-check process that restarts a crashed agent and requeues its
last-accepted-but-unfinished task from the persisted store; and make `write_report`
itself idempotent (e.g. write-then-atomic-rename) so a retried write after a
partial-crash doesn't corrupt `output/quarterly_report.md`.

### Live-Coding Interview Template (Project 05)

#### Q: The README lists an 8-step "trained model to production" checklist and an 8-step "post-deployment" checklist — what are the two or three most interview-relevant points to hit if time is short?
From the "trained model to production" list: evaluate on the held-out test set
exactly once (not the validation set used for HPO/early stopping — that number is
already optimistic), and freeze/serialize the *whole* pipeline (preprocessing +
model as one artifact) rather than re-deriving preprocessing logic in a different
service — the README calls this out as the most common cause of training-serving
skew. From the post-deployment list: shadow/canary before a full rollout, and
closing the label loop (log every prediction with an ID, join back the real
outcome once it arrives, and compute live AUC/calibration — proxy signals like
drift and latency alone aren't a substitute).

#### Q: What's explicitly out of scope for this template, and what would you say if an interviewer handed you image or text data instead?
The README lists explicit non-goals: image/CNN, text/transformer, or sequence
models (pointed instead at `03-deep-learning/methods/`); multiclass targets (a
small, stated delta: swap to `nn.CrossEntropyLoss`, use macro-F1/multiclass
ROC-AUC, widen the final layer to `n_classes`, but keep the same split-ordering
and preprocessing contract); pure system-design/theory-only formats
(`07-interview-prep/` instead); and messy/raw data needing on-the-spot EDA (the
bundled toy datasets are pre-cleaned). If handed non-tabular data, the README's
explicit advice is to say so immediately rather than force-fitting the tabular
template's `03_deep_learning_model.py` (embeddings + MLP) to it, and instead reuse
just the six-phase shape (framing → features → baseline → model → HPO/CV
narration → perf/scaling) against a more appropriate architecture.

### MCP Agent System (Project 09)

#### Q: The README describes four "variations" (HITL approval, remote SSE servers, MCP Resources, Hierarchical Multi-Client) — are any of these actually implemented in the code here?
No — none of the four variations described in the README's section 4 are present
in `orchestrator.py`, `server_database.py`, or `server_weather.py`. They're
explicitly framed as "try these once the base architecture is working," i.e.
suggested extensions/prose patterns, not runnable code in this folder. The
Hierarchical Multi-Client variation specifically *is* actually implemented, but in
the separate Project 04 (`04-multi-agent-a2a-mcp`), where each of the three agents
genuinely owns a differently-scoped MCP client — Project 09's own README points
this out by cross-referencing Project 04 as "the runnable A2A-protocol
counterpart."

#### Q: How would you extend `server_database.py`'s security model for a real production deployment?
The current guard (string-prefix check for `SELECT`, a crude semicolon check) is
easy to defeat with more creative SQL (e.g. a `SELECT` that embeds a subquery
calling a dangerous stored procedure, or comment-based obfuscation) and doesn't use
parameterized queries or a query-plan-level allowlist. In production you'd instead
run the agent's DB access through a read-only database role/credential at the
connection level (so even a successfully-injected write statement fails at the
database, not just at an app-level string check), use parameterized queries
throughout, add row/column-level access control matching the calling agent's
permissions, log every executed query with the requesting agent's identity for
audit, and consider a query-plan analyzer or an allowlisted view layer instead of
raw ad hoc SQL strings from an LLM.

### General Portfolio/Project Discussion

#### Q: If you had to cut one of these six projects for time, which would you cut and why?
Project 09 is the most defensible cut, by the repo's own framing — its README
explicitly positions it as a "broader MCP surface-area tour... as a design
doc/spec," with Project 04 called out as "the runnable A2A-protocol counterpart"
that covers the more rigorous, protocol-accurate version of the same territory
(real MCP servers, a real Hierarchical Multi-Client pattern, plus a genuine A2A
implementation Project 09 lacks entirely). Project 09 does add distinct value —
it's the one place a real external REST API (Open-Meteo) gets wrapped as an MCP
tool, and the one place a live OpenAI function-calling loop against MCP tools is
shown — but structurally it overlaps most with Project 04.

#### Q: What would you say is the single most reused design decision across all six projects?
The leakage-safety discipline: fit every preprocessing step (imputation, scaling,
encoding) inside a single pipeline object, and fit that pipeline only on the
training fold, never on the full dataset before splitting. This shows up
identically in `train.py` and `train_dl.py` (Project 01), the template's
`01_feature_engineering.py` (Project 05), and is implicitly present in the RAG
pipeline's index/query separation (Project 02, where the index is built once from
the corpus and queries never leak into the index-building step) and the
fine-tuning project's before/after comparison on held-out prompts distinct from
training prompts (Project 03).

#### Q: None of these projects use a real production-grade experiment tracker (MLflow, Weights & Biases) or model registry — how would you defend that omission if asked?
It's a deliberate scope decision consistent with the stated goal of each project:
"sized to run on a laptop in minutes as a correctness smoke test," not a
production deployment. Each project's README instead documents, in prose, the path
to a production-grade result (Project 01's README explicitly separates
"which script maps to which interview phase" from what a real deployment needs;
Project 05's README has an entire section, "From trained model to production: the
steps in between," listing artifact versioning and a model-registry-equivalent
step as things to say out loud). The right answer in an interview is to
acknowledge the omission directly and describe exactly what you'd add (MLflow
Model Registry tagged with data version + commit hash + metrics, per Project 05's
own checklist) rather than pretend the toy version is already production-ready.

#### Q: How do these projects demonstrate "production scaling" thinking without actually deploying anything?
Each project that has a serving concern documents the actual mechanism, not just
the concept: Project 01's `serve.py` genuinely implements a stateless FastAPI
`/score` endpoint with the model loaded once per worker (`uvicorn serve:app
--workers 4` is horizontally scalable by construction) plus a chunked
(`chunksize=10_000`) batch-scoring path for nightly jobs bounded in memory. Project
04's three-agent system is already architected as genuinely separate OS processes
communicating over real HTTP and stdio — the "scaling" story there is that each
agent/server could become an independently deployed, independently scaled service
with no code changes, since the process boundary already exists. Project 05's
README spells out the full "trained model to production" and "post-deployment"
checklists (versioning, canary rollout, drift monitoring, label-loop closure) as
things to narrate even where no infra is actually stood up.

#### Q: If an interviewer asked you to combine two of these projects into one system, which two would combine most naturally, and how?
RAG (Project 02) and the multi-agent A2A/MCP system (Project 04) combine most
naturally: wrap Project 02's `retrieve()`/`generate()` pipeline as a new MCP tool
(e.g. `query_knowledge_base(question: str)`) exposed by a new MCP server, then give
the Research Agent in Project 04 an MCP client wired to that server alongside (or
instead of) `notes-server`. This would let the Research Agent ground its findings
in the RAG pipeline's chunked/embedded/reranked retrieval over a real document
corpus instead of the current flat-text `data/notes.txt` keyword search — a
direct, incremental upgrade path that reuses both projects' existing interfaces
(MCP tool exposure on one side, `retrieve()`/`generate()` as a Python function on
the other) without redesigning either one.

#### Q: Suppose an interviewer asks you to add real-time drift detection across the tabular pipeline (Project 01) and the RAG pipeline (Project 02) using a shared monitoring layer. How would you design that, given how differently the two projects define "input"?
The core challenge is that Project 01's input is structured, fixed-schema tabular
rows (so distributional drift is well-defined per column: PSI or KS-test on each
`ColumnTransformer` input feature, plus prediction-score drift on `/score`'s
output distribution) while Project 02's "input" is unstructured natural-language
queries and retrieved chunks, where classic per-column statistical drift tests
don't apply. A shared design: build one generic monitoring service that ingests
timestamped (input_summary, output, model_version) triples from both systems via a
common event schema, but compute *different* drift signals per source — for
Project 01, standard PSI/KS-test on numeric features and chi-squared on
categoricals, fed by instrumenting `serve.py`'s `/score` handler to emit an event
per request; for Project 02, track proxy signals instead of raw feature drift:
query-embedding centroid drift (mean pairwise cosine distance of a rolling window
of query embeddings vs. a baseline window), retrieval-hit-rate drift (are the same
top-k chunks being returned less often, suggesting corpus/query mismatch), and
citation-compliance rate (fraction of generated answers containing a valid
`[source:...]` tag) as an LLM-specific quality-drift proxy. Both feed the same
alerting/dashboard layer keyed by model/index version, so a single on-call rotation
watches one system even though the underlying statistics per project are
...
- Hard: 25 questions (extend/productionize/combine, deep tracing, staff-level judgment)
