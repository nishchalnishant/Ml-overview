...
1. Algorithms that need StandardScalerThese models assume the data is centered around 0 with a standard deviation of 1. They rely on distance metrics or gradient descent optimization.Linear & Logistic Regression: Scales the gradients to ensure fast, stable optimization.Support Vector Machines (SVM): Finds maximum margin boundaries; large scales distort the margins.K-Means Clustering: Relies entirely on Euclidean distance; unscaled features distort cluster shapes.Principal Component Analysis (PCA): Maximizes variance; unscaled variables with large ranges artificially dominate the components.Linear Discriminant Analysis (LDA): Assumes normal distribution and equal variance across features.
2. Algorithms that need MinMaxScalerThese models require data to be bounded within a strict, specific interval (usually 0 to 1 or -1 to 1).Neural Networks (Deep Learning): Essential when using activation functions like Sigmoid or Tanh, which saturate outside bounded ranges.K-Nearest Neighbors (KNN): While it works with StandardScaler, MinMaxScaler is preferred if the features have strict physical bounds (like image pixels).Image Processing (CNNs): Input pixel values (0–255) must be scaled to a 0–1 range for stable training.
3. Algorithms that don't need any scalingThese models handle features independently, or use rules that are completely unaffected by the scale of the numbers.Decision Trees: Split data using thresholds, looking only at the rank order of values.Random Forests: Ensembles of decision trees, inheriting the same scale-invariant properties.Gradient Boosting (XGBoost, LightGBM, CatBoost): Sequential tree models that use splitting thresholds.Naive Bayes: Calculates independent probabilities per feature, making it immune to scale differences.
