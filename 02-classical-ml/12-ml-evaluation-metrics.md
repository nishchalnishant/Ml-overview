...
Accuracy fails on imbalanced data. Use F1, PR-AUC, or MCC. ROC-AUC is threshold-independent but optimistic when positives are rare.

**Calibration?** #flashcard
ECE/Brier measure whether probabilities are trustworthy, not just whether hard predictions are correct.

**Regression?** #flashcard
MAE is robust to outliers; MSE penalizes large errors more. MAPE breaks near zero. R² alone is not enough.

**Ranking?** #flashcard
NDCG handles graded relevance with position discount; MAP/MRR for binary relevance. Precision@k vs recall@k depends on whether the corpus is bounded.

**Generative?** #flashcard
Human win rate is ground truth; automated metrics (BLEU, FID) are proxies that can be gamed.

**Offline vs Online?** #flashcard
Offline metrics are proxies. Proxy–outcome correlation drifts. Always set guardrail metrics before launching experiments.

**Statistical testing?** #flashcard
Always run a significance test. McNemar for classifiers; paired t-test or Wilcoxon for regression. Correct for multiple comparisons.

**Range?** #flashcard
$[-1, 1]$. $+1$ is perfect, $0$ is random, $-1$ is perfectly inverted.

**Uses all four cells of the confusion matrix?** #flashcard
works well for any class balance.

**Harder to explain to stakeholders than F1 but more informative mathematically.?** #flashcard
Harder to explain to stakeholders than F1 but more informative mathematically.

**A model predicting all negatives on a 99/1 split gets MCC = 0, not 0.99.?** #flashcard
A model predicting all negatives on a 99/1 split gets MCC = 0, not 0.99.
...
**Decompose?** #flashcard
$\text{Brier} = \text{Calibration} + \text{Resolution} - \text{Uncertainty}$ (Murphy decomposition).

**Proper scoring rule?** #flashcard
maximized in expectation only when $\hat{p}_i = P(y_i = 1)$.

**Skill score?** #flashcard
$\text{BSS} = 1 - \text{Brier} / \text{Brier}_{\text{ref}}$ (reference = always predict base rate).

**MAE?** #flashcard
median regression; outliers expected and shouldn't dominate.

**MSE?** #flashcard
mean regression; outliers are real signal you care about.

**Huber?** #flashcard
production systems with occasional bad labels or anomalous inputs.

**$R^2$?** #flashcard
fraction of variance explained. $R^2 = 1$ is perfect; $R^2 = 0$ matches a constant baseline. Can be negative for worse-than-baseline models.

**Adding features always increases $R^2$. Adjusted $R^2$ penalizes for number of predictors $p$.?** #flashcard
Adding features always increases $R^2$. Adjusted $R^2$ penalizes for number of predictors $p$.

**Pitfall: High $R^2$ does not guarantee a good model. Anscombe's quartet?** #flashcard
four datasets with identical $R^2$ but wildly different distributions.

**Minimizing pinball loss at quantile $\tau$ produces the $\tau$-th conditional quantile.?** #flashcard
Minimizing pinball loss at quantile $\tau$ produces the $\tau$-th conditional quantile.

**$\tau = 0.5$ recovers MAE.?** #flashcard
$\tau = 0.5$ recovers MAE.

**Use for prediction intervals?** #flashcard
fit $\tau = 0.1$ and $\tau = 0.9$ separately to get an 80% interval.

**Proper scoring rule for quantiles?** #flashcard
unlike symmetric losses.

**Range?** #flashcard
$[0, 1]$. NDCG = 1 means perfect ordering.

**Undefined if there are no relevant documents (IDCG = 0)?** #flashcard
handle by treating as 0 or excluding.

**$R$?** #flashcard
total number of relevant documents for the query.

**AP rewards finding relevant documents early.?** #flashcard
AP rewards finding relevant documents early.

**MAP averages AP across queries.?** #flashcard
MAP averages AP across queries.

**Sensitive to missing relevant items (unlike NDCG with fixed $k$).?** #flashcard
Sensitive to missing relevant items (unlike NDCG with fixed $k$).

**$\text{rank}_q$?** #flashcard
rank of the first relevant result for query $q$.

**Only cares about the first relevant hit?** #flashcard
ignores subsequent ones.

**Use when there is one "right" answer (e.g., QA, spelling correction, entity lookup).?** #flashcard
Use when there is one "right" answer (e.g., QA, spelling correction, entity lookup).
