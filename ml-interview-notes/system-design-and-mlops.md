...
A fashion recommendation model trained on winter inventory suddenly sees summer catalog traffic.
...
A fraud model was good last month, but fraudsters changed behavior and the old patterns no longer predict fraud well.
...
- error rate
- business KPI
- delayed label metrics when available

**Azure lens**

Imagine deploying a service and only tracking CPU.
You would laugh.

Same here.

Monitoring only model accuracy is not enough.
