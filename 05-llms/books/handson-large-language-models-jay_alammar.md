...
* Special tokens: Beyond subwords, tokenizers add structural tokens — `start-of-text` (marks the beginning of a sequence), `padding` (fills shorter sequences to a uniform length in a batch), and `unknown` (fallback for characters the vocabulary cannot represent). Vocabulary sizes typically range from 30,000 to 100,000 tokens depending on the model. Capitalization affects tokenization: the same word in different cases may produce different token IDs.
...
* RoPE (Rotary Positional Embeddings): A modern method for encoding the _order_ of words. Instead of just stamping a "1" or "2" on a word, it uses geometric properties to help the model understand relative positions. RoPE is applied _inside_ the attention calculation (to the Q and K matrices), not at the embedding input — this is a common misconception. _Context:_ A key component in newer models like Llama 2<sup>6</sup>.
* KV Cache: During inference, the model avoids recomputing attention for tokens it has already seen by caching the Key (K) and Value (V) matrices for each previous token across all layers. Only the new token's Q is computed fresh each step. This is what makes autoregressive generation fast but memory-intensive — cache size grows linearly with sequence length times batch size.
...
* Zero-shot embedding classification: An alternative to prompt engineering — encode the class labels themselves as embeddings (e.g., embed "positive" and "negative"), then at inference time embed the input and pick the nearest label by cosine similarity. No training data required. Achieves approximately 78% F1 on the Rotten Tomatoes benchmark, meaningfully weaker than the 85% achieved with a trained Logistic Regression on top of embeddings, but requires zero labeled examples.
