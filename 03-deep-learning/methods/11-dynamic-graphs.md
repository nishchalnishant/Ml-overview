...
> Graphs that change over time and methods to model, predict, and generate graph-structured data. Assumes familiarity with static GNNs (GCN, GraphSAGE, GAT, message passing) — see [04-specialized-domains/06-graph-neural-networks.md](../../04-specialized-domains/06-graph-neural-networks.md) first if those are unfamiliar.
