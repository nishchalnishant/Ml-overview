...
Backprop multiplies local derivatives across all layers; sigmoid's derivative maxes at 0.25 so it systematically shrinks gradients, while ReLU's is exactly 1 in the active region. Full derivation, numeric example, and fixes: see [components/01-backpropagation.md](components/01-backpropagation.md#vanishing-gradients).
