...
> For advanced retrieval patterns within agentic pipelines — including Self-RAG (the agent decides when to retrieve and critiques its own retrieved passages), Agentic RAG (multi-step retrieval loops), RAPTOR (hierarchical document summarization), MemGPT (virtual context management), and Mem0 (user-level persistent memory) — see [11-advanced-rag-and-memory.md](11-advanced-rag-and-memory.md). This section covers the memory architecture abstractions; that file covers the retrieval system internals.
