...
> For a full worked model card (fields filled in end-to-end, plus the "what breaks" failure modes — cards written once and never updated, out-of-scope uses documented but not technically enforced) see [model-governance.md §6](../../06-production-ml/model-governance.md#6-model-cards).
...
> For a full production implementation (SHAP-based adverse action notice generator, erasure request pipeline, breach notification timeline) see [model-governance.md §5](../../06-production-ml/model-governance.md#5-regulatory-compliance).
