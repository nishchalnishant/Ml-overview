...
A model card is a standardized one-to-two page document attached to a deployed model, covering Model Details, Intended Use, Factors, Metrics, Evaluation Data, Training Data, Quantitative Analysis, Ethical Considerations, and Caveats/Recommendations.

> For a full worked model card (fields filled in end-to-end, plus the "what breaks" failure modes — cards written once and never updated, out-of-scope uses documented but not technically enforced) see [model-governance.md §6](../../06-production-ml/03-model-governance.md#6-model-cards).
...
> For mechanistic interpretability depth (superposition hypothesis with toy model proofs, full SAE math and implementation, IOI circuit with 26 attention heads, induction heads, path patching, and scalability challenges), see [02-mechanistic-interpretability.md](02-mechanistic-interpretability.md). This section covers the practical LLM interpretability toolkit.
...
> Full SAE derivation, PyTorch implementation, IOI circuit analysis, induction heads, path patching, and scalability challenges: [02-mechanistic-interpretability.md](02-mechanistic-interpretability.md)
...
> For a full production implementation (SHAP-based adverse action notice generator, erasure request pipeline, breach notification timeline) see [model-governance.md §5](../../06-production-ml/03-model-governance.md#5-regulatory-compliance).
